...
# ★★★ [PATCHED — Hold-Out WF 분할, 2026-05-22] ★★★
# 학계의 Walk-Forward 중 single hold-out 방식.
# 룰은 코드 그대로(atom/sub-tier 등 고정), 거래 시기만 train/test 분할.
# 같은 1회 백테스트로 train/test 구간별 PF/WR/MDD 분리 계산.
#
# TRAIN_END_DATE 이전 = Train (룰이 fit됐다고 가정)
# TRAIN_END_DATE 이후 = Test (진짜 OOS)
#
# 기본 분할: 2022.01~2025.05 (40개월 train, 76%) / 2025.06~2026.05 (12개월 test, 24%)
# 환경변수로 변경: TRAIN_END_DATE=2024-12-31 python ...
import os as _os_holdout
TRAIN_END_DATE = _os_holdout.environ.get("TRAIN_END_DATE", "2025-05-31")
HOLDOUT_ENABLED = _os_holdout.environ.get("HOLDOUT_ENABLED", "1") == "1"
print(f"[HOLD-OUT] TRAIN_END_DATE = {TRAIN_END_DATE}  (이전=train, 이후=test)")
print(f"[HOLD-OUT] ENABLED = {HOLDOUT_ENABLED}")
...
def simulate_scenario_v19b_rpboost(scenario, candidates_dict, risk_multiplier=1.0):
    ...
    if len(equity_df) > 0:
        # ★★★ [PATCHED — MDD 2종 분리, 2026-05-22] ★★★
        # 크립토는 세금(tax) 시뮬이 없으므로 excess(인출)만 분리하면 됨.
        #   ① strategy : equity + excess 복원 (인출 안 했다면? = 순수 전략 손실폭)
        #   ② actual   : equity 그대로 (Phase B excess 인출까지 반영한 실잔고)
        # 기존 total_assets(=equity+excess) 가 이미 ① 에 해당하지만,
        # 인출 효과를 명확히 보이도록 ② 도 함께 계산.

        # --- ① 순수 전략 MDD: 인출 복원 (기존 total_assets) ---
        equity_df["assets_strategy"] = equity_df["equity"] + equity_df["cumulative_excess"]
        equity_df["cummax_strategy"] = equity_df["assets_strategy"].cummax()
        equity_df["dd_strategy"] = equity_df["assets_strategy"] - equity_df["cummax_strategy"]
        equity_df["dd_pct_strategy"] = np.where(
            equity_df["cummax_strategy"] > 0,
            equity_df["dd_strategy"] / equity_df["cummax_strategy"] * 100, 0
        )

        # --- ② 실제 통장 MDD: 인출 반영 (balance 그대로) ---
        equity_df["cummax_actual"] = equity_df["equity"].cummax()
        equity_df["dd_actual"] = equity_df["equity"] - equity_df["cummax_actual"]
        equity_df["dd_pct_actual"] = np.where(
            equity_df["cummax_actual"] > 0,
            equity_df["dd_actual"] / equity_df["cummax_actual"] * 100, 0
        )

        # 하위 호환: 기존 컬럼은 strategy 기준 유지 (= 기존 동작과 동일)
        equity_df["total_assets"] = equity_df["assets_strategy"]
        equity_df["cummax"] = equity_df["cummax_strategy"]
        equity_df["dd"] = equity_df["dd_strategy"]
        equity_df["dd_pct"] = equity_df["dd_pct_strategy"]
...
def print_scenario_summary(result_dict):
    ...
    # ★★★ [PATCHED — MDD 2종, 2026-05-22] ★★★
    mdd = equity_df["dd_pct"].min() if len(equity_df) > 0 else np.nan  # 하위호환(strategy)
    mdd_strategy = equity_df["dd_pct_strategy"].min() if len(equity_df) > 0 and "dd_pct_strategy" in equity_df else np.nan
    mdd_actual = equity_df["dd_pct_actual"].min() if len(equity_df) > 0 and "dd_pct_actual" in equity_df else np.nan
    ...
    print(f"  ──────────────────────────────────────────────")
    print(f"  📉 MDD 분해 (★ 인출 vs 진짜 손실 구분):")
    print(f"    ① MDD 순수전략 : {mdd_strategy:.2f}%" if pd.notna(mdd_strategy) else "    ① MDD 순수전략 : N/A")
    print(f"       (excess 인출 복원 = 인출 안 했다면? = 진짜 트레이딩 손실폭)")
    print(f"    ② MDD 실제통장 : {mdd_actual:.2f}%" if pd.notna(mdd_actual) else "    ② MDD 실제통장 : N/A")
    print(f"       (Phase B 도달 시 excess 인출까지 반영한 실잔고 변동)")
    print(f"       → ①과 ② 차이가 크면 = MDD 일부가 자발적 인출 때문 (손실 아님)")
    print(f"       → 크립토는 세금 시뮬 없음 (양도세 미적용)")
    print(f"  ──────────────────────────────────────────────")

    # ★★★ [PATCHED — Hold-Out WF 분할 출력, 2026-05-22] ★★★
    if HOLDOUT_ENABLED and len(trades_df) > 0 and "exit_time" in trades_df.columns:
        try:
            train_end_ts = pd.Timestamp(TRAIN_END_DATE, tz="UTC")

            # trades 분할 (exit_time 기준)
            tr_t = pd.to_datetime(trades_df["exit_time"], utc=True, errors="coerce")
            train_mask = tr_t <= train_end_ts
            test_mask = tr_t > train_end_ts
            train_trades = trades_df[train_mask]
            test_trades = trades_df[test_mask]

            # equity 분할 (time 기준)
            eq_t = pd.to_datetime(equity_df["time"], utc=True, errors="coerce") if len(equity_df) > 0 else None
            if eq_t is not None and len(eq_t) > 0:
                eq_train = equity_df[eq_t <= train_end_ts]
                eq_test = equity_df[eq_t > train_end_ts]
            else:
                eq_train = pd.DataFrame(); eq_test = pd.DataFrame()

            def calc_stats(trades_seg, eq_seg, label):
                """구간별 PF/WR/Avg_R/MDD 계산"""
                n = len(trades_seg)
                if n == 0:
                    return dict(label=label, n=0, wr=np.nan, pf=np.nan, avg_r=np.nan,
                                mdd_strat=np.nan, mdd_actual=np.nan, total_r=np.nan)
                wins = trades_seg[trades_seg["net_pnl"] > 0]
                losses = trades_seg[trades_seg["net_pnl"] <= 0]
                gw = wins["net_pnl"].sum()
                gl = abs(losses["net_pnl"].sum())
                pf = gw / gl if gl > 0 else float("inf")
                wr = len(wins) / n * 100
                avg_r = trades_seg["r_multiple"].mean() if "r_multiple" in trades_seg.columns else np.nan
                total_r = trades_seg["r_multiple"].sum() if "r_multiple" in trades_seg.columns else np.nan

                # 해당 구간의 equity로 MDD 재계산 (구간 시작점을 peak로 리셋)
                mdd_s = np.nan; mdd_a = np.nan
                if len(eq_seg) > 0 and "assets_strategy" in eq_seg.columns:
                    seg = eq_seg.copy()
                    seg["cummax_s"] = seg["assets_strategy"].cummax()
                    seg["dd_s"] = np.where(seg["cummax_s"] > 0,
                                            (seg["assets_strategy"] - seg["cummax_s"]) / seg["cummax_s"] * 100, 0)
                    mdd_s = seg["dd_s"].min()
                    seg["cummax_a"] = seg["equity"].cummax()
                    seg["dd_a"] = np.where(seg["cummax_a"] > 0,
                                            (seg["equity"] - seg["cummax_a"]) / seg["cummax_a"] * 100, 0)
                    mdd_a = seg["dd_a"].min()

                return dict(label=label, n=n, wr=wr, pf=pf, avg_r=avg_r,
                            mdd_strat=mdd_s, mdd_actual=mdd_a, total_r=total_r)

            train_stats = calc_stats(train_trades, eq_train, "TRAIN")
            test_stats = calc_stats(test_trades, eq_test, "TEST")
            full_stats = calc_stats(trades_df, equity_df, "FULL")

            print(f"\n  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
            print(f"  📊 HOLD-OUT WF 분석 (TRAIN_END_DATE = {TRAIN_END_DATE})")
            print(f"  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
            print(f"  구간       거래  승률    PF      Avg_R   총R     MDD순수  MDD실제")
            for s in [train_stats, test_stats, full_stats]:
                if s["n"] == 0:
                    print(f"  {s['label']:<10} {s['n']:>4}  (no trades)")
                else:
                    print(f"  {s['label']:<10} {s['n']:>4}  {s['wr']:>5.1f}%  "
                          f"{s['pf']:>5.2f}   {s['avg_r']:>5.2f}   {s['total_r']:>5.1f}   "
                          f"{s['mdd_strat']:>6.2f}%  {s['mdd_actual']:>6.2f}%")
            print(f"  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")

            # 진단 메시지
            if train_stats["n"] > 0 and test_stats["n"] > 0 and pd.notna(train_stats["pf"]) and pd.notna(test_stats["pf"]):
                pf_drop = (train_stats["pf"] - test_stats["pf"]) / train_stats["pf"] * 100
                wr_drop = train_stats["wr"] - test_stats["wr"]
                if pf_drop < 10 and abs(wr_drop) < 5:
                    print(f"  ✅ 강건성 양호: TEST PF가 TRAIN 대비 {pf_drop:+.1f}% (10% 이내)")
                    print(f"     → 룰이 OOS에서도 유지됨. 진짜 alpha 가능성 높음.")
                elif pf_drop < 30:
                    print(f"  🟡 약간 감소: TEST PF가 TRAIN 대비 {pf_drop:+.1f}%")
                    print(f"     → 부분 in-sample fit. 실전엔 다소 보수적 운용 권장.")
                else:
                    print(f"  ⚠️ 큰 감소: TEST PF가 TRAIN 대비 {pf_drop:+.1f}%")
                    print(f"     → in-sample 과최적화 의심. 실전 신중 검토 필요.")
            print(f"  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")

            # CSV 저장
            holdout_df = pd.DataFrame([train_stats, test_stats, full_stats])
            holdout_csv = f"{OUTDIR}/{prefix}_holdout_summary.csv"
            holdout_df.to_csv(holdout_csv, index=False)
            print(f"  💾 hold-out 결과 저장: {holdout_csv}")
        except Exception as e:
            print(f"  [HOLD-OUT WARN] split 분석 실패: {e}")
...
OUTDIR = "stage4l_redist_outputs_v3_conservative"
os.makedirs(OUTDIR, exist_ok=True)
print(f"\n[OUTDIR] {os.path.abspath(OUTDIR)}")
print(f"[CONFIG] OUT_BEHAVIOR = {OUT_BEHAVIOR!r}")
...
for SCENARIO_CURRENT in SCENARIOS_LIST:
    ...
    mdd_overall = eq['dd_pct'].min() if len(eq)>0 else 0  # 하위호환(strategy)
    mdd_overall_strategy = eq['dd_pct_strategy'].min() if len(eq)>0 and 'dd_pct_strategy' in eq else 0
    mdd_overall_actual = eq['dd_pct_actual'].min() if len(eq)>0 and 'dd_pct_actual' in eq else 0
    ...
    summary_row = {
        ...
        "PF":                pf_overall,
        "avg_R":             avgr_overall,
        "MDD_strategy_%":    mdd_overall_strategy,
        "MDD_actual_%":      mdd_overall_actual,
        "MDD%":              mdd_overall,
        "Return_%":          return_pct,
