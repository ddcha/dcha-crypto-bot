# =========================================================
# smc_crypto_9coins  STAGE 4A: TAG-ONLY EXPERIMENT
#
# [실험 목적]
#   Score 7.5 P/F만 유일 게이트로 유지.
#   4개 필터(D1_SELF, SWEEP, VOLUME, BTC_TREND)는 전부 태깅만.
#   게이트 역할 일체 없음. 로그에서 각 trade의 근거 조합을
#   기록해서 cross-sectional 분석으로 시너지 발견.
#
# [Stage 3 대비 변경사항]
#
#   ❌ 제거
#     - FILTER_GROUPS 로직 전체 (USE_FILTER_GROUPS = False 고정)
#     - Tier 시스템 (classify_tier_v19b_rp_boost 호출 제거)
#     - RP_BOOST (RP_TIER_MULT_TABLE 미사용, tier_mult = 1.0 고정)
#     - S/A cap FREE (tier 없으니 불필요, effective_notional_cap = 3.0 고정)
#     - PULLBACK_DEPTH / ATR 필터 (원래 OFF 상태 유지)
#
#   ✅ 추가
#     - BTC_TREND 태깅 (BTC D1 trend 정렬 여부)
#     - tag_d1_self, tag_sweep, tag_volume, tag_btc_trend (4개)
#     - tag_combo (정렬된 조합 문자열), tag_count (0~4)
#     - 분석 산출물 6개 자동 생성:
#       * filter_solo_analysis.csv  — 각 필터 단독 효과
#       * filter_combo_analysis.csv — 16가지 조합 성과
#       * pair_synergy_matrix.csv   — 2-필터 시너지 행렬
#       * tag_count_effect.csv      — 통과 개수별 효과
#       * btc_trend_deep_dive.csv   — BTC 필터 연도별 분석
#       * side_x_filter.csv         — LONG/SHORT × 필터 효과
#
# [실행 구조]
#   단일 시나리오 (Case 분리 없음) — 태깅만이 목적이므로 risk 조합 불필요
#
# [유지]
#   - 9코인 / MIN_SCORE 7.5 / REFINE 제거 / H4 exit
#   - 단일 포지션 / 병렬화
#   - 실전 근사 fill (SHORT: zone_low, LONG: zone_high)
#
# [Risk 설정]
#   - risk_pct = risk_pct_base × 1.0 × 1.0 (tier_mult 없음, risk_multiplier=1.0)
#   - tier/RP 곱 제거로 Return% 감소 예상 — 실험 목적은 PF/Win% 분석
# =========================================================
...
# =========================================================
# ★★★ Stage 4A: TAG-ONLY 시스템 ★★★
# =========================================================
# FILTER_GROUPS 완전 비활성 (태깅만 사용, 게이트 아님)
USE_FILTER_GROUPS = False

# 하위 호환용 (절대 True로 만들지 말 것)
FILTER_GROUPS = []

# ─── Stage 4A 핵심 플래그 ────────────────────────────
# Tier/RP_BOOST 완전 제거
USE_TIER_SYSTEM = False        # tier 계산 호출 자체를 skip
FIXED_TIER_MULT = 1.0          # tier_mult 고정값 (실험 기간 내내 1.0)

# S/A cap FREE 제거 (tier 없으므로)
FIXED_NOTIONAL_CAP = MAX_NOTIONAL_MULT  # = 3.0 (전거래 공통)

# BTC 태깅 설정
BTC_SYMBOL_FOR_TAG = "BTCUSDT"  # BTC_TREND 태그 기준 심볼
...
# =========================================================
# ★★★ Stage 4A: TAG-ONLY 헬퍼 ★★★
# =========================================================
# BTC D1 trend을 전역 캐시 (심볼 루프 전에 1회 계산, 모든 entry에 공통 lookup)
_BTC_D1_TREND_CACHE = None

def set_btc_d1_trend_cache(btc_d1_df):
    """백테스트 시작 전 BTC D1 trend DataFrame을 전역에 저장."""
    global _BTC_D1_TREND_CACHE
    _BTC_D1_TREND_CACHE = btc_d1_df


def get_btc_d1_trend_cache():
    return _BTC_D1_TREND_CACHE


def compute_trade_tags(df_struct, df_d1_self, entry_idx, zone_created_idx,
                       zone_low, zone_high, side, row_open_price):
    """
    Stage 4A 전용: 각 trade에 4개 태그 계산 (진입 결정에 영향 없음, 로그용).
    
    태그:
        tag_d1_self    : 자체 코인 D1 trend 정렬
        tag_sweep      : entry 직전 sweep 발생 (pivot or recent)
        tag_volume     : zone candle volume >= 평균 × VOLUME_SPIKE_MULT
        tag_btc_trend  : BTC D1 trend 정렬 (전역 캐시에서 lookup)
    
    Returns:
        dict: {
            "tag_d1_self": bool, "tag_sweep": bool,
            "tag_volume": bool, "tag_btc_trend": bool,
            "tag_combo": str,   "tag_count": int,
        }
    """
    tags = {}
    entry_ts = df_struct.loc[entry_idx, "timestamp"]

    # ── tag_d1_self ──
    sub = df_d1_self[df_d1_self["timestamp"] < entry_ts]
    if len(sub) == 0:
        tags["tag_d1_self"] = False
    else:
        d1_trend = sub.iloc[-1]["d1_trend"]
        if side == "long":
            tags["tag_d1_self"] = (d1_trend == "up")
        else:
            tags["tag_d1_self"] = (d1_trend == "down")

    # ── tag_btc_trend ──
    btc_cache = get_btc_d1_trend_cache()
    if btc_cache is None or len(btc_cache) == 0:
        tags["tag_btc_trend"] = False
    else:
        sub_btc = btc_cache[btc_cache["timestamp"] < entry_ts]
        if len(sub_btc) == 0:
            tags["tag_btc_trend"] = False
        else:
            btc_trend = sub_btc.iloc[-1]["d1_trend"]
            if side == "long":
                tags["tag_btc_trend"] = (btc_trend == "up")
            else:
                tags["tag_btc_trend"] = (btc_trend == "down")
    ...
    if side == "long":
        ...
        pivot_sw = df_struct.loc[start:end, "pivot_sweep_high"].any()
        recent_sw = df_struct.loc[start:end, "recent_sweep_high"].any()
    tags["tag_sweep"] = bool(pivot_sw or recent_sw)
    ...
    if "volume" in df_struct.columns:
        ...
        if vend <= vstart:
            ...
            if pd.isna(avg_vol) or avg_vol <= 0:
                ...
                tags["tag_volume"] = (zone_vol / avg_vol) >= VOLUME_SPIKE_MULT
    else:
        tags["tag_volume"] = False
    ...
    passed_list = [
        name for name, v in [
            ("D1_SELF", tags["tag_d1_self"]),
            ("SWEEP", tags["tag_sweep"]),
            ("VOLUME", tags["tag_volume"]),
            ("BTC_TREND", tags["tag_btc_trend"]),
        ] if v
    ]
    tags["tag_combo"] = "+".join(sorted(passed_list)) if passed_list else "none"
    tags["tag_count"] = len(passed_list)
    return tags
...
def generate_candidates_from_prepared(prepared):
    ...
    while i < len(df_struct) - 1:
        ...
        for s, eff_score, fresh_reasons in scored_active:
            ...
            # ⭐⭐ Stage 4A: 태깅만 (진입 게이트 아님)
            _tags = compute_trade_tags(
                df_struct=df_struct, df_d1_self=df_d1,
                entry_idx=i, zone_created_idx=s["zone_created_idx"],
                zone_low=s["zone_low"], zone_high=s["zone_high"],
                side=s["type"], row_open_price=row["open"],
            )
            # 하위 호환: 기존 _filter_passes / _pass_combo / _pass_count 자리에 태그값 매핑
            _filter_passes = {
                "pass_vol":   _tags["tag_volume"],
                "pass_sweep": _tags["tag_sweep"],
                "pass_d1":    _tags["tag_d1_self"],
                "pass_pull":  False,          # Stage 4A 에서 제거
                "pass_atr":   False,          # Stage 4A 에서 제거
            }
            _pass_combo = _tags["tag_combo"]
            _pass_count = _tags["tag_count"]
            # USE_FILTER_GROUPS = False 이므로 아래 블록이 skip 되고 기존 필터(USE_VOLUME_FILTER 등)는 모두 OFF

            if USE_FILTER_GROUPS:
                # (Stage 4A 에서는 USE_FILTER_GROUPS = False 고정)
        ...
        candidates.append({
            ...
            # ⭐⭐ Stage 4A: 태그 기록 (진입 결정 영향 없음)
            "tag_d1_self":   bool(_tags.get("tag_d1_self", False)),
            "tag_sweep":     bool(_tags.get("tag_sweep", False)),
            "tag_volume":    bool(_tags.get("tag_volume", False)),
            "tag_btc_trend": bool(_tags.get("tag_btc_trend", False)),
            "tag_combo":     _tags.get("tag_combo", "none"),
            "tag_count":     int(_tags.get("tag_count", 0)),
            # 하위 호환 (기존 CSV 스키마 유지)
    ...
    for current_time in event_times:
        ...
        if current_time in entry_map:
            for idx, row in entry_map[current_time]:
                ...
                # ⭐⭐ Stage 4A: Tier 시스템 완전 제거
                #   - classify_tier_v19b_rp_boost 호출 안 함
                #   - tier_mult = 1.0 고정
                #   - RP_BOOST 없음
                #   - S/A cap FREE 없음 (effective_notional_cap = 3.0 고정)
                #   - 단, 로그 스키마 호환을 위해 tier_label / rp_action 필드는 유지
                ...
                tier_label = "N/A"
                tier_mult  = FIXED_TIER_MULT      # 1.0
                rp_action  = "tier_off_stage4a"

                risk_pct = risk_pct_base * tier_mult * risk_multiplier  # = risk_pct_base (tier_mult=1, mult=1)

                # Stage 4A: notional cap 고정 (S/A cap FREE 없음)
                effective_notional_cap = FIXED_NOTIONAL_CAP  # = MAX_NOTIONAL_MULT = 3.0
                sa_cap_tag = "capFixed"
                ...
                open_positions[idx] = {
                    ...
                    # ⭐⭐ Stage 4A: 태그 컬럼 (분석의 본체)
                    "tag_d1_self":   bool(row.get("tag_d1_self", False)),
                    "tag_sweep":     bool(row.get("tag_sweep", False)),
                    "tag_volume":    bool(row.get("tag_volume", False)),
                    "tag_btc_trend": bool(row.get("tag_btc_trend", False)),
                    "tag_combo":     row.get("tag_combo", "none"),
...
# =========================================================
# ★★★ Stage 4A: BTC D1 trend 전역 캐시 구축 ★★★
# =========================================================
print()
print("=" * 75)
print("🪙 BTC D1 trend 사전 계산 (전역 캐시, 모든 심볼 공통 참조)")
print("=" * 75)

if BTC_SYMBOL_FOR_TAG not in prepared_data:
    raise RuntimeError(f"BTC 심볼 '{BTC_SYMBOL_FOR_TAG}' 이 prepared_data 에 없음. SYMBOLS_TO_PREPARE 확인 필요.")

# prepared_data[sym] 은 이미 df_d1 (D1 trend 계산 완료) 를 포함 (build_d1_trend 호출됨).
# BTC 전용으로 그대로 재사용 (추가 계산 불필요).
_btc_d1_df = prepared_data[BTC_SYMBOL_FOR_TAG]["df_d1"]
set_btc_d1_trend_cache(_btc_d1_df)

_btc_up   = (_btc_d1_df["d1_trend"] == "up").sum()
_btc_down = (_btc_d1_df["d1_trend"] == "down").sum()
_btc_neu  = (_btc_d1_df["d1_trend"] == "neutral").sum()
print(f"  BTC D1 bars: {len(_btc_d1_df)} | up: {_btc_up} | down: {_btc_down} | neutral: {_btc_neu}")
print(f"  ✅ BTC 태그 lookup 준비 완료 (prepared_data[{BTC_SYMBOL_FOR_TAG}]['df_d1'] 재사용)")


# =========================================================
# ★★★ Stage 4A: candidates 생성 (단일 버전 — 태깅만, 게이트 없음) ★★★
# =========================================================
print()
print("=" * 75)
print("🎯 candidates 생성 — Stage 4A TAG-ONLY (모든 게이트 OFF, 태깅만)")
print("=" * 75)

USE_FILTER_GROUPS = False    # ⭐ 필터 그룹 비활성 (절대 True 금지)
USE_VOLUME_FILTER = False    # 모든 진입 필터 OFF
USE_LIQUIDITY_SWEEP_CONF = False
USE_D1_TREND_FILTER = False
USE_PULLBACK_DEPTH = False
USE_ATR_FILTER = False
...
try:
    candidates_results_stage4a = _parallel_map(
        _generate_candidates_single, SYMBOLS_TO_PREPARE,
        task_name="candidates Stage 4A TAG-ONLY", io_bound=False,
    )
    candidates_dict_stage4a = dict(candidates_results_stage4a)
except Exception as e:
    print(f"  ⚠️ candidates 병렬 실패: {e}, 순차 처리")
    candidates_dict_stage4a = {}
    for sym in SYMBOLS_TO_PREPARE:
        candidates_dict_stage4a[sym] = generate_candidates_from_prepared(prepared_data[sym])

total_stage4a = sum(len(candidates_dict_stage4a[s]["candidates"]) for s in SYMBOLS_TO_PREPARE)
print(f"\n  TOTAL candidates (Stage 4A): {total_stage4a}")
print("\n✅ candidates 생성 완료 (Stage 4A 단일 버전).")


# =========================================================
# ★★★ 셀 3: 단일 시나리오 백테스트 실행 (Stage 4A) ★★★
# =========================================================
import os
OUTDIR = "stage4a_outputs"
os.makedirs(OUTDIR, exist_ok=True)
print(f"\n📂 OUTDIR = {os.path.abspath(OUTDIR)}")

# Stage 4A 단일 시나리오 (risk_multiplier=1.0 고정, candidates_dict_stage4a 사용)
STAGE4A_CASE = {
    "name":  "stage4a",
    "label": "Stage 4A TAG-ONLY (tier=1.0, all filters tagging only)",
    "risk_mult": 1.0,
    "candidates": candidates_dict_stage4a,
}

print()
print("█" * 75)
print(f"█ Stage 4A 백테스트 시작: {STAGE4A_CASE['label']}")
print("█" * 75)

res = simulate_scenario_v19b_rpboost(
    scenario=SCENARIO_MULTI,
    candidates_dict=STAGE4A_CASE["candidates"],
    risk_multiplier=STAGE4A_CASE["risk_mult"],
)
...
retirement_month = qualified.iloc[0]['month'] if len(qualified)>0 else "미달성"
months_taken = monthly_report[monthly_report['month'] == retirement_month].index[0] + 1 if len(qualified)>0 else None

summary_df = pd.DataFrame([{
    "scenario":  STAGE4A_CASE["name"],
    "label":     STAGE4A_CASE["label"],
    "trades":    len(trades),
    "win%":      wr_overall,
    "PF":        pf_overall,
    "avg_R":     avgr_overall,
    "MDD%":      mdd_overall,
    "Total_억KRW":  final_total_krw / 1e8,
    "Excess_억KRW": final_excess_krw / 1e8,
    "Return_%":  return_pct,
    "퇴사":      retirement_month,
    "퇴사_개월": months_taken if months_taken else "X",
}])
summary_df.to_csv(f"{OUTDIR}/stage4a_overall_summary.csv", index=False)

print()
print("=" * 75)
print("📊 Stage 4A 전체 성과")
print("=" * 75)
...
print(f"  퇴사     : {retirement_month} ({months_taken if months_taken else 'X'}개월)")


# =========================================================
# ★★★ Stage 4A: 태그 기반 심층 분석 (본 실험의 본체) ★★★
# =========================================================
print()
print("█" * 75)
print("█ 🔬 Stage 4A TAG-ONLY 심층 분석 — 태그 교차분석")
print("█" * 75)
...
if len(tdf) == 0:
    ...
    # 태그 컬럼 존재 확인
    tag_cols = ["tag_d1_self", "tag_sweep", "tag_volume", "tag_btc_trend"]
    for c in tag_cols:
    ...
    def _stats(sub):
        if len(sub) == 0:
            return {
                "trades": 0, "win_pct": 0.0, "avg_R": 0.0,
                "PF": 0.0, "total_pnl": 0.0, "pnl_per_trade": 0.0,
            }
        return {
            "trades": len(sub),
    ...
    # ─────────────────────────────────────────────
    # (A) filter_solo_analysis.csv — 각 필터 단독 효과 (통과 vs 미통과)
    # ─────────────────────────────────────────────
    print()
    print("=" * 75)
    print("📊 [A] filter_solo_analysis — 각 필터 PASS vs FAIL")
    print("=" * 75)
    print(f"{'tag':<14} {'상태':<6} {'trades':>7} {'win%':>7} {'avg_R':>7} {'PF':>7} {'total $':>14}")
    print("-" * 75)

    solo_rows = []
    for c in tag_cols:
        for status, sub in [("PASS", tdf[tdf[c] == True]),
                             ("FAIL", tdf[tdf[c] == False])]:
            st = _stats(sub)
            solo_rows.append({
                "tag": c.replace("tag_", "").upper(),
                "status": status,
                **st,
            })
            print(f"{c.replace('tag_','').upper():<14} {status:<6} {st['trades']:>7} "
                  f"{st['win_pct']:>6.2f}% {st['avg_R']:>7.3f} {st['PF']:>7.3f} "
                  f"{st['total_pnl']:>14,.0f}")

    solo_df = pd.DataFrame(solo_rows).round(4)
    solo_df.to_csv(f"{OUTDIR}/filter_solo_analysis.csv", index=False)

    # ─────────────────────────────────────────────
    # (B) filter_combo_analysis.csv — 16가지 조합 (2^4)
    # ─────────────────────────────────────────────
    print()
    print("=" * 75)
    print("📊 [B] filter_combo_analysis — 16가지 tag 조합 성과")
    print("=" * 75)

    combo_stat = tdf.groupby("tag_combo").agg(
        trades=("symbol", "count"),
        win_pct=("net_pnl", lambda s: (s > 0).mean() * 100),
        avg_R=("r_multiple", "mean"),
        total_pnl=("net_pnl", "sum"),
        pnl_per_trade=("net_pnl", "mean"),
    ).reset_index()

    def _pf_by_combo(combo):
        return _pf_of(tdf[tdf["tag_combo"] == combo])

    combo_stat["PF"] = combo_stat["tag_combo"].apply(_pf_by_combo)
    combo_stat = combo_stat.sort_values("total_pnl", ascending=False).reset_index(drop=True)
    combo_stat = combo_stat.round({"win_pct": 2, "avg_R": 3, "total_pnl": 2, "pnl_per_trade": 2, "PF": 3})

    print(f"{'tag_combo':<42} {'trades':>7} {'win%':>7} {'avg_R':>7} {'PF':>7} {'$/trade':>10} {'total $':>14}")
    print("-" * 100)
    for _, r in combo_stat.iterrows():
        combo_str = r["tag_combo"][:40] if len(r["tag_combo"]) > 40 else r["tag_combo"]
        print(f"{combo_str:<42} {int(r['trades']):>7} {r['win_pct']:>6.2f}% "
              f"{r['avg_R']:>7.3f} {r['PF']:>7.3f} "
              f"{r['pnl_per_trade']:>10,.0f} {r['total_pnl']:>14,.0f}")

    combo_stat.to_csv(f"{OUTDIR}/filter_combo_analysis.csv", index=False)

    # ─────────────────────────────────────────────
    # (C) pair_synergy_matrix.csv — 2-필터 시너지 행렬 (PF 기준)
    # ─────────────────────────────────────────────
    print()
    print("=" * 75)
    print("📊 [C] pair_synergy_matrix — 2-필터 조합 PF 행렬 (대각선=solo)")
    print("=" * 75)

    tag_names_short = ["D1_SELF", "SWEEP", "VOLUME", "BTC_TREND"]
    tag_to_col = dict(zip(tag_names_short,
                          ["tag_d1_self", "tag_sweep", "tag_volume", "tag_btc_trend"]))

    header = " " * 14 + "".join([f"{n:>13}" for n in tag_names_short])
    print(header)
    pair_rows = []
    for r_name in tag_names_short:
        row_print = f"{r_name:<14}"
        r_col = tag_to_col[r_name]
        for c_name in tag_names_short:
            c_col = tag_to_col[c_name]
            if r_name == c_name:
                sub = tdf[tdf[r_col] == True]
                cell_pf = _pf_of(sub)
                cell_n  = len(sub)
            else:
                sub = tdf[(tdf[r_col] == True) & (tdf[c_col] == True)]
                cell_pf = _pf_of(sub)
                cell_n  = len(sub)
            pair_rows.append({
                "row_tag": r_name, "col_tag": c_name,
                "is_diag": (r_name == c_name),
                "trades": cell_n, "PF": round(cell_pf, 3),
            })
            row_print += f"  {cell_pf:>5.2f}({cell_n:>4})"
        print(row_print)

    pair_df = pd.DataFrame(pair_rows)
    pair_df.to_csv(f"{OUTDIR}/pair_synergy_matrix.csv", index=False)
    print("\n  해석: (PF)(trades) — 대각선 solo 대비 pair 값이 크면 양의 시너지")

    # ─────────────────────────────────────────────
    # (D) tag_count_effect.csv — 통과한 태그 수별 효과
    # ─────────────────────────────────────────────
    print()
    print("=" * 75)
    print("📊 [D] tag_count_effect — 통과한 태그 수별 성과")
    print("=" * 75)

    tc_stat = tdf.groupby("tag_count").agg(
        trades=("symbol", "count"),
        win_pct=("net_pnl", lambda s: (s > 0).mean() * 100),
        avg_R=("r_multiple", "mean"),
        total_pnl=("net_pnl", "sum"),
    ).reset_index()
    tc_stat["PF"] = tc_stat["tag_count"].apply(lambda k: _pf_of(tdf[tdf["tag_count"] == k]))
    tc_stat = tc_stat.round({"win_pct": 2, "avg_R": 3, "total_pnl": 2, "PF": 3})

    print(f"{'tag_count':<12} {'trades':>7} {'win%':>7} {'avg_R':>7} {'PF':>7} {'total $':>14}")
    print("-" * 75)
    for _, r in tc_stat.iterrows():
        print(f"{int(r['tag_count']):<12} {int(r['trades']):>7} {r['win_pct']:>6.2f}% "
              f"{r['avg_R']:>7.3f} {r['PF']:>7.3f} {r['total_pnl']:>14,.0f}")

    tc_stat.to_csv(f"{OUTDIR}/tag_count_effect.csv", index=False)

    # ─────────────────────────────────────────────
    # (E) btc_trend_deep_dive.csv — BTC 필터 연도별 심층 분석
    # ─────────────────────────────────────────────
    print()
    print("=" * 75)
    print("📊 [E] btc_trend_deep_dive — BTC 태그 연도별/분기별")
    print("=" * 75)

    tdf["_year"]  = pd.to_datetime(tdf["entry_time"], utc=True).dt.year
    tdf["_yq"]    = pd.to_datetime(tdf["entry_time"], utc=True).dt.to_period("Q").astype(str)

    btc_rows = []
    for period_col in ("_year", "_yq"):
        for period_val in sorted(tdf[period_col].dropna().unique().tolist()):
            sub = tdf[tdf[period_col] == period_val]
            for aligned in (True, False):
                sub2 = sub[sub["tag_btc_trend"] == aligned]
                st = _stats(sub2)
                btc_rows.append({
                    "granularity": "year" if period_col == "_year" else "yearQ",
    ...
    # 전체 기간
    for aligned in (True, False):
        sub2 = tdf[tdf["tag_btc_trend"] == aligned]
        st = _stats(sub2)
        btc_rows.append({
            "granularity": "all",
            "period": "ALL",
    ...
    btc_df = pd.DataFrame(btc_rows).round(4)
    btc_df.to_csv(f"{OUTDIR}/btc_trend_deep_dive.csv", index=False)

    # 연도별 표 출력
    print(f"{'period':<10} {'aligned':<8} {'trades':>7} {'win%':>7} {'PF':>7} {'total $':>14}")
    print("-" * 75)
    year_sub = btc_df[btc_df["granularity"].isin(["year", "all"])].copy()
    year_sub = year_sub.sort_values(["granularity", "period", "btc_aligned"], ascending=[True, True, False])
    for _, r in year_sub.iterrows():
        print(f"{str(r['period']):<10} {str(bool(r['btc_aligned'])):<8} "
              f"{int(r['trades']):>7} {r['win_pct']:>6.2f}% {r['PF']:>7.3f} "
              f"{r['total_pnl']:>14,.0f}")

    # ─────────────────────────────────────────────
    # (F) side_x_filter.csv — LONG vs SHORT × 4개 태그
    # ─────────────────────────────────────────────
    print()
    print("=" * 75)
    print("📊 [F] side_x_filter — LONG vs SHORT × 각 태그 효과")
    print("=" * 75)

    sxf_rows = []
    print(f"{'side':<6} {'tag':<14} {'상태':<6} {'trades':>7} {'win%':>7} {'PF':>7} {'total $':>14}")
    print("-" * 80)
    for side_val in ("long", "short"):
        side_sub = tdf[tdf["side"] == side_val]
        for c in tag_cols:
            for status, sub in [("PASS", side_sub[side_sub[c] == True]),
                                 ("FAIL", side_sub[side_sub[c] == False])]:
                st = _stats(sub)
                sxf_rows.append({
                    "side": side_val,
                    "tag": c.replace("tag_", "").upper(),
                ...
                print(f"{side_val:<6} {c.replace('tag_','').upper():<14} {status:<6} "
                      f"{st['trades']:>7} {st['win_pct']:>6.2f}% {st['PF']:>7.3f} "
                      f"{st['total_pnl']:>14,.0f}")

    sxf_df = pd.DataFrame(sxf_rows).round(4)
    sxf_df.to_csv(f"{OUTDIR}/side_x_filter.csv", index=False)

    # 정리
    tdf.drop(columns=["_year", "_yq"], inplace=True, errors="ignore")

print()
print("=" * 75)
print("💾 Stage 4A 분석 산출물 저장 완료")
print("=" * 75)
print(f"  {OUTDIR}/stage4a_trades.csv")
print(f"  {OUTDIR}/stage4a_equity.csv")
print(f"  {OUTDIR}/stage4a_skipped.csv")
print(f"  {OUTDIR}/stage4a_monthly.csv")
print(f"  {OUTDIR}/stage4a_overall_summary.csv")
print(f"  {OUTDIR}/filter_solo_analysis.csv")
print(f"  {OUTDIR}/filter_combo_analysis.csv")
print(f"  {OUTDIR}/pair_synergy_matrix.csv")
print(f"  {OUTDIR}/tag_count_effect.csv")
print(f"  {OUTDIR}/btc_trend_deep_dive.csv")
print(f"  {OUTDIR}/side_x_filter.csv")

print()
print("=" * 75)
print("💡 해석 가이드")
print("=" * 75)
print("  (A) filter_solo : PASS vs FAIL PF delta 가 큰 태그 = 진짜 alpha")
print("  (B) filter_combo: total_pnl 상위 조합이 실제 돈 번 신호 집합")
print("  (C) pair_matrix : pair PF > max(row solo, col solo) → 양의 시너지")
print("  (D) tag_count   : 단조 증가면 '많이 통과할수록 좋다', 그 외는 비선형")
print("  (E) btc_deep    : BTC 태그 효과가 연도별/분기별로 안정적인지")
print("  (F) side_x      : LONG/SHORT 에서 태그 효과가 달라지는지 (bias 감지)")

print()
print("=" * 75)
print("🎯 Stage 4A 실험 완료")
print("=" * 75)
print("  설정: tier 제거, FILTER_GROUPS 제거, 게이트 MIN_SCORE 7.5 뿐")
print("  4 태그 (D1_SELF, SWEEP, VOLUME, BTC_TREND) 는 로그 전용")
print("  → Case C (Stage 3 NO FILTER) 대비 Return 감소 예상 (tier×RP boost 없음)")
print("  → 핵심 산출물: filter_combo_analysis.csv + pair_synergy_matrix.csv")
