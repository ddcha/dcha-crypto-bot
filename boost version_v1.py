# =========================================================
# smc_crypto_9coins  STAGE 4K: SENTIMENT MOMENTUM (4J + lookback sentiment booster layer)
#
# [기반]
#   Stage 4J 의 모든 규칙 유지 (4I 차단 + 4J outlier 차단 + 4J booster matrix)
#
...
TIER_RISK_MULT_COMMON = {
    "ALPHA_MAX":    1.5,
    "ALPHA_HIGH":   1.2,
    "ALPHA_MED":    0.8,   # side-aware override
...
SENTIMENT_BOOSTER_BY_SCENARIO = {
    "BOOST11": {"BOOST": 1.1, "CUT": 0.5},   # ⭐ 신규: 매우 가벼운 부스트 (테스트용)
...
# =========================================================
def simulate_scenario_v19b_rpboost(scenario, candidates_dict, risk_multiplier=1):
...
# =========================================================
# ★★★ 셀 3: Stage 4K — 3 시나리오 loop 백테스트 (BOOST15/25/35) ★★★
# =========================================================
import os
OUTDIR = "stage4k_boost11_outputs"   # ⭐ BOOST11 단독 결과 폴더
os.makedirs(OUTDIR, exist_ok=True)
print(f"\n[OUTDIR] {os.path.abspath(OUTDIR)}")
print(f"[CONFIG] OUT_BEHAVIOR = {OUT_BEHAVIOR!r}")

# Stage 4K 단일 시나리오 — BOOST11 (1.1× test)
SCENARIO_DESCRIPTIONS = {
    "BOOST11": "Stage 4K BOOST11 (sentiment booster=1.1×, cut=0.5×) — 매우 가벼움 / 테스트",
}

SCENARIOS_LIST = ["BOOST11"]   # ⭐ BOOST11 만 단독 실행
scenario_results = {}
...
for SCENARIO_CURRENT in SCENARIOS_LIST:
    ...
    STAGE4K_CASE = {
        "name":  f"stage4k_{SCENARIO_CURRENT.lower()}",
    ...
    prefix = f"stage4k_{SCENARIO_CURRENT.lower()}_out_{OUT_BEHAVIOR}"
    ...
    if len(tdf) > 0 and "sentiment_label" in tdf.columns:
        ...
        pd.DataFrame(sent_rows).round(4).to_csv(
            f"{OUTDIR}/sentiment_distribution_stage4k_{SCENARIO_CURRENT.lower()}.csv", index=False)
    ...
    if len(tdf) > 0 and "tier_4e" in tdf.columns:
        ...
        pd.DataFrame(tier_rows).round(4).to_csv(
            f"{OUTDIR}/tier_stage4k_{SCENARIO_CURRENT.lower()}_distribution.csv", index=False)
...
comp_df.to_csv(f"{OUTDIR}/stage4k_comparison_summary.csv", index=False)
...
print("=" * 90)
print(f"  산출물: {OUTDIR}/")
print(f"    stage4k_boost{{15,25,35}}_out_skip_*.csv  (trades/equity/monthly/summary/skipped)")
print(f"      └ trades.csv 신규 컬럼: sentiment_mult, sentiment_label, stage4j_mult, stage4j_label")
print(f"    sentiment_distribution_stage4k_*.csv  ← 4K 신규: sentiment cell 별 분포")
print(f"    tier_stage4k_*_distribution.csv")
print(f"    stage4k_comparison_summary.csv  ← 3 시나리오 비교")
