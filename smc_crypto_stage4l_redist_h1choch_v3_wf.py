...
# =========================================================
# ⭐⭐⭐ WALK-FORWARD 옵션 (v3_wf 신규) ⭐⭐⭐
# =========================================================
# WF_BAR_STEP: candidate evaluation 간격 (H4 봉 단위)
#   = 1   : 매 bar 마다 평가 (v3 conservative 와 동일, 가장 정밀, 가장 느림)
#   = 2   : 2 bar 마다 (8h 간격, 약 2배 빠름)
#   = 6   : 6 bar 마다 (1일 간격, 약 6배 빠름)
#   = 12  : 12 bar 마다 (2일 간격)
#   = 42  : 42 bar 마다 (1주 간격, 약 40배 빠름, 매우 sparse)
# 환경변수 WF_BAR_STEP 로 override 가능. 기본값 6 (1일 간격).
#
# WF_USE_DATA_CACHE: Binance 데이터 download 결과를 parquet 으로 캐시
#   True  : 첫 실행 후 data_cache/ 폴더에 저장 → 다음 실행은 즉시 로드
#   False : 매 실행마다 다운로드 (느림)
#
# WF_OUTDIR_SUFFIX: 결과 폴더 suffix (default = f"_step{WF_BAR_STEP}")
# =========================================================
WF_BAR_STEP        = int(_os_wf.environ.get("WF_BAR_STEP", "1"))
WF_USE_DATA_CACHE  = _os_wf.environ.get("WF_USE_DATA_CACHE", "1") != "0"
WF_CACHE_DIR       = _os_wf.environ.get("WF_CACHE_DIR", "data_cache")
WF_OUTDIR_SUFFIX   = _os_wf.environ.get("WF_OUTDIR_SUFFIX", f"_step{WF_BAR_STEP}")
WF_RUN_SCENARIOS   = _os_wf.environ.get("WF_SCENARIOS", "BOOST15,BOOST25,BOOST35")

print("=" * 75)
print(f"[WALK-FORWARD v3_wf] BAR_STEP        = {WF_BAR_STEP}  (= 매 {WF_BAR_STEP*4}h 마다 candidate 평가)")
print(f"                     USE_DATA_CACHE  = {WF_USE_DATA_CACHE}  (cache_dir = {WF_CACHE_DIR})")
print(f"                     OUTDIR_SUFFIX   = {WF_OUTDIR_SUFFIX}")
print(f"                     SCENARIOS       = {WF_RUN_SCENARIOS}")
if WF_BAR_STEP > 1:
    print(f"  ⚠ BAR_STEP={WF_BAR_STEP} 이므로 v3 (BAR_STEP=1) 대비 거래 수 / 결과가 달라질 수 있음")
print("=" * 75)
...
# =========================================================
def _wf_download_with_cache(symbol, interval):
    """
    [v3_wf 신규] Binance 다운로드 결과를 parquet 으로 캐시.
      - WF_USE_DATA_CACHE = True  : cache 우선 (없으면 download → cache 저장)
      - WF_USE_DATA_CACHE = False : 매번 다운로드
    캐시 파일 경로: {WF_CACHE_DIR}/{symbol}_{interval}.parquet
    """
    if not WF_USE_DATA_CACHE:
        return download_data(symbol=symbol, interval=interval)
    cache_dir = _Path(WF_CACHE_DIR)
    ...
    if cache_file.exists():
        try:
            ...
            print(f"  [cache read fail] {cache_file.name}: {e} — re-download")
    # cache miss → download → save
    df = download_data(symbol=symbol, interval=interval)
    try:
        df.to_parquet(cache_file, index=False)
        print(f"  [cache saved] {cache_file.name} ({len(df)} rows)")
    except Exception as e:
        print(f"  [cache save fail] {cache_file.name}: {e}")
    return df


def download_symbol_data(symbol):
    print(f"\n==================== {symbol} DATA DOWNLOAD ====================")
    df = _wf_download_with_cache(symbol=symbol, interval="4h")
    df_h1 = _wf_download_with_cache(symbol=symbol, interval="1h")
...
def generate_candidates_from_prepared(prepared):
    ...
    while i < len(df_struct) - 1:
        ...
        if daily_trade_count.get(trade_day, 0) >= DAY_TRADE_LIMIT:
            i += WF_BAR_STEP   # [v3_wf] BAR_STEP 적용 (was: i += 1)
            continue
...
# =========================================================
import os
# ⭐ [V3_WF — WALK-FORWARD] BAR_STEP 적용 결과를 별도 폴더에 저장
OUTDIR = f"stage4l_redist_outputs_v3_wf{WF_OUTDIR_SUFFIX}"
os.makedirs(OUTDIR, exist_ok=True)
print(f"\n[OUTDIR] {os.path.abspath(OUTDIR)}")
print(f"[CONFIG] OUT_BEHAVIOR = {OUT_BEHAVIOR!r}")
print(f"[V3 PATCHES — kept]")
print(f"  ① CUT_ShortStable_L → DISABLED (IS 0.47 vs OOS 12.4 flip)")
print(f"  ② strong_T2aligned_h1comp_short → 1.5× (OOS PF 18.78 검증됨)")
print(f"  ③ ALPHA_MED_RISK_SHORT: 0.3 (IS/OOS PF<1 일관)")
print(f"[V3_WF — WALK-FORWARD]")
print(f"  ④ BAR_STEP = {WF_BAR_STEP}  (candidate 평가 = 매 {WF_BAR_STEP*4}h)")
print(f"     1=v3 동일, 6=하루 1번, 42=주 1번. 클수록 sparse 하고 빠름.")
print(f"  ⑤ DATA_CACHE = {WF_USE_DATA_CACHE}  (parquet, dir={WF_CACHE_DIR})")
...
# [v3_wf] 환경변수 WF_SCENARIOS 로 일부만 실행 가능 (예: "BOOST25" → BOOST25만)
_wf_filter = [s.strip().upper() for s in WF_RUN_SCENARIOS.split(",")]
SCENARIOS_LIST = [s for s in SCENARIOS_LIST if s in _wf_filter]
if len(SCENARIOS_LIST) == 0:
    print(f"⚠ WF_SCENARIOS={WF_RUN_SCENARIOS!r} 매칭되는 시나리오 없음. 전체 실행.")
    SCENARIOS_LIST = ["BOOST15", "BOOST25", "BOOST35"]
print(f"[v3_wf] 실행할 시나리오: {SCENARIOS_LIST}")
scenario_results = {}
