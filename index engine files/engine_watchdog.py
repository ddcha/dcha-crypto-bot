...
STARTUP_STDOUT_FILE = Path("watchdog_main_idx_stdout.log")
STARTUP_STDERR_FILE = Path("watchdog_main_idx_stderr.log")
...
def start_engine() -> int | None:
    try:
        ...
        proc = subprocess.Popen(
            [sys.executable, str(base_dir / "main_idx.py")],
            cwd=str(base_dir),
    ...
    while True:
        try:
            ...
            if is_engine_stalled(system):
                ...
                # Give main_idx.py a moment to start and update runtime_state itself
                time.sleep(2)
