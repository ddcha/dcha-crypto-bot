...
from config_kis import LOOP_SLEEP_SECONDS
...
def sanitize_secret_value(value: str | None) -> str:
    """API key/secret 값 정제 (단순 trim 만, = 처리 X — KIS Base64 키는 == 패딩 있음)"""
    if value is None:
        return ""
...
def active_key_preview(live_settings: Dict[str, Any]) -> str:
    ...
    key = sanitize_secret_value(api_cfg.get("demo_app_key" if mode == "demo" else "live_app_key", ""))
    if not key:
        return "없음"
...
def get_active_api_credentials(live_settings: Dict[str, Any]) -> Tuple[str, str, str, str, str]:
    """
    Returns: (app_key, app_secret, account_no, product_code, mode)
    """
    mode = str(live_settings.get("mode", "demo")).strip().lower()
    api_cfg = live_settings.get("api", {})
    product_code = str(api_cfg.get("product_code", "08")).strip()
    if mode == "live":
        return (
            sanitize_secret_value(api_cfg.get("live_app_key", "")),
            sanitize_secret_value(api_cfg.get("live_app_secret", "")),
            str(api_cfg.get("live_account_no", "")).strip(),
            product_code,
            "live",
        )
    return (
        sanitize_secret_value(api_cfg.get("demo_app_key", "")),
        sanitize_secret_value(api_cfg.get("demo_app_secret", "")),
        str(api_cfg.get("demo_account_no", "")).strip(),
        product_code,
        "demo",
    )
...
def get_balance_metrics(live_settings: Dict[str, Any], runtime_state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    app_key, app_secret, account_no, product_code, mode = get_active_api_credentials(live_settings)
    ...
    try:
        ...
        # KIS 잔고 조회 (USD 환산)
        # 모의에서는 해외선물 잔고 API 미지원 → manual_balance_usd fallback 사용
        portfolio_cfg = live_settings.get("portfolio", {})
        manual_balance = float(portfolio_cfg.get("manual_balance_usd", 0.0) or 0.0)
        wallet_balance = ex.get_balance_usd(fallback_manual_balance=manual_balance)
        equity = wallet_balance  # KIS 는 wallet=equity (separate margin tracking 추가 가능)

        metrics["wallet_balance"] = wallet_balance
        metrics["equity"] = equity
        metrics["is_manual_balance"] = (mode == "demo" and manual_balance > 0 and wallet_balance == manual_balance)
...
def build_system_check(live_settings: Dict[str, Any]) -> Tuple[bool, List[Tuple[str, bool, str]]]:
    ...
    checks.append(("api_present", bool(app_key and app_secret and account_no), f"{mode.upper()} KIS API + account 입력"))
    try:
        import strategy_engine_idx  # noqa: F401
        checks.append(("strategy_engine_import", True, "strategy_engine_idx import ok"))
    except Exception as e:
        checks.append(("strategy_engine_import", False, str(e)))

    try:
        import main_idx  # noqa: F401
        checks.append(("main_import", True, "main_idx import ok"))
    except Exception as e:
    ...
    if app_key and app_secret and account_no:
        try:
            ...
            srv = ex.get_server_time()
            ok_server = srv.get("rt_cd") == "0"
            checks.append(("server_time", ok_server, "KIS token ok" if ok_server else str(srv)))
            
            # Wallet check: 모의는 미지원이라 manual_balance 로 fallback
            wallet = ex.get_wallet_balance()
            if wallet.get("_demo_unsupported"):
                # 모의: manual_balance_usd 가 입력되어 있는지 확인
                portfolio_cfg = live_settings.get("portfolio", {})
                manual_balance = float(portfolio_cfg.get("manual_balance_usd", 0.0) or 0.0)
                if manual_balance > 0:
                    checks.append(("wallet_balance", True, f"manual_balance_usd=${manual_balance:,.0f} (모의 - API 미지원, 수동 입력)"))
                else:
                    checks.append(("wallet_balance", False, "모의 미지원: portfolio.manual_balance_usd 입력 필요"))
            else:
                ok = wallet.get("rt_cd", "1") == "0"
                checks.append(("wallet_balance", ok, "wallet balance ok" if ok else str(wallet.get("msg1", ""))))
        except Exception as e:
...
def render_status_cards(runtime_state: Dict[str, Any], live_settings: Dict[str, Any]) -> None:
    ...
    c3.metric("Current Balance", f"${metrics['wallet_balance']:,.2f}" if metrics["wallet_balance"] is not None else "-")
    c4.metric("Current Equity", f"${metrics['equity']:,.2f}" if metrics["equity"] is not None else "-")
...
def render_api_controls(live_settings: Dict[str, Any], runtime_state: Dict[str, Any]) -> None:
    ...
    with demo_col:
        st.markdown("### Demo API (모의투자)")
        demo_key = st.text_input(
            "KIS Demo App Key",
            value=str(api_cfg.get("demo_app_key", "")),
            type="password",
            key="demo_app_key_input",
        )
        demo_secret = st.text_input(
            "KIS Demo App Secret",
            value=str(api_cfg.get("demo_app_secret", "")),
            type="password",
            key="demo_app_secret_input",
        )
        demo_account = st.text_input(
            "Demo 계좌번호 (8자리, 예: 00225351)",
            value=str(api_cfg.get("demo_account_no", "")),
            key="demo_account_no_input",
            help="해외선물옵션 모의계좌 8자리 (앞에서 8자리, 뒤 2자리는 product_code='08' 자동)",
        )
        st.caption(f"저장된 Demo Key: {masked_value(api_cfg.get('demo_app_key', ''))} | 계좌: {api_cfg.get('demo_account_no', '(없음)')}")

        if st.button("Demo API 적용", use_container_width=True):
            api_cfg["demo_app_key"] = sanitize_secret_value(demo_key)
            api_cfg["demo_app_secret"] = sanitize_secret_value(demo_secret)
            api_cfg["demo_account_no"] = sanitize_secret_value(demo_account)
    ...
    with live_col:
        st.markdown("### Live API (실전)")
        live_key = st.text_input(
            "KIS Live App Key",
            value=str(api_cfg.get("live_app_key", "")),
            type="password",
            key="live_app_key_input",
        )
        live_secret = st.text_input(
            "KIS Live App Secret",
            value=str(api_cfg.get("live_app_secret", "")),
            type="password",
            key="live_app_secret_input",
        )
        live_account = st.text_input(
            "Live 계좌번호 (8자리)",
            value=str(api_cfg.get("live_account_no", "")),
            key="live_account_no_input",
            help="해외선물옵션 실전계좌 8자리",
        )
        st.caption(f"저장된 Live Key: {masked_value(api_cfg.get('live_app_key', ''))} | 계좌: {api_cfg.get('live_account_no', '(없음)')}")

        if st.button("Live API 적용", use_container_width=True):
            api_cfg["live_app_key"] = sanitize_secret_value(live_key)
            api_cfg["live_app_secret"] = sanitize_secret_value(live_secret)
            api_cfg["live_account_no"] = sanitize_secret_value(live_account)
...
def render_main_controls(live_settings: Dict[str, Any], runtime_state: Dict[str, Any]) -> None:
    ...
    with top_left:
        ...
        # ⭐ Manual Balance USD - 모의 미지원 fallback
        manual_balance_usd = st.number_input(
            "Manual Balance USD (모의용 fallback)",
            min_value=0.0,
            value=float(portfolio.get("manual_balance_usd", 0.0)),
            step=1000.0,
            format="%.0f",
            help="KIS 해외선물옵션 잔고조회는 모의 미지원. 모의계좌 사용 시 여기에 예탁금 직접 입력. 실전계좌면 0 (자동 조회).",
        )
                ...
                with c3:
                    cfg["qty_step"] = float(
                        st.number_input(
                            f"{symbol} Qty Step",
                            min_value=1.0,
                            value=float(cfg.get("qty_step", 1.0)),
                            step=1.0,
                            format="%.0f",
                            key=f"{symbol}_step",
                            help="인덱스 선물은 1 단위 (정수 계약)",
                        )
                    )
                with c4:
                    cfg["min_order_qty"] = float(
                        st.number_input(
                            f"{symbol} Min Qty",
                            min_value=1.0,
                            value=float(cfg.get("min_order_qty", 1.0)),
                            step=1.0,
                            format="%.0f",
                ...
                with c5:
                    cfg["qty_decimals"] = int(
                        st.number_input(
                            ...
                            min_value=0,
                            max_value=10,
                            value=int(cfg.get("qty_decimals", 0)),
                            step=1,
                            key=f"{symbol}_dec",
                            help="인덱스는 정수(0) 권장",
                        )
    ...
    portfolio["manual_balance_usd"] = float(manual_balance_usd)  # ⭐ 모의 fallback
