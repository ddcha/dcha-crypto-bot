...
from strategy_engine_idx import generate_entry_signal_idx as generate_entry_signal, get_balance_usd_from_kis_response as get_latest_balance_usdt
# v2.2: zone cache 빌드용
from strategy_engine_idx import (
    prepare_htf_dataframe as _prepare_h4_for_zone,
    prepare_ltf_dataframe as _prepare_h1_for_zone,
...
from config_kis import EXCLUDE_RECENT_H1_FOR_TIER as _EXCLUDE_RECENT_H1_FOR_ZONE
...
def resolve_api_credentials(settings: dict) -> tuple[str, str, str, str, bool]:
    """
    KIS Open API credentials 추출.
    Returns: (app_key, app_secret, account_no, product_code, use_demo)
    """
    from settings_store import get_kis_credentials
    creds = get_kis_credentials(settings)
    
    return (
        sanitize_secret_value(creds["app_key"]),
        sanitize_secret_value(creds["app_secret"]),
        str(creds.get("account_no", "")).strip(),
        str(creds.get("product_code", "08")).strip(),
        bool(creds.get("use_demo", True)),
    )
...
def recover_tp1_order_from_exchange_open_orders(
    ...
    """
    try:
        resp = exchange.get_open_orders( symbol=symbol)
    except Exception:
        return None
...
def attach_tp1_limit_order(
    ...
    if existing_order_id:
        try:
            cancel_resp = exchange.cancel_order( symbol=symbol, order_id=existing_order_id)
            log_trade_event(
...
def cancel_tp1_limit_order(
    ...
    try:
        resp = exchange.cancel_order( symbol=symbol, order_id=order_id)
    except Exception:
        return None
...
def handle_tp1_exchange_fill(
    ...
    if order_id:
        try:
            status = exchange.get_order_fill_status( symbol=symbol, order_id=order_id)
        except Exception:
            status = None
...
def get_latest_closed_h4_bar(exchange: KisExchange, symbol: str) -> dict | None:
    try:
...
def register_runner_lifecycle_from_latest_h4_bar(
    ...
    bar = get_latest_closed_h4_bar(exchange=exchange,  symbol=symbol)
    if not bar:
        mp["runner_state_badge"] = get_runner_state_badge(mp)
        return
...
def rebuild_managed_positions_from_exchange(
    ...
    managed_positions: dict,
    open_positions_map: dict,
    exchange: KisExchange | None = None,
    category: str | None = None,  # KIS 호환 (사용 안함, BC 호환용)
) -> None:
    for symbol, mp in list(managed_positions.items()):
        ...
        if not actual or not actual.get("has_position"):
            # 포지션 청산 감지 → 쿨다운용 exit_time 기록
            from datetime import datetime, timezone as tz
            sym_state = state.get("symbol_states", {}).get(symbol, {})
            sym_state["last_exit_time"] = datetime.now(tz.utc).isoformat()
            sym_state["last_exit_side"] = mp.get("side", "")
            state.setdefault("symbol_states", {})[symbol] = sym_state
            managed_positions.pop(symbol, None)
            print(f"[COOLDOWN] {symbol} position closed, exit_time recorded")
            continue
...
def manage_open_positions(
    ...
    for symbol, mp in managed_positions.items():
        ...
        current_price = exchange.get_last_price( symbol=symbol)
        ...
        if mp.get("runner_active", False):
            ...
            if trailing_stop is not None:
                ...
                if current_stop is None:
                    should_update = True
                elif side == "Buy" and trailing_stop > float(current_stop):
                    should_update = True
                elif side == "Sell" and trailing_stop < float(current_stop):
                    should_update = True
...
def _build_zone_cache_for_symbol(exchange, symbol: str) -> list:
    ...
    """
    df_h4 = exchange.get_recent_klines_df(
        symbol=symbol, interval=H4_INTERVAL, limit=H4_LIMIT
    )
    df_h1_raw = exchange.get_recent_klines_df(
        symbol=symbol, interval=H1_INTERVAL, limit=H1_LIMIT
    )
...
def main() -> None:
    ...
    app_key, app_secret, account_no, product_code, use_demo = resolve_api_credentials(settings)

    if not app_key or not app_secret or not account_no:
        msg = "ERROR: live_settings_idx.json 에 KIS app_key/app_secret/account_no 가 없습니다.\n  → control_panel 에서 입력하세요."
        print(msg)
    ...
    while True:
        try:
            ...
            save_state(state)
            # ⭐ 모의 fallback 지원: portfolio.manual_balance_usd
            _portfolio_for_balance = settings.get("portfolio", {})
            _manual_balance = float(_portfolio_for_balance.get("manual_balance_usd", 0.0) or 0.0)
            balance = exchange.get_balance_usd(fallback_manual_balance=_manual_balance)
            wallet = {"balance_usd": balance, "manual_fallback_used": (_manual_balance > 0 and balance == _manual_balance)}
            pretty_print_json("Wallet Balance", wallet)

            system_state["last_action"] = "POSITIONS SYNC"
            save_state(state)
            all_positions = safe_api_call(lambda: exchange.get_positions(symbol=None), label="get_positions_initial")
            ...
            print(f"account_no={account_no}-{product_code}")
            ...
            save_state(state)
            refreshed_positions = safe_api_call(lambda: exchange.get_positions(symbol=None), label="get_positions_refresh")
            ...
            if USE_ZONE_PROXIMITY_TRIGGER:
                ...
                # ⭐ 인덱스: 시장 시간 체크 추가 (closed 자산은 skip)
                from market_hours import is_asset_market_open
                _hot_symbols = []
                _current_prices = {}
                _closed_symbols = []
                for _sym, _cfg in assets.items():
                    if not _cfg.get("enabled", False):
                        continue
                    # ⭐ 시장 closed 면 skip
                    if not is_asset_market_open(_sym):
                    ...
                    if _pos.get("has_position"):
                        continue
                    try:
                        _price = exchange.get_last_price(symbol=_sym)
                ...
                if _closed_symbols:
                    print(f"  [market closed] skipping: {_closed_symbols}")
            ...
            for symbol, asset_cfg in assets.items():
                try:
                    ...
                    # ⭐ v2.0_WICK: 1분봉 fetch (인덱스: KIS 1분봉 지원 약함 → 비활성)
                    df_1m = None
                    # 인덱스 봇은 1분봉 wick touch 사용 안 함 (USE_1M_WICK_TOUCH=False in config_kis)
                    ...
                    current_price = safe_api_call(
                        lambda: exchange.get_last_price(symbol=symbol),
                    ...
                    entry_signal = generate_entry_signal(
                        ...
                        df_ltf_raw=df_h1,
                        balance_usd=balance,
                        base_risk_pct=float(asset_cfg["risk_pct"]),
                        fee_rate=FEE_RATE,
                    ...
                    if fill_info is not None:
                        ...
                        position_resp = safe_api_call(
                            lambda: exchange.get_position_by_symbol(symbol=symbol),
