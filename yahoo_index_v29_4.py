...
INDEX_SYMBOLS = {
    # ============================================================
    # ★ v29.4: 9자산 (v29.3 에서 FDAX 추가 제거 - PF 0.80 음수) ★
    # ============================================================
    # 🇺🇸 US 4
...
# ============================================================
# ★★★ v29.4 패치: v29.3 결과 기반 추가 차단 + asset mult 조정 ★★★
# v29.3 → v29.4 변경:
#   [자산]
#   - FDAX 제거 (PF 0.80, -$2,169 음수 — 9자산으로 축소)
#   [RULE 추가 차단 (v29.3 결과 음수 패턴 3종)]
#   - A+_MAX 차단 (n=7, PF 0.58, -$691)
#   - S_SCORE_MAX_WICK 차단 (n=3, PF 0.23, -$921)
#   - A_RP3 차단 (n=3, PF 0.02, -$908)
#   [Asset mult 조정]
#   - ES: 1.3 → 1.5x (PF 32.94 폭발)
#   - NQ: 1.0 → 1.2x (PF 11.23 강함)
# ============================================================
USE_V29_1_PATCH = True       # v29.1 base 차단 (시나리오 B 12종)
USE_V29_2_PATCH = True       # v29.2: 자산별 mult 활성화
USE_V29_3_PATCH = True       # v29.3: 10자산 (+FTSE/HSI/TWII)
USE_V29_4_PATCH = True       # v29.4: 9자산 (FDAX 제거) + 추가 RULE 차단 + ES/NQ 부스트

# v29.2 RULE mult (v29.4: 변경 없음)
V29_1_RULE_MULT = {
    # S++ 핵심
    "S++_MSS_OVL":          5.0,   # n=10 Win 90% PF 579 (운용 무패)
...
# ★ v29.4: 자산별 mult (ES/NQ 부스트, FDAX 제거)
V29_2_ASSET_MULT = {
    # 강력 자산 (v29.3 검증)
    "ES":   1.5,   # ★ v29.4: 1.3 → 1.5 (PF 32.94 폭발)
    "NQ":   1.2,   # ★ v29.4: 1.0 → 1.2 (PF 11.23 강함)
    # 기본 1.0
    "YM":   1.0,
    "RTY":  1.0,
    "NKD":  1.0,
    # 신규 자산 (v29.3 baseline 검증)
    "FESX": 1.0,   # PF 1.81 (모니터링)
    "FTSE": 1.0,   # PF 3.82
    "HSI":  1.0,   # PF 3.06
    "TWII": 1.0,   # PF 2.18
    # FDAX 제거됨
...
OUTDIR = Path("yahoo_index_v29_4_outputs")
...
def simulate_portfolio_v25(candidates_packs, max_total_risk=0.1325, scenario_name="BOOST15"):
    ...
    return {
        ...
        "strategy_variant": "v29_4_FDAX_REMOVED_BLOCKS_ADDED",
        "scenario": scenario_name,
        "trades": trades_df,
        "skipped": skipped_df,
        "equity": equity_df,
...
if __name__ == "__main__":
    ...
    try:
        res["trades"].to_csv(outdir / "v29_4_trades.csv", index=False)
        res["equity"].to_csv(outdir / "v29_4_equity.csv", index=False)
        res["skipped"].to_csv(outdir / "v29_4_skipped.csv", index=False)
        res["phase_transitions"].to_csv(outdir / "v29_4_phase_transitions.csv", index=False)
        res["excess_log"].to_csv(outdir / "v29_4_excess_log.csv", index=False)
        res["tax_log"].to_csv(outdir / "v29_4_tax_log.csv", index=False)
        all_cand.to_csv(outdir / "v29_4_all_candidates.csv", index=False)
        print(f"\n💾 CSV 저장 완료: {outdir}/")
    ...
    if len(tdf) == 0:
        ...
        summary_df.to_csv(outdir / "v29_4_summary.csv", index=False)
        ...
        monthly_summary.to_csv(outdir / "v29_4_monthly_summary.csv", index=False)
        ...
        asset_summary.to_csv(outdir / "v29_4_asset_summary.csv", index=False)
        ...
        if "tier_label" in tdf.columns:
            ...
            tier_summary.to_csv(outdir / "v29_4_tier_summary.csv", index=False)
        ...
        if "v25idx_subtier" in tdf.columns:
            ...
            subtier_summary.to_csv(outdir / "v29_4_v25idx_subtier_summary.csv", index=False)
            ...
            rule_summary.to_csv(outdir / "v29_4_v25idx_rule_summary.csv", index=False)

        # 차트
        try:
            plot_equity_v25(res, save_path=str(outdir / "v29_4_equity_chart.png"))
        except Exception as e:
