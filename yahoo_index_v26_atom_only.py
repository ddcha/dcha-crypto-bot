# =========================================================
# YAHOO INDEX FUTURES BACKTEST — v2.6 ATOM-ONLY TIER REDESIGN
#
# [v26 변경점 (vs v25 stage 4K)]
#   ★ 기존 tier 시스템 완전 제거:
#     ALPHA_MAX/HIGH/MED, SWEEP_GEM/ROOM_FVG/ROOM_ONLY, COMPLETE_OUT, SKIP_MSS 다 폐기
#   ★ atom-only 새 tier 분류:
#       TIER_S       :  K ≥ 7  AND  CORE ≥ 1   (압도적 수익 그룹)
#       TIER_A       :  K=5~6  AND  CORE ≥ 2   (양호)
#       TIER_B       :  K=5~6  AND  CORE = 1   (보통)
#       TIER_C       :  K = 4    AND  CORE ≥ 1 (마진)
#       TIER_OVERLAP :  overlap 단독 (다른 atom 다 FAIL)
#       SKIP_no_atom_pass / SKIP_K_le_3 / SKIP_K{n}_no_core: 진입 차단
#       (CORE = a_score_ge13 / a_fvg / a_pre_total_ge4 / a_wick_le_q1 PASS 갯수)
#       (K = a_room 제외 11 atom PASS 갯수)
#   ★ 모든 가중 시스템 무력화: tier_mult / RP_mult / stage4j booster / sentiment 모두 1.0
#   ★ 자산별 risk 1% 고정 (Phase A/B 동일)
#   ★ TIER_CAP_4H 모두 999× (cap 제거)
#   ★ GEM_LONG_ONLY 비활성화, ROOM_ONLY blacklist 비움, RP_FILTER 비활성화
#
# [v26 유지 (v25 와 동일)]
#   ★ Yahoo 데이터 (8 indices: NQ/ES/YM/RTY/NKD/KS/SSE/SZSE)
#   ★ Dual-TF 2h/1h, MIN_SCORE 8.5
#   ★ 자본/입금: 시작 300만원 + 매월 200만원 × 5회
#   ★ 한국 해외선물 양도세 (22%, 250만원 공제)
#   ★ atomic 12 컨디션 계산 (compute_trade_tags)
#   ★ Entry: 보수 모드 (LONG=zone_high, SHORT=zone_low, refine OFF)
#   ★ Yahoo 60m 최대 = 729d (≈ 2년) — Yahoo Finance 제한
# =========================================================
...
# v26: 모든 자산 risk 1% 고정 (Phase A/B 동일)
PHASE_A_RISK = {
...
# v26 ATOM-ONLY: 4I 차단 비활성화 (atom-only tier 가 SKIP_ 으로 결정)
GEM_LONG_ONLY = False
ROOM_ONLY_SYMBOL_BLACKLIST = set()
...
# v26 ATOM-ONLY: RP filter / RP mult 무력화
RP_MULT_4H = {
    0: 1.0, 1: 1.0, 2: 1.0, 3: 1.0, 4: 1.0, 5: 1.0,
    6: 1.0, 7: 1.0, 8: 1.0, 9: 1.0, 10: 1.0,
}
RP_FILTER_ENABLE = False
...
OUTDIR = Path("yahoo_index_v26_atom_only_outputs")
...
print("=" * 80)
print("YAHOO INDEX v26 — ATOM-ONLY TIER REDESIGN (8 INDICES)")
print("=" * 80)
print(f"Total assets: {len(INDEX_SYMBOLS)}")
print()
print("📊 Indices (Dual-TF 2h/1h, risk 1% 고정):")
for k in INDEX_SYMBOLS:
    print(f"   {k:4s}: risk={PHASE_A_RISK[k]*100:.2f}%  thr=${THRESHOLD_USD[k]/1e6:.1f}M")
print()
print(f"MIN_SCORE: {MIN_SCORE} | Volume Filter ON ({VOLUME_SPIKE_MULT})")
print(f"Tier System: ATOM-ONLY (TIER_S/A/B/C/OVERLAP)")
print(f"  TIER_S       : K ≥ 7  AND  CORE ≥ 1")
print(f"  TIER_A       : K=5~6  AND  CORE ≥ 2")
print(f"  TIER_B       : K=5~6  AND  CORE = 1")
print(f"  TIER_C       : K = 4    AND  CORE ≥ 1")
print(f"  TIER_OVERLAP : overlap 단독")
print(f"  SKIP         : K ≤ 3  OR  (K ≥ 4 AND CORE = 0)")
print(f"  CORE = score_ge13 / fvg / pre_total_ge4 / wick_le_q1 (PASS 갯수)")
print(f"  K = a_room 제외 11 atom PASS 갯수")
print(f"Wick Q1 threshold: {WICK_RATIO_5_Q1_THRESHOLD}")
print(f"GEM_LONG_ONLY: {GEM_LONG_ONLY} (비활성화)")
print(f"RP_FILTER_ENABLE: {RP_FILTER_ENABLE} (비활성화)")
...
total_risk = sum(PHASE_A_RISK.values())
print(f"Total Risk Cap : {total_risk*100:.2f}% (모든 자산 1% 고정)")
print(f"Tax           : 연 {TAX_RATE*100:.0f}% (공제 {TAX_DEDUCTION_KRW/1e4:.0f}만원)")
print(f"Initial       : {INITIAL_CAPITAL_KRW:,} KRW + {MONTHLY_DEPOSIT_KRW:,} x {NUM_MONTHLY_DEPOSITS}회 (월말 입금)")
print(f"Period        : {PERIOD_1H} (Yahoo 60m 최대)")
print("=" * 80)
...
# ─── v26 ATOM-ONLY: Stage 4J 차단/booster 무력화 ───
def is_stage4j_blocked(tier, side, symbol, run_potential):
    """v26: 차단 비활성화 (atom-only tier 가 SKIP_ 으로 결정)."""
    return None

def get_stage4j_extra_mult(tier, side, symbol, run_potential):
    """v26: booster 비활성화 (모두 1.0)."""
    return (1.0, "v26_no_booster")
...
def get_stage4k_sentiment_mult(target_time, side, executed_trades_so_far, scenario):
    """v26 ATOM-ONLY: sentiment booster/cut 무력화 (모두 1.0)."""
    return (1.0, "v26_no_sentiment")

# ─── Side-aware ALPHA_MED ───
ALPHA_MED_RISK_LONG  = 0.8
ALPHA_MED_RISK_SHORT = 1.2

# v26 ATOM-ONLY: RP mult 무력화 (위에서 RP_FILTER_ENABLE = False)
RP_MULT_4H = {
    0: 1.0, 1: 1.0, 2: 1.0, 3: 1.0, 4: 1.0, 5: 1.0,
    6: 1.0, 7: 1.0, 8: 1.0, 9: 1.0, 10: 1.0,
}
...
def classify_tier_stage4h(atoms_dict, side="long"):
    """
    v26 ATOM-ONLY tier 분류 (기존 tier 시스템 완전 폐기).
    
    [진입 게이트]
        atom 1+ PASS (모든 atom OR)
    
    [차단 — 손실 분석 기반]
        K ≤ 3                → SKIP (PF 0.55, 실제 손실 그룹)
        CORE = 0  (K ≥ 4)    → SKIP (CORE 부재 setup)
        * 단, overlap_solo 는 예외
    
    [Tier 분류]
        TIER_S       :  K ≥ 7  AND  CORE ≥ 1   (PF 4.0+)
        TIER_A       :  K=5~6  AND  CORE ≥ 2
        TIER_B       :  K=5~6  AND  CORE = 1
        TIER_C       :  K = 4    AND  CORE ≥ 1
        TIER_OVERLAP :  overlap 단독 (PF 4.19)
    """
    _ATOMS_SYN_V26 = [
    ...
    _CORE4_V26 = ["a_score_ge13", "a_fvg", "a_pre_total_ge4", "a_wick_le_q1"]
    _NON_OVERLAP_V26 = [a for a in _ATOMS_SYN_V26 if a != "a_overlap"]
    
    def _g(k): return bool(atoms_dict.get(k, False))
    
    K = sum(1 for a in _ATOMS_SYN_V26 if _g(a))
    core_count = sum(1 for a in _CORE4_V26 if _g(a))
    overlap = _g("a_overlap")
    others_excl_overlap = any(_g(a) for a in _NON_OVERLAP_V26)
    overlap_solo = overlap and not others_excl_overlap
    
    # 진입 게이트
    if K == 0:
        return "SKIP_no_atom_pass"
    ...
    if K >= 5:
        if core_count >= 2:
            ...
            return "TIER_B"
    return "TIER_C"  # K == 4 AND CORE ≥ 1


def get_tier_risk_mult_4h(tier_label, side="long", symbol=None):
    """
    v26 ATOM-ONLY: 사용자 risk 1% 고정.
    SKIP_ 으로 시작하는 tier 만 0.0, 그 외 (TIER_S/A/B/C/OVERLAP) 는 1.0.
    """
    if tier_label is None:
        return 0.0
    try:
        if pd.isna(tier_label):
            return 0.0
    except Exception:
        pass
    if str(tier_label).startswith("SKIP_"):
        return 0.0
    return 1.0


def get_tier_cap_4h(tier_label):
    """v26 ATOM-ONLY: cap 제거 (모두 999×)."""
    return 999.0
...
def simulate_portfolio_v25(candidates_packs, max_total_risk=0.1325, scenario_name="BOOST15"):
    ...
    for current_time in event_times:
        ...
        if current_time in entry_map:
            ...
            for idx, row in entry_events:
                ...
                if tier_mult <= 0.0:
                    # v26 ATOM-ONLY: SKIP_ tier 들의 label 을 reason 으로 그대로 사용
                    sk_reason = f"atom_skip_{tier_label}"
                    skipped_entries.append({
                        "entry_time": row["entry_time"], "symbol_key": symbol_key,
    ...
    return {
        "mode": "portfolio",
        "symbol_key": "PORTFOLIO_INDEX_V26_ATOM_ONLY",
        "yahoo_symbol": "+".join(INDEX_SYMBOLS.values()),
        "strategy_variant": "v26_atom_only",
        "scenario": scenario_name,
        "trades": trades_df,
        "skipped": skipped_df,
        "equity": equity_df,
...
if __name__ == "__main__":
    ...
    print("\n" + "=" * 80)
    print("  🎯 v26 ATOM-ONLY TIER REDESIGN — 지수 엔진")
    print("=" * 80)
    print(f"  Atom-only Tier 매트릭스:")
    print(f"    TIER_S       : risk × 1.0   (K≥7, CORE≥1, 핵심 수익 그룹)")
    print(f"    TIER_A       : risk × 1.0   (K=5~6, CORE≥2)")
    print(f"    TIER_B       : risk × 1.0   (K=5~6, CORE=1)")
    print(f"    TIER_C       : risk × 1.0   (K=4, CORE≥1)")
    print(f"    TIER_OVERLAP : risk × 1.0   (overlap 단독)")
    print(f"    SKIP_*       : 차단 (K≤3 OR CORE=0)")
    print(f"  무력화 항목:")
    print(f"    Stage 4I (GEM/ROOM_ONLY 차단)  → 비활성화")
    print(f"    Stage 4J (booster/차단)        → 비활성화")
    print(f"    Stage 4K Sentiment             → 비활성화")
    print(f"    RP_FILTER                      → 비활성화")
    print(f"    Tier risk mult                 → 1.0 고정 (자산별 risk 1%)")
    ...
    try:
        res["trades"].to_csv(outdir / "v26_atom_only_trades.csv", index=False)
        res["equity"].to_csv(outdir / "v26_atom_only_equity.csv", index=False)
        res["skipped"].to_csv(outdir / "v26_atom_only_skipped.csv", index=False)
        res["phase_transitions"].to_csv(outdir / "v26_atom_only_phase_transitions.csv", index=False)
        res["excess_log"].to_csv(outdir / "v26_atom_only_excess_log.csv", index=False)
        res["tax_log"].to_csv(outdir / "v26_atom_only_tax_log.csv", index=False)
        all_cand.to_csv(outdir / "v26_atom_only_all_candidates.csv", index=False)
        print(f"\n💾 CSV 저장 완료: {outdir}/")
    ...
    if len(tdf) == 0:
        ...
        summary_row = {
            "Name": "PORTFOLIO_INDEX_V26_ATOM_ONLY",
            "Scenario": SCENARIO,
            "Trades": len(tdf),
        ...
        summary_df.to_csv(outdir / "v26_atom_only_summary.csv", index=False)
        ...
        monthly_summary.to_csv(outdir / "v26_atom_only_monthly_summary.csv", index=False)
        ...
        asset_summary.to_csv(outdir / "v26_atom_only_asset_summary.csv", index=False)

        # TIER
        print("\n" + "=" * 110)
        print("  [4] v26 ATOM-ONLY TIER 별 성과")
        print("=" * 110)
        if "tier_label" in tdf.columns:
            ...
            tier_summary.to_csv(outdir / "v26_atom_only_tier_summary.csv", index=False)

        # 차트
        try:
            plot_equity_v25(res, save_path=str(outdir / "v26_atom_only_equity_chart.png"))
        except Exception as e:
            print(f"[WARN] 차트 생성 실패: {e}")

        # 최종 요약
        print("\n" + "=" * 110)
        print("  🏆 v26 ATOM-ONLY 결과")
        print("=" * 110)
