...
FEE_RATE = 0.0005
SL_BUFFER_MULT = 0.08
MIN_SCORE = float(_os_cfg.environ.get("STAGE4D_MINSCORE", "7.5"))   # ⭐ 진입 문턱 (env 스윕)
    ...
    while i < len(df_struct) - 1:
        ...
        for s in active_structures:
            ...
            if eff_score < MIN_SCORE:   # ⭐ 7.5 게이트 — flip/비flip 동일(존 품질 유지: 강한 존만 flip 매매)
                continue
            _zr = s.get("reasons", "")   # ⭐ FVG-only 게이트: OB 포함 zone(OB-only·OB+FVG) 배제
            if ("valid_bull_ob" in _zr) or ("valid_bear_ob" in _zr):
                continue
            if _flipped:
