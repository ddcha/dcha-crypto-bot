...
def run_one(label, rules_json, np, pd, SYMBOLS, REST6, prepared, bts, bdist, ncore):
    ...
    tr["matched_rule"] = [x[0] for x in lab]; tr["n_rules_matched"] = [x[1] for x in lab]
    ...
    import os as _os
    _os.makedirs("walkforward_result", exist_ok=True)
    rl.to_csv(f"walkforward_result/trades_{label.split('(')[0]}.csv", index=False)
    ...
    return {"label": label, "zero": zero, "la_cap": f"{mism}/{ch}", "btc_future": fut, "full": full, "s22": s22, "cap": cap, "rl": rl}
...
def main():
    ...
    # ── walk-forward: 학습12개월 음수규칙제거 → 검증3개월 롤링(3개월씩) ──
    def walk_forward(rl):
        d = rl[["exit_time", "net_pnl", "r_multiple", "matched_rule"]].copy()
        d["exit_time"] = pd.to_datetime(d["exit_time"], utc=True)
        start = pd.Timestamp("2022-01-01", tz="UTC"); end = d["exit_time"].max()
        wins = []; v_start = start + pd.DateOffset(months=12)
        while v_start < end:
            v_end = v_start + pd.DateOffset(months=3); t_start = v_start - pd.DateOffset(months=12)
            train = d[(d.exit_time >= t_start) & (d.exit_time < v_start)]
            valid = d[(d.exit_time >= v_start) & (d.exit_time < v_end)]
            if len(valid) >= 5:
                negs = set(ru for ru, g in train.groupby("matched_rule") if g["r_multiple"].sum() < 0)
                keep = valid[~valid["matched_rule"].isin(negs)]
                wins.append({"valid": v_start.strftime("%Y-%m"), "n": len(valid),
                             "pf_fixed": round(_pf(valid["net_pnl"].values, np), 3),
                             "n_keep": len(keep), "pf_pruned": round(_pf(keep["net_pnl"].values, np), 3) if len(keep) else float("nan")})
            v_start = v_start + pd.DateOffset(months=3)
        return pd.DataFrame(wins)

    print("\n" + "=" * 80); print("=== 2단계 Walk-forward (검증3개월 롤링) ===")
    wfs = {}
    for o in outs:
        wf = walk_forward(o["rl"]); wfs[o["label"]] = wf
        wf.to_csv(f"walkforward_result/wf_{o['label'].split('(')[0]}.csv", index=False)
        for col in ["pf_fixed", "pf_pruned"]:
            v = wf[col].replace([np.inf, -np.inf], np.nan).dropna()
            print(f"  [{o['label']} {col}] 검증창 {len(v)}개 | 평균PF {v.mean():.3f} | 표준편차 {v.std():.3f} | "
                  f"최악 {v.min():.3f} | PF≥1 비율 {(v >= 1).mean()*100:.0f}%")
        print(f"    창별 pf_fixed: {dict(zip(wf['valid'], wf['pf_fixed']))}")

    print("\n=== a3 vs v4 종합비교 ===")
    a, v = outs[0], outs[1]; wa, wv = wfs[a["label"]], wfs[v["label"]]
    def _wfstat(wf, col):
        s = wf[col].replace([np.inf, -np.inf], np.nan).dropna(); return s.mean(), s.std(), (s >= 1).mean() * 100
    print(f"{'지표':<18}{'a3(63)':>14}{'v4(42)':>14}")
    print(f"{'전체PF':<18}{a['full']['PF']:>14}{v['full']['PF']:>14}")
    print(f"{'2022 PF(순수OOS)':<18}{a['s22']['PF']:>14}{v['s22']['PF']:>14}")
    print(f"{'최종자본':<18}{a['cap']['final']:>14,.0f}{v['cap']['final']:>14,.0f}")
    print(f"{'전체MDD%':<18}{a['cap']['mdd']:>14.2f}{v['cap']['mdd']:>14.2f}")
    for col in ["pf_fixed", "pf_pruned"]:
        ma, sa, ra = _wfstat(wa, col); mv, sv, rv = _wfstat(wv, col)
        print(f"{'WF '+col+' 평균':<18}{ma:>14.3f}{mv:>14.3f}")
        print(f"{'WF '+col+' PF≥1%':<18}{ra:>14.0f}{rv:>14.0f}")
