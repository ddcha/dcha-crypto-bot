# =========================================================
# smc_crypto_v19b_CONSERVATIVE_ENTRY  (zone 최상/최하 진입)
#
# [변경점 vs v1.9b 원본]
#   ★ LONG_BASE_ENTRY_FRAC  = 0.10 → 1.0   (LONG: zone_high 진입)
#   ★ SHORT_BASE_ENTRY_FRAC = 0.90 → 0.0   (SHORT: zone_low 진입)
#   ★ H1_REFINE_MAX_IMPROVE_FRAC = 0.25 → 0.0  (refine 무력화)
#   ★ refine_entry_with_h1_fvg → DISABLED 버전으로 강제 swap
#
# [예상]
#   baseline: Return 21,916%, PF 9.44, WinR 79.58%, MDD -3.96%
#   변경 후:   Return 폭락, PF ~1.0, WinR ~30~40%, MDD -30%~-60%
#
# [의도]
#   "zone 빨리 닿자마자 진입 = 더 보수" 라는 직관 검증.
#   실제로는 R/R 단위가 3배 이상 커져 SL 적중률 폭증.
# =========================================================
...
# ★★★ 보수 entry 모드 ★★★
LONG_BASE_ENTRY_FRAC = 1.0       # zone_high (최상단)
SHORT_BASE_ENTRY_FRAC = 0.0      # zone_low (최하단)
...
H1_REFINE_MAX_IMPROVE_FRAC = 0.0  # ★ refine 무력화
...
SCENARIO_MULTI = {
    "name": "v1.9b CONSERVATIVE ENTRY 9coins (zone_high/low, no refine)",
    "assets": {
...
# ★★★ refine_entry_with_h1_fvg DISABLED (보수 entry 모드) ★★★
def refine_entry_with_h1_fvg(df_h1_local, h4_timestamp, side, zone_low, zone_high, base_entry,
    ...
    """보수 entry 모드 — refine 무력화. base_entry 그대로 반환."""
    return base_entry, False, np.nan, "refine_disabled"
        ...
        if current_time in entry_map:
            for idx, row in entry_map[current_time]:
                ...
                open_positions[idx] = {
                    ...
                    "strategy_variant": "v19b_CONSERVATIVE_ENTRY",
                    "symbol": symbol,
...
def print_scenario_summary(result_dict):
    ...
    print(f"  ★ ENTRY MODE: LONG=zone_high, SHORT=zone_low, refine=DISABLED ★")
...
if __name__ == "__main__":
    ...
    print("\n" + "=" * 80)
    print("v1.9b CONSERVATIVE ENTRY MODE — 9 COINS, PARALLEL")
    print("=" * 80)
    print(f"  LONG_BASE_ENTRY_FRAC  = {LONG_BASE_ENTRY_FRAC}  (zone_high 진입)")
    print(f"  SHORT_BASE_ENTRY_FRAC = {SHORT_BASE_ENTRY_FRAC}  (zone_low 진입)")
    print(f"  H1_REFINE             = DISABLED")
    ...
    # ===================== 셀 3: 백테스트 실행 =====================
    _t = _time.time()
    OUTDIR = "smc_crypto_v19b_conservative_entry_outputs"
    ...
    res["trades"].to_csv(f"{OUTDIR}/conservative_trades.csv", index=False)
    res["equity"].to_csv(f"{OUTDIR}/conservative_equity.csv", index=False)
    res["skipped"].to_csv(f"{OUTDIR}/conservative_skipped.csv", index=False)
    res["phase_transitions"].to_csv(f"{OUTDIR}/conservative_phase_transitions.csv", index=False)
    res["excess_log"].to_csv(f"{OUTDIR}/conservative_excess_log.csv", index=False)
    monthly_report.to_csv(f"{OUTDIR}/conservative_monthly_pnl.csv", index=False)
    ...
    print(f"\n💾 CSV 저장: {OUTDIR}/")
    print(f"\n🎯 v1.9b CONSERVATIVE ENTRY 9코인 백테스트 완료")
