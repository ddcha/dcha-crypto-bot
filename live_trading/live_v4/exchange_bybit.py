...
from config_live import READ_API_MAX_RETRIES, READ_API_RETRY_SLEEP_SECONDS
...
class BybitExchange:
    ...
    def get_instrument_info(self, category: str, symbol: str) -> dict:
        resp = self._call_with_retry(self.session.get_instruments_info, category=category, symbol=symbol)
        items = resp.get("result", {}).get("list", [])
        if not items:
            return {"qty_step": 0.001, "min_qty": 0.001, "qty_decimals": 3}
        lot = items[0].get("lotSizeFilter", {})
        step = float(lot.get("qtyStep", "0.001") or 0.001)
        minq = float(lot.get("minOrderQty", step) or step)
        dec = max(0, len(str(step).split(".")[1]) if "." in str(step) else 0)
        return {"qty_step": step, "min_qty": minq, "qty_decimals": dec}
