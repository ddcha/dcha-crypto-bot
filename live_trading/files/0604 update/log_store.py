...
def _write_csv_rows(path: Path, rows: List[Dict[str, Any]]) -> None:
    ...
    # ★ [PATCHED 2026-06-04] 원자적 쓰기.
    # 기존엔 path.open("w") 로 원본을 바로 덮어써서, recovery_failed 초당 스팸으로
    # 파일 전체가 초당 재기록되는 와중에 watchdog SIGKILL 이 쓰기 도중 떨어지면
    # 파일이 잘려나가 order_filled 진입 기록 전체가 유실됨(이번 orphan 사태의 근본 원인).
    # 임시파일에 완전히 쓰고 fsync 후 os.replace 로 원자 교체 → 중간에 죽어도 원본 보존.
    dir_name = str(path.parent) if str(path.parent) else "."
    fd, tmp_path = tempfile.mkstemp(prefix=path.name + ".", suffix=".tmp", dir=dir_name)
    try:
        with os.fdopen(fd, "w", encoding="utf-8-sig", newline="") as f:
            ...
            f.flush()
            os.fsync(f.fileno())
        os.replace(tmp_path, path)  # POSIX 에서 원자적 교체
    except Exception:
        try:
