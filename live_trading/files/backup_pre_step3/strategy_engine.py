...
try:
    ...
    RP_MULT_4H = {0: 1.5, 1: 1.0, 2: 1.0, 3: 1.0, 4: 1.0, 5: 1.0}
...
def prepare_h4_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df = apply_h4_market_state(df, transition_bars=360)
    return df
...
def generate_entry_signal(
    ...
    for z in evaluated_zones:
        ...
        if s["type"] == "short":
            ...
            # 최종 risk_pct (tier_mult 가 이미 4J extra mult 포함, 여기서 sentiment 만 추가)
            risk_pct_adjusted = float(risk_pct) * float(tier_mult) * float(sentiment_mult)
