...
# =========================================================
# Stage 4L BOOST25 시스템 (Edge Ratio 기반 tier mult 재배정)
#
# 백테스트 결과 (9코인, 3년):
#   4L BOOST25: trades 1069, win 65.01%, PF 2.671, sum_pnl $1,764K (4K 대비 +12%)
#   MDD% (CSV) -35.10% / MDD% (Total) ~-12% 추정
#
# [Stage 4L 변경 매트릭스 — Edge Ratio 분석 결과]
#   ★ SWEEP_GEM      : 1.0 → 4.0  (Edge 4.76 최고, worst $467 — 매우 안전)
#   ★ ALPHA_MAX      : 2.0 → 2.5  (Edge 3.82, DD/Net 9.3% — 안전)
#   ★ ALPHA_MED LONG : 0.8 → 0.6  (Edge 1.67, streak 8번 — 약점)
#   ★ ALPHA_MED SHORT: 1.2 → 1.0
#   ALPHA_HIGH       : 1.5 (유지) — 시스템 핵심
#   SWEEP_ROOM_FVG/ONLY: 1.0 (유지) — ONLY 는 DD/Net 33% 위험
#
# 진입 결정 흐름:
...
# Stage 4L: Edge Ratio 기반 tier mult 재배정
TIER_RISK_MULT_COMMON = {
    "ALPHA_MAX":    2.5,   # ⭐ 4K 2.0 → 2.5 (Edge 3.82, DD/Net 9.3% — 안전)
...
#   원래: LONG 0.8 / SHORT 1.2  → 변경: LONG 0.6 / SHORT 1.0  (평균 0.8, 비율 유지)
...
# ─────────────────────────────────────────────
SCENARIO = "BOOST25"   # ⭐ v2.2 → BOOST25 (booster 1.5x → 2.5x) + Stage 4L

# SWEEP 계열 base risk mult (4J/4K: 모두 1.0 평평)
# Stage 4L: SWEEP_GEM ★ 1.0 → 4.0 (Edge 4.76 최고, worst loss $467 가장 작음)
#   SWEEP_ROOM_FVG 유지 / SWEEP_ROOM_ONLY 유지 (DD/Net 33% 위험)
SWEEP_RISK_BY_SCENARIO = {
    "BOOST15": {
        "SWEEP_GEM":       4.0,   # ⭐ 4K 1.0 → 4.0
        "SWEEP_ROOM_FVG":  1.0,   # 유지
        "SWEEP_ROOM_ONLY": 1.0,   # 유지
    },
    "BOOST25": {
        "SWEEP_GEM":       4.0,   # ⭐ 4K 1.0 → 4.0
        "SWEEP_ROOM_FVG":  1.0,   # 유지
