...
def _fetch_zone_data_for_symbol(
    api_key: str,
    api_secret: str,
    mode: str,
    symbol: str,
    use_1m_wick: bool,
    wick_lookback: int,
    exclude_recent_h1: int,
    relax_min_score: float,
):
    """
    한 심볼에 대해 무거운 zone 평가 데이터 fetch + 계산.
    캐싱 함수에서 호출됨. 캐시 가능한 형태로 반환.
    """
    from strategy_engine import (
        prepare_h4_dataframe,
        prepare_h1_dataframe,
        build_structures,
    ...
    df_h4 = ex.get_recent_klines_df(category="linear", symbol=symbol, interval="240", limit=2000)
    df_h1_raw = ex.get_recent_klines_df(category="linear", symbol=symbol, interval="60", limit=1200)
    df_h1 = prepare_h1_dataframe(df_h1_raw.copy())
    df_struct, structures, _ = build_structures(prepare_h4_dataframe(df_h4.copy()))
    i = len(df_struct) - 2
    current_close = float(df_struct.iloc[i]["close"])
    h4_trend = str(df_struct.iloc[i].get("trend", "?"))

    # 실시간 현재가 조회
    try:
        live_price = ex.get_last_price(category="linear", symbol=symbol)
        if live_price:
            current_close = float(live_price)
    except Exception:
        pass
    ...
    if use_1m_wick:
        try:
            df_1m_recent = ex.get_recent_klines_df(
                category="linear", symbol=symbol, interval="1",
                limit=max(int(wick_lookback) + 5, 10),
            )
            df_1m_records = df_1m_recent.tail(int(wick_lookback) + 5).to_dict(orient="records")
        except Exception:
            df_1m_records = []
    ...
    try:
        ...
        pos_list = pos_resp.get("result", {}).get("list", []) if isinstance(pos_resp, dict) else []
        for p in pos_list:
            ...
            if sz > 0:
                ...
                position_info = f" | 📍 {side} @ {avg} (PnL {pnl})"
                break
    except Exception:
        pass
    ...
    return {
        ...
        "df_1m_records": df_1m_records,
        "position_info": position_info,
    }


@st.cache_data(ttl=300, show_spinner=False)
def _fetch_all_zones_cached(
    api_key: str,
    api_secret: str,
    mode: str,
    active_symbols_tuple: tuple,
    use_1m_wick: bool,
    wick_lookback: int,
    exclude_recent_h1: int,
    relax_min_score: float,
    ...
    """
    모든 active_symbols 의 zone 데이터를 한 번에 fetch + 캐시.
    TTL 300초 (5분). 같은 인자로 호출 시 즉시 반환.
    
    캐시 키: (api_key, mode, active_symbols, use_1m_wick, wick_lookback, ...)
    심볼 목록이나 옵션이 바뀌면 새로 계산.
    """
    results = {}
    for symbol in active_symbols_tuple:
        try:
            results[symbol] = _fetch_zone_data_for_symbol(
                api_key=api_key,
                api_secret=api_secret,
                mode=mode,
                symbol=symbol,
                use_1m_wick=use_1m_wick,
...
def render_active_zones(live_settings: Dict[str, Any]) -> None:
    ...
    # ==========================================================
    # v2.1: streamlit cache 로 무거운 zone 계산 캐싱 (TTL 300초)
    # 첫 클릭만 30~60초 무거움, 5분 내 재클릭 시 즉시 반환
    # ==========================================================
    try:
        from strategy_engine import RELAX_MIN_SCORE
    except Exception:
        RELAX_MIN_SCORE = 7.5
    ...
    try:
        ...
        if not active_symbols:
            st.info("활성화된 심볼이 없습니다. assets 설정 확인.")
            return

        # 캐싱된 fetch (TTL 300초)
        with st.spinner(f"Zone 데이터 조회 중 ({len(active_symbols)}개 심볼)... 첫 호출 시 30~60초 소요. 5분 내 재클릭 시 즉시 응답."):
            zone_data_all = _fetch_all_zones_cached(
                api_key=api_key,
                api_secret=api_secret,
                mode=mode,
                active_symbols_tuple=tuple(active_symbols),
                use_1m_wick=bool(use_1m_wick_panel),
                wick_lookback=int(WICK_TOUCH_LOOKBACK_BARS),
                exclude_recent_h1=int(EXCLUDE_RECENT_H1_FOR_TIER),
                relax_min_score=float(RELAX_MIN_SCORE),
            )

        # 캐시 새로고침 버튼
        col_info, col_refresh = st.columns([4, 1])
        with col_info:
            st.caption(f"📦 Cache TTL 5분 | symbols={len(zone_data_all)}")
        with col_refresh:
            if st.button("🔄 캐시 새로고침", key="btn_refresh_zone_cache", help="강제로 다시 조회 (다시 무거움)"):
                _fetch_all_zones_cached.clear()
                st.rerun()

        for symbol in active_symbols:
            try:
                if symbol not in zone_data_all:
                    st.warning(f"{symbol}: 데이터 없음")
                    continue

                cached = zone_data_all[symbol]

                if "error" in cached:
                    st.error(f"{symbol}: {cached['error']}")
                    continue

                evaluated_zones = cached["evaluated_zones"]
                active_structures = cached["active_structures"]
                current_close = float(cached["current_close"])
                h4_trend = cached["h4_trend"]
                position_info = cached.get("position_info", "")

                # 1m wick touch DataFrame 복원
                df_1m_recent = None
                if use_1m_wick_panel and cached.get("df_1m_records"):
                    try:
                        import pandas as _pd
                        df_1m_recent = _pd.DataFrame(cached["df_1m_records"])
