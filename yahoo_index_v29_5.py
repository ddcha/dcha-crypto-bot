...
INDEX_SYMBOLS = {
    # ============================================================
    # ★ v29.5: 9자산 (FESX 제거, TPX 추가) ★
    # v29.4 → v29.5: FESX (PF 0.46 음수) 제거 + TPX 신규 추가
    # ============================================================
    # 🇺🇸 US 4
    ...
    "TPX":  "TPX_TV",       # ⭐ v29.5 신규: csv (TV: OSE:TOPIX1!)
    # 🇬🇧 GB 1 (v29.5: FESX 제거)
...
# ============================================================
# ★★★ v29.5 패치: FESX 제거 + TPX 추가 + asset mult 유지 ★★★
# v29.4 → v29.5 변경:
#   [자산]
#   - FESX 제거 (PF 0.46, -$6,149 음수)
#   - TPX 추가 (TOPIX, JP 시총 가중 — NKD 와 다른 신호)
#   [RULE / mult: v29.4 그대로 유지]
# ============================================================
USE_V29_1_PATCH = True       # v29.1 base 차단 (시나리오 B 12종)
USE_V29_2_PATCH = True       # v29.2: 자산별 mult 활성화
USE_V29_3_PATCH = True       # v29.3: 신규 자산 추가
USE_V29_4_PATCH = True       # v29.4: 음수 RULE 3종 차단 (A+_MAX, S_SCORE_MAX_WICK, A_RP3) + ES/NQ 부스트
USE_V29_5_PATCH = True       # v29.5: FESX 제거 + TPX 추가
...
# ★ v29.5: 자산별 mult (FESX 제거, TPX 추가)
V29_2_ASSET_MULT = {
    # 강력 자산 (v29.4 검증)
    "ES":   1.5,   # PF 25.45 (v29.4: 1.3 → 1.5)
    "NQ":   1.2,   # PF 20.66 (v29.4: 1.0 → 1.2)
    # 기본 1.0
    "YM":   1.0,
    "RTY":  1.0,
    "NKD":  1.0,
    "TPX":  1.0,   # ⭐ v29.5 신규 (백테 후 조정)
    "FTSE": 1.0,   # PF 2.41
    "HSI":  1.0,   # PF 3.55
    "TWII": 1.0,   # PF 2.89
    # FDAX 제거됨 (v29.4)
    # FESX 제거됨 (v29.5: PF 0.46)
}
...
OUTDIR = Path("yahoo_index_v29_5_outputs")
...
def simulate_portfolio_v25(candidates_packs, max_total_risk=0.1325, scenario_name="BOOST15"):
    ...
    return {
        ...
        "strategy_variant": "v29_5_FESX_REMOVED_TPX_ADDED",
        "scenario": scenario_name,
        "trades": trades_df,
        "skipped": skipped_df,
        "equity": equity_df,
...
if __name__ == "__main__":
    ...
    try:
        res["trades"].to_csv(outdir / "v29_5_trades.csv", index=False)
        res["equity"].to_csv(outdir / "v29_5_equity.csv", index=False)
        res["skipped"].to_csv(outdir / "v29_5_skipped.csv", index=False)
        res["phase_transitions"].to_csv(outdir / "v29_5_phase_transitions.csv", index=False)
        res["excess_log"].to_csv(outdir / "v29_5_excess_log.csv", index=False)
        res["tax_log"].to_csv(outdir / "v29_5_tax_log.csv", index=False)
        all_cand.to_csv(outdir / "v29_5_all_candidates.csv", index=False)
        print(f"\n💾 CSV 저장 완료: {outdir}/")
    ...
    if len(tdf) == 0:
        ...
        summary_df.to_csv(outdir / "v29_5_summary.csv", index=False)
        ...
        monthly_summary.to_csv(outdir / "v29_5_monthly_summary.csv", index=False)
        ...
        asset_summary.to_csv(outdir / "v29_5_asset_summary.csv", index=False)
        ...
        if "tier_label" in tdf.columns:
            ...
            tier_summary.to_csv(outdir / "v29_5_tier_summary.csv", index=False)
        ...
        if "v25idx_subtier" in tdf.columns:
            ...
            subtier_summary.to_csv(outdir / "v29_5_v25idx_subtier_summary.csv", index=False)
            ...
            rule_summary.to_csv(outdir / "v29_5_v25idx_rule_summary.csv", index=False)

        # 차트
        try:
            plot_equity_v25(res, save_path=str(outdir / "v29_5_equity_chart.png"))
        except Exception as e:
