# =========================================================
# YAHOO INDEX FUTURES BACKTEST — v2.5 STAGE 4K BOOST15
#
# [v25 변경점 (vs v24_final)]
#   ★ Tier 시스템: v24n S/A/B/C/D → Stage 4K ALPHA/SWEEP/COMPLETE_OUT/SKIP_MSS
#   ★ Wick threshold: 0.1618 → 0.2300 (4K BOOST15 매트릭스 일치)
#   ★ atomic 12 컨디션 + 319 WIN69 canonical combos 적용
#   ★ Stage 4I 차단 규칙: GEM SHORT 차단, ROOM_ONLY blacklist (지수는 보수적으로 비움)
#   ★ Stage 4J 차단/booster: outlier 의존 차단 + 1.5×/1.2× 매트릭스
#   ★ Stage 4K Sentiment Booster: 직전 7일 LONG/SHORT RP 격차 기반
#   ★ RP_MULT_4H = {0: 1.5, 1~5: 1.0}
#   ★ TIER_CAP_4H: ALPHA 999×, SWEEP 3×
#   ★ Entry: 보수 모드 (LONG=zone_high, SHORT=zone_low, refine OFF)
#       → 실전 시장가 진입 모델과 일치 (4K 백테스트와 동일)
#
# [v25 유지 (v24_final 와 동일)]
#   ★ Phase A/B 자본 Cap 시스템
#   ★ 한국 해외선물 양도세 (22%, 250만원 공제)
#   ★ Yahoo 데이터 (8 indices: NQ/ES/YM/RTY/NKD/KS/SSE/SZSE)
#   ★ Dual-TF 2h/1h
#   ★ MIN_SCORE 8.5 (지수용 보수)
#
# [백테스트 기대]
#   - 크립토 4K BOOST15 비교 데이터:
#     PF 2.644, MDD -10.36%, Return 15,797% (9코인 3년)
#   - 지수는 자산 특성 다름:
#       . SWEEP 행동 패턴 약함 (외환은 stop hunt 약함)
#       . Volume 의미 다름 (지수는 거래량 → tick volume 근사)
#       . ATR scale 다름 (자산별 SL_BUFFER_MULT 검증 필요)
#     → Win rate, PF 자산별 변동 클 가능성
#     → 일단 4K 시스템 그대로 적용하고 결과 보면서 튜닝
# =========================================================
...
THRESHOLD_USD = {
    "NQ": 1_000_000.0, "ES": 1_000_000.0, "YM": 1_000_000.0, "RTY": 500_000.0,
    "NKD": 500_000.0, "KS": 300_000.0, "SSE": 300_000.0, "SZSE": 300_000.0,
}

PHASE_A_RISK = {
    "NQ":   0.010, "ES":   0.010, "YM":   0.010, "RTY":  0.010,
    "NKD":  0.010, "KS":   0.010, "SSE":  0.010, "SZSE": 0.010,
}

PHASE_B_RISK = {
    "NQ":   0.005, "ES":   0.005, "YM":   0.005, "RTY":  0.005,
    "NKD":  0.005, "KS":   0.005, "SSE":  0.005, "SZSE": 0.005,
}
...
GEM_LONG_ONLY = True
ROOM_ONLY_SYMBOL_BLACKLIST = set()  # 지수 데이터로 검증 후 추가
...
print("=" * 80)
print("YAHOO INDEX v25 — Stage 4K BOOST15 (8 INDICES)")
print("=" * 80)
print(f"Total assets: {len(INDEX_SYMBOLS)}")
print()
print("📊 Indices (Dual-TF 2h/1h):")
for k in INDEX_SYMBOLS:
    print(f"   {k:4s}: A_risk={PHASE_A_RISK[k]*100:.2f}%  B_risk={PHASE_B_RISK[k]*100:.2f}%  thr=${THRESHOLD_USD[k]/1e6:.1f}M")
print()
print(f"MIN_SCORE: {MIN_SCORE} | Volume Filter ON ({VOLUME_SPIKE_MULT})")
print(f"SCENARIO: {SCENARIO}")
print(f"Wick Q1 threshold: {WICK_RATIO_5_Q1_THRESHOLD}")
print(f"GEM_LONG_ONLY: {GEM_LONG_ONLY}")
print(f"ROOM_ONLY blacklist: {ROOM_ONLY_SYMBOL_BLACKLIST or 'NONE (지수 백테 후 추가)'}")
...
def resample_ohlcv(df_h1, rule):
    ...
    df["intra_idx"] = df.groupby("date_ny").cumcount()
    df["block_idx"] = (df["intra_idx"] // hours).astype(int)
    agg = (
        df.groupby(["date_ny", "block_idx"], as_index=False)
        .agg(
    ...
    for current_time in event_times:
        ...
        if current_time in entry_map:
            ...
            for idx, row in entry_events:
                ...
                if any(p["symbol_key"] == symbol_key for p in open_positions.values()):
                ...
                # RP filter (RP6+ SKIP)
                if RP_FILTER_ENABLE and rp_v not in RP_MULT_4H:
                    skipped_entries.append({
                        "entry_time": row["entry_time"], "symbol_key": symbol_key,
                ...
                # 최종 risk
                risk_pct_effective = risk_pct_base * tier_mult * rp_mult * stage4j_mult * sentiment_mult
                ...
                if projected_total_risk > allowed_total_risk + 1e-12:
                    skipped_entries.append({
                        "entry_time": row["entry_time"], "symbol_key": symbol_key,
...
# =========================================================
# %% Cell N: ★ APPENDIX — ATOMIC SYNERGY ANALYSIS (자동 실행) ★
# =========================================================
# 백테가 끝나면 OUTDIR 에 trades.csv + all_candidates.csv 가
# 저장됨. 이 블록은 그 결과를 자동으로 읽어 synergy 분해를 실행.
#
# [정의]
...
#
# [산출물 → OUTDIR/]
#   v25_stage4k_pairwise_synergy.csv
#   v25_stage4k_triple_synergy.csv
#   v25_stage4k_conditional_alpha.csv
#   v25_stage4k_anti_synergy.csv
#   v25_stage4k_synergy_ranked_combos.csv
# =========================================================

from itertools import combinations as _combinations

# ── 12 atom 정의 (크립토 4D / 지수 v25 동일) ──
_SYN_ATOMS_ALL_12 = [
    "a_sweep", "a_volume",
...
_SYN_ATOMS_SYN = [a for a in _SYN_ATOMS_ALL_12 if a != "a_room"]

_SYN_MIN_PAIR   = 30
_SYN_MIN_TRIPLE = 25
_SYN_MIN_COMBO  = 25
_SYN_MIN_COND   = 30
_SYN_PF_INF_CAP = 50.0


# ── ProcessPool worker 글로벌 (initializer 로 데이터 1회 적재) ──
_SYN_WORKER_DATA = {}


def _syn_init_worker(atom_matrix, net_pnl, r_multiple,
                     atom_pf, atom_count, baseline_pf, baseline_n):
    _SYN_WORKER_DATA["atoms"]       = atom_matrix
    _SYN_WORKER_DATA["net_pnl"]     = net_pnl
    _SYN_WORKER_DATA["r_multiple"]  = r_multiple
    _SYN_WORKER_DATA["atom_pf"]     = atom_pf
    _SYN_WORKER_DATA["atom_count"]  = atom_count
    _SYN_WORKER_DATA["baseline_pf"] = baseline_pf
    _SYN_WORKER_DATA["baseline_n"]  = baseline_n


def _syn_stats_from_mask(mask):
    n = int(mask.sum())
    if n == 0:
        return None
    pnl = _SYN_WORKER_DATA["net_pnl"][mask]
    rmul = _SYN_WORKER_DATA["r_multiple"][mask]
    g_p = float(pnl[pnl > 0].sum())
    g_l = float(abs(pnl[pnl < 0].sum()))
    pf = g_p / max(g_l, 1e-9) if g_l > 0 else (_SYN_PF_INF_CAP if g_p > 0 else 0.0)
    return {
        "trades":  n,
        "win_pct": float((pnl > 0).mean() * 100.0),
        "avg_R":   float(rmul.mean()),
        "PF":      float(min(pf, _SYN_PF_INF_CAP)),
        "total_pnl": float(pnl.sum()),
    }


def _syn_metrics(atom_indices, st):
    atom_pf  = _SYN_WORKER_DATA["atom_pf"]
    atom_cnt = _SYN_WORKER_DATA["atom_count"]
    base_pf  = _SYN_WORKER_DATA["baseline_pf"]
    base_n   = _SYN_WORKER_DATA["baseline_n"]

    indiv_pfs  = [float(atom_pf[i]) for i in atom_indices]
    indiv_cnts = [int(atom_cnt[i]) for i in atom_indices]
    n = len(atom_indices)
    ...
    synergy_lift  = actual_pf - max(indiv_pfs)
    ...
    return {
        ...
        "max_atom_PF":   max(indiv_pfs),
        "min_atom_PF":   min(indiv_pfs),
        "actual_count":  actual_count,
        "expected_count": expected_count,
        "freq_lift":     freq_lift,
        "indiv_PFs":     ",".join(f"{p:.2f}" for p in indiv_pfs),
    }


def _syn_worker_combo(args):
    size, atom_indices, atom_names, min_trades = args
    atoms = _SYN_WORKER_DATA["atoms"]
    mask = np.ones(atoms.shape[0], dtype=bool)
    for idx in atom_indices:
        mask = mask & atoms[:, idx]
    if int(mask.sum()) < min_trades:
        return None
    st = _syn_stats_from_mask(mask)
    if st is None:
        return None
    syn = _syn_metrics(atom_indices, st)
    return {"size": size, "combo": "+".join(atom_names), **st, **syn}


def _syn_worker_cond(args):
    a_idx, b_idx, a_name, b_name, min_trades = args
    atoms = _SYN_WORKER_DATA["atoms"]
    b_mask  = atoms[:, b_idx]
    b_st    = _syn_stats_from_mask(b_mask)
    ...
    ab_st = _syn_stats_from_mask(ab_mask) or {"PF":0,"win_pct":0,"trades":0}
    return {
        "A_atom": a_name, "given_B": b_name,
...
def _syn_stats_dict(sub):
    if len(sub) == 0:
        return {"trades":0, "win_pct":0.0, "avg_R":0.0, "PF":0.0, "total_pnl":0.0}
...
# === Synergy 분석 자동 실행 ===
if __name__ == "__main__":
    import time as _syn_time
    from multiprocessing import Pool as _SynPool, cpu_count as _syn_cpu_count

    _trades_path = OUTDIR / "v25_stage4k_trades.csv"
    _cand_path   = OUTDIR / "v25_stage4k_all_candidates.csv"

    if not _trades_path.exists():
        print(f"\n[SYNERGY] trades.csv 없음 → 분석 스킵: {_trades_path}")
    elif not _cand_path.exists():
        print(f"\n[SYNERGY] all_candidates.csv 없음 → 분석 스킵: {_cand_path}")
    else:
        print()
        print("=" * 95)
        ...
        _t_syn_start = _syn_time.time()
        _N_WORKERS_SYN = max(1, _syn_cpu_count() - 1)
        print(f"#   N_WORKERS: {_N_WORKERS_SYN} / cpu_count = {_syn_cpu_count()}")

        # 데이터 로드 + atom join
        _trades_syn = pd.read_csv(_trades_path)
        _cand_syn   = pd.read_csv(_cand_path)

        _atoms_cols_in = [f"atoms_{a}" for a in _SYN_ATOMS_ALL_12]
        _missing = [c for c in _atoms_cols_in if c not in _cand_syn.columns]
        if _missing:
            print(f"[SYNERGY] candidates 컬럼 누락 → 분석 스킵: {_missing}")
        else:
            for _c in _atoms_cols_in:
                _cand_syn[_c] = _syn_to_bool(_cand_syn[_c])
            _cand_syn = _cand_syn.rename(columns={f"atoms_{a}": a for a in _SYN_ATOMS_ALL_12})

            _trades_syn["entry_time"] = pd.to_datetime(_trades_syn["entry_time"])
            _cand_syn["entry_time"]   = pd.to_datetime(_cand_syn["entry_time"])

            _cols_t = ["trade_id", "symbol_key", "entry_time", "side",
                       "tier_label", "r_multiple", "net_pnl", "run_potential"]
            _cols_c = ["symbol_key", "entry_time", "side"] + _SYN_ATOMS_ALL_12

            _tdf_syn = pd.merge(
                _trades_syn[_cols_t], _cand_syn[_cols_c],
                on=["symbol_key", "entry_time", "side"], how="left",
            )
            for _c in _SYN_ATOMS_ALL_12:
                _tdf_syn[_c] = _tdf_syn[_c].fillna(False).astype(bool)

            print(f"[LOAD] trades={len(_trades_syn)} | candidates={len(_cand_syn)} | joined={len(_tdf_syn)}")

            # Baseline + 개별 atom PFs
            _atom_matrix = _tdf_syn[_SYN_ATOMS_ALL_12].values.astype(bool)
            _net_pnl_arr = _tdf_syn["net_pnl"].values.astype(float)
            _r_mul_arr   = _tdf_syn["r_multiple"].values.astype(float)
            _atom_idx    = {a: _SYN_ATOMS_ALL_12.index(a) for a in _SYN_ATOMS_ALL_12}

            _base_st = _syn_stats_dict(_tdf_syn)
            _baseline_pf = _base_st["PF"]
            _baseline_n  = _base_st["trades"]

            _atom_pf_arr     = np.zeros(len(_SYN_ATOMS_ALL_12))
            _atom_cnt_arr    = np.zeros(len(_SYN_ATOMS_ALL_12), dtype=int)
            _atom_winpct_arr = np.zeros(len(_SYN_ATOMS_ALL_12))
            for _i, _a in enumerate(_SYN_ATOMS_ALL_12):
                _m = _atom_matrix[:, _i]
                _st = _syn_stats_dict(_tdf_syn[_m])
                _atom_pf_arr[_i]     = _st["PF"]
                _atom_cnt_arr[_i]    = _st["trades"]
                _atom_winpct_arr[_i] = _st["win_pct"]

            print()
            print(f"[BASELINE] Trades {_baseline_n} | Win {_base_st['win_pct']:.2f}% | "
                  f"PF {_baseline_pf:.3f} | avg_R {_base_st['avg_R']:.3f}")
            print()
            print("[INDIVIDUAL ATOM PFs]")
            print(f"  {'atom':<22} {'trades':>7} {'PF':>7} {'win%':>7}")
            print("  " + "-" * 50)
            for _i, _a in enumerate(_SYN_ATOMS_ALL_12):
                _marker = " (excl.)" if _a == "a_room" else ""
                print(f"  {_a:<22} {_atom_cnt_arr[_i]:>7} {_atom_pf_arr[_i]:>7.3f} "
                      f"{_atom_winpct_arr[_i]:>6.2f}%{_marker}")

            _pool_kwargs = dict(
                processes=_N_WORKERS_SYN,
                initializer=_syn_init_worker,
                initargs=(_atom_matrix, _net_pnl_arr, _r_mul_arr,
                          _atom_pf_arr, _atom_cnt_arr, _baseline_pf, _baseline_n),
            )
            ...
            print("=" * 95)
            _pair_jobs = [(2, (_atom_idx[a1], _atom_idx[a2]), (a1, a2), _SYN_MIN_PAIR)
                          for a1, a2 in _combinations(_SYN_ATOMS_SYN, 2)]
            print(f"  jobs: {len(_pair_jobs)} | min_trades: {_SYN_MIN_PAIR}")
            _t0 = _syn_time.time()
            with _SynPool(**_pool_kwargs) as _pool:
                _pair_results = _pool.map(_syn_worker_combo, _pair_jobs)
            _pair_results = [r for r in _pair_results if r is not None]
            print(f"  완료: {len(_pair_results)} valid pairs ({_syn_time.time()-_t0:.1f}s)")

            _pair_df = pd.DataFrame(_pair_results)
            if len(_pair_df) > 0:
                _pair_df = _pair_df.sort_values("synergy_ratio", ascending=False).reset_index(drop=True)
                _pair_df = _pair_df.round({"win_pct":2, "avg_R":3, "PF":3, "total_pnl":2,
                                           "actual_PF":3, "expected_PF":3, "synergy_ratio":3,
                                           "synergy_lift":3, "max_atom_PF":3, "min_atom_PF":3,
                                           "expected_count":2, "freq_lift":3})
                _out_path = OUTDIR / "v25_stage4k_pairwise_synergy.csv"
                _pair_df.to_csv(_out_path, index=False)
                print(f"  [SAVE] {_out_path}")
                print()
                print(f"  {'#':<3} {'pair':<50} {'tr':>5} {'PF':>6} {'expPF':>6} "
                      f"{'synR':>6} {'synLift':>8}")
                print("  " + "-" * 95)
                for _i, _r in _pair_df.head(15).iterrows():
                    print(f"  {_i+1:<3} {_r['combo']:<50} {int(_r['trades']):>5} "
                          f"{_r['actual_PF']:>6.2f} {_r['expected_PF']:>6.2f} "
                          f"{_r['synergy_ratio']:>6.2f} {_r['synergy_lift']:>+8.3f}")
            ...
            print("=" * 95)
            _triple_jobs = [(3, tuple(_atom_idx[a] for a in trio), trio, _SYN_MIN_TRIPLE)
                            for trio in _combinations(_SYN_ATOMS_SYN, 3)]
            print(f"  jobs: {len(_triple_jobs)} | min_trades: {_SYN_MIN_TRIPLE}")
            _t0 = _syn_time.time()
            with _SynPool(**_pool_kwargs) as _pool:
                _triple_results = _pool.map(_syn_worker_combo, _triple_jobs, chunksize=32)
            _triple_results = [r for r in _triple_results if r is not None]
            print(f"  완료: {len(_triple_results)} valid triples ({_syn_time.time()-_t0:.1f}s)")

            _triple_df = pd.DataFrame(_triple_results)
            if len(_triple_df) > 0:
                _triple_df = _triple_df.sort_values("synergy_ratio", ascending=False).reset_index(drop=True)
                _triple_df = _triple_df.round({"win_pct":2, "avg_R":3, "PF":3, "total_pnl":2,
                                               "actual_PF":3, "expected_PF":3, "synergy_ratio":3,
                                               "synergy_lift":3, "max_atom_PF":3, "min_atom_PF":3,
                                               "expected_count":2, "freq_lift":3})
                _out_path = OUTDIR / "v25_stage4k_triple_synergy.csv"
                _triple_df.head(50).to_csv(_out_path, index=False)
                print(f"  [SAVE] {_out_path}")
                print()
                print(f"  {'#':<3} {'triple':<60} {'tr':>5} {'PF':>6} {'expPF':>6} "
                      f"{'synR':>6} {'synLift':>8}")
                print("  " + "-" * 105)
                for _i, _r in _triple_df.head(15).iterrows():
                    _cstr = _r['combo'][:58]
                    print(f"  {_i+1:<3} {_cstr:<60} {int(_r['trades']):>5} "
                          f"{_r['actual_PF']:>6.2f} {_r['expected_PF']:>6.2f} "
                          f"{_r['synergy_ratio']:>6.2f} {_r['synergy_lift']:>+8.3f}")
            ...
            print("=" * 95)
            _cond_jobs = [(_atom_idx[a], _atom_idx[b], a, b, _SYN_MIN_COND)
                          for a in _SYN_ATOMS_SYN for b in _SYN_ATOMS_SYN if a != b]
            print(f"  jobs: {len(_cond_jobs)} | min_trades: {_SYN_MIN_COND}")
            _t0 = _syn_time.time()
            with _SynPool(**_pool_kwargs) as _pool:
                _cond_results = _pool.map(_syn_worker_cond, _cond_jobs, chunksize=32)
            print(f"  완료 ({_syn_time.time()-_t0:.1f}s)")

            _cond_df   = pd.DataFrame(_cond_results)
            _cond_df_v = _cond_df[_cond_df["valid"]].drop(columns=["valid"]).copy()
            _cond_df_v = _cond_df_v.round({"B_only_PF":3, "B_only_winpct":2,
                                           "AB_PF":3, "AB_winpct":2,
                                           "cond_alpha_PF":3, "cond_alpha_winpct":2})
            _out_path = OUTDIR / "v25_stage4k_conditional_alpha.csv"
            _cond_df_v.to_csv(_out_path, index=False)
            print(f"  [SAVE] {_out_path}")
            print()
            print("  [TOP 15 cond_alpha_PF]")
            _cond_top = _cond_df_v.sort_values("cond_alpha_PF", ascending=False).head(15)
            print(f"  {'#':<3} {'A given B':<55} {'B_only':>7} {'A∧B':>7} "
                  f"{'AB_tr':>7} {'cond_α':>9}")
            print("  " + "-" * 100)
            for _i, _r in _cond_top.reset_index(drop=True).iterrows():
                _s = f"{_r['A_atom']} | {_r['given_B']}"
                print(f"  {_i+1:<3} {_s:<55} {_r['B_only_PF']:>7.3f} {_r['AB_PF']:>7.3f} "
                      f"{int(_r['AB_trades']):>7} {_r['cond_alpha_PF']:>+9.3f}")
            print()
            print("  [BOTTOM 10 cond_alpha_PF]")
            _cond_bot = _cond_df_v.sort_values("cond_alpha_PF").head(10)
            for _i, _r in _cond_bot.reset_index(drop=True).iterrows():
                _s = f"{_r['A_atom']} | {_r['given_B']}"
                print(f"  {_i+1:<3} {_s:<55} {_r['B_only_PF']:>7.3f} {_r['AB_PF']:>7.3f} "
                      f"{int(_r['AB_trades']):>7} {_r['cond_alpha_PF']:>+9.3f}")
            ...
            print("=" * 95)
            _anti_p = _pair_df[_pair_df["synergy_ratio"] < 1.0].copy() if len(_pair_df) > 0 else pd.DataFrame()
            _anti_t = _triple_df[_triple_df["synergy_ratio"] < 1.0].copy() if len(_triple_df) > 0 else pd.DataFrame()
            if len(_anti_p) > 0:
                _anti_p["combo_size"] = 2
            if len(_anti_t) > 0:
                _anti_t["combo_size"] = 3
            _anti_df = pd.concat([_anti_p, _anti_t], ignore_index=True) if (len(_anti_p) or len(_anti_t)) else pd.DataFrame()

            if len(_anti_df) > 0:
                _anti_df = _anti_df.sort_values("synergy_ratio").reset_index(drop=True)
                _out_path = OUTDIR / "v25_stage4k_anti_synergy.csv"
                _anti_df.to_csv(_out_path, index=False)
                print(f"  [SAVE] {_out_path}  ({len(_anti_df)} 개)")
                print()
                print(f"  {'#':<3} {'size':<5} {'combo':<55} {'tr':>5} {'PF':>6} "
                      f"{'expPF':>6} {'synR':>6}")
                print("  " + "-" * 100)
                for _i, _r in _anti_df.head(15).iterrows():
                    _cstr = _r["combo"][:53]
                    print(f"  {_i+1:<3} {int(_r['combo_size']):<5} {_cstr:<55} "
                          f"{int(_r['trades']):>5} {_r['actual_PF']:>6.2f} "
                          f"{_r['expected_PF']:>6.2f} {_r['synergy_ratio']:>6.3f}")
            else:
                print("  반시너지 조합 없음")
            ...
            print("=" * 95)
            _combo_jobs = []
            for _size in (2, 3, 4, 5):
                for _combo in _combinations(_SYN_ATOMS_SYN, _size):
                    _idx_t = tuple(_atom_idx[a] for a in _combo)
                    _combo_jobs.append((_size, _idx_t, _combo, _SYN_MIN_COMBO))
            print(f"  jobs: {len(_combo_jobs):,} | min_trades: {_SYN_MIN_COMBO}")
            _t0 = _syn_time.time()
            with _SynPool(**_pool_kwargs) as _pool:
                _combo_results = _pool.map(_syn_worker_combo, _combo_jobs, chunksize=64)
            _combo_results = [r for r in _combo_results if r is not None]
            print(f"  완료: {len(_combo_results):,} valid combos ({_syn_time.time()-_t0:.1f}s)")

            _combo_df = pd.DataFrame(_combo_results)
            if len(_combo_df) > 0:
                _combo_top = _combo_df.sort_values("synergy_ratio", ascending=False).head(50)
                _combo_top = _combo_top.round({"win_pct":2, "avg_R":3, "PF":3, "total_pnl":2,
                                               "actual_PF":3, "expected_PF":3, "synergy_ratio":3,
                                               "synergy_lift":3, "max_atom_PF":3, "min_atom_PF":3,
                                               "expected_count":2, "freq_lift":3})
                _out_path = OUTDIR / "v25_stage4k_synergy_ranked_combos.csv"
                _combo_top.to_csv(_out_path, index=False)
                print(f"  [SAVE] {_out_path}")
                print()
                ...
                print("  " + "-" * 110)
                for _i, _r in _combo_top.head(20).reset_index(drop=True).iterrows():
                    _cstr = _r['combo'][:58]
                    print(f"  {_i+1:<3} {int(_r['size']):<5} {_cstr:<60} "
                          f"{int(_r['trades']):>5} {_r['actual_PF']:>6.2f} "
                          f"{_r['expected_PF']:>6.2f} {_r['synergy_ratio']:>6.2f} "
                          f"{_r['synergy_lift']:>+7.3f}")

            print()
            print("=" * 95)
            print(f"[SYNERGY DONE] {_syn_time.time() - _t_syn_start:.1f}s | 산출물 → {OUTDIR}/")
            print("=" * 95)
            for _f in ["v25_stage4k_pairwise_synergy.csv",
                       "v25_stage4k_triple_synergy.csv",
                       "v25_stage4k_conditional_alpha.csv",
                       "v25_stage4k_anti_synergy.csv",
                       "v25_stage4k_synergy_ranked_combos.csv"]:
