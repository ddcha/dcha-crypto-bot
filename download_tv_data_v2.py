"""
================================================================
Tradingview 데이터 일괄 다운로드 스크립트 (v2 - 둠챠 지시 반영)
================================================================
- 한 번 실행해서 11개 자산 1H csv 저장
- 이후 v29.3 백테는 csv 만 읽음 → yfinance 의존 X
- 다운로드 시간: 자산당 약 5~10초 (총 약 2~3분)
- 저장 위치: ./tv_data/

[v1 → v2 변경 (2026-04-29 둠챠 지시):]
  - KS (KOSPI200) 제거
  - FTSE 100 (Z1! @ ICEEUR) 추가
  - 합계 11자산 (US 4 + JP 1 + EU 3 + HK 1 + TW 1 + AU 1)

설치:
  pip install tvDatafeed
  
실행:
  python download_tv_data.py
================================================================
"""
...
N_BARS = 5500          # 729일 × 24h = 약 17,500. 거래시간만 → 약 5,000~6,000봉
RETRY = 3
SLEEP_BETWEEN = 2      # 초
