...
#
# [v26 변경점 (vs v25)]
#   ★★★ v19b RP boost layer 추가 (기존 v25 에서 누락) ★★★
#   ★ classify_tier_v19b_rp_boost: pre/sweep/score/wick + RP → S/A/B/C/D
#       S (D AND wick_Q1):       risk × 3.0
#       A (D 단독):               risk × 1.5
#       B (wick_Q1 단독):         risk × 0.8
#       C (pre>=1):               risk × 0.7
#       D (pre=0):                SKIP
#   ★ RP_TIER_MULT_TABLE: rp 0,1 → 1.5× / rp 2-5 → 1.0× / rp 6+ → SKIP
#   ★ Risk 최종식: base × tier_v19b_mult × tier_4h_mult × stage4j_mult × sentiment_mult
#       (이전 v25 는 tier_4h_mult 만 있었음 — v19b layer 추가로 진짜 4K 완성)
#   ★ candidate 출력에 tier_v19b 컬럼 추가
#
# [v25 유지 (vs v24_final)]
    ...
    for current_time in event_times:
        ...
        if current_time in entry_map:
            ...
            for idx, row in entry_events:
                ...
                # 최종 risk: base × tier_v19b × tier_4h × rp_4h × stage4j × sentiment
                #   (v25 = base × tier_4h × rp × j × sent  →  v26 = + tier_v19b 추가)
