# =========================================================
# YAHOO INDEX FUTURES BACKTEST — v2.8 ATOM SEARCH
#
# [v28 변경점 (vs v27)]
#   ★★★ score 7.5(or 8.5) gate 통과한 모든 거래 받음 ★★★
#       - tier_4h 차단 제거 (SKIP_MSS, COMPLETE_OUT, ROOM_ONLY blacklist 모두 거래)
#       - tier_v19b D 차단 제거 (D tier 도 거래)
#       - RP 차단 제거 (RP 6+ 도 거래)
#       - stage4j 차단 제거
#       - GEM_LONG_ONLY 차단 제거
#   ★★★ 34 atom 으로만 trade record 구성 ★★★
#       카테고리 1: atomic 12 (4K base)
#       카테고리 2: v19b decision raw (3) - v19b_d_cond, v19b_wick_q1, v19b_pre_ge1
#       카테고리 3: RP (6) - rp_eq_0 ~ rp_eq_5
#       카테고리 4: tier_4h decision (9) - 분해된 진짜 atom
#       카테고리 5: tier_v19b match (4) - v19b_match_S/A/B/C
#       → 라벨 atom (is_alpha_max 등) 제거. 9개 + 4개 분해 atom 으로 대체.
#   ★★★ Risk 평탄화 (모든 mult 1.0) ★★★
#       risk_pct_effective = risk_pct_base
#       → 순수 진입 조건 alpha 측정
#
# [목적]
#   34 atom 의 모든 OR/AND 상호작용 분석으로 자산군별 진짜 alpha 발견.
#   v27 의 5 조합 whitelist 는 reference 로만 유지 (차단 안함).
# =========================================================
#
# [v25 유지 (vs v24_final)]
#   ★ Tier 4H 시스템: v24n S/A/B/C/D → Stage 4K ALPHA/SWEEP/COMPLETE_OUT/SKIP_MSS
...
INDEX_SYMBOLS = {
    # ============================================================
    # ★ v29.3: 10자산 (KS 제거, AXJO 제거 - 데이터 미수신, FTSE 추가) ★
    # ============================================================
    # 🇺🇸 US 4
    "NQ":   "NQ_TV",        # csv (TV: CME_MINI:NQ1!)
    "ES":   "ES_TV",        # csv (TV: CME_MINI:ES1!)
    "YM":   "YM_TV",        # csv (TV: CBOT_MINI:YM1!)
    "RTY":  "RTY_TV",       # csv (TV: CME_MINI:RTY1!)
    # 🇯🇵 JP 1
    "NKD":  "NKD_TV",       # csv (TV: CME:NKD1!)
    # 🇩🇪 EU 3 (v29.3: FTSE 추가)
    "FDAX": "FDAX_TV",      # csv (TV: EUREX:FDAX1!)
    "FESX": "FESX_TV",      # csv (TV: EUREX:FESX1!)
    "FTSE": "FTSE_TV",      # ⭐ v29.3: csv (TV: ICEEUR:Z1!)
    # 🇭🇰 HK 1
    "HSI":  "HSI_TV",       # csv (TV: HKEX:HSI1!)
    # 🇹🇼 TW 1
    "TWII": "TWII_TV",      # csv (TV: TAIFEX:TXF1!)
}
SINGLE_TARGETS = list(INDEX_SYMBOLS.keys())

# yahoo_symbol → key reverse map (csv cache lookup 용)
YAHOO_TO_KEY = {v: k for k, v in INDEX_SYMBOLS.items()}

SYMBOL_TF_MAP = {
    "NQ":   ("2h", "1h"), "ES":   ("2h", "1h"),
    "YM":   ("2h", "1h"), "RTY":  ("2h", "1h"),
    "NKD":  ("2h", "1h"),
    "FDAX": ("2h", "1h"), "FESX": ("2h", "1h"),
    "FTSE": ("2h", "1h"),    # ⭐ v29.3 신규
    "HSI":  ("2h", "1h"),
    "TWII": ("2h", "1h"),
}
...
# ============================================================
# ★★★ v29.2: TradingView CSV 캐시 모드 ★★★
# ============================================================
USE_TV_CSV_CACHE = True               # True 면 ./tv_data/{key}_1h.csv 우선 사용
TV_CSV_DIR = Path("./tv_data")        # download_tv_data.py 가 저장하는 디렉토리
ALLOW_YFINANCE_FALLBACK = False       # csv 없을 때 yfinance 시도? (True 면 fallback)
...
THRESHOLD_USD = {
    ...
    "NKD": 500_000.0,
    # 🇩🇪 EU 3 (v29.3 신규)
    "FDAX": 500_000.0, "FESX": 500_000.0, "FTSE": 500_000.0,
    # 🇭🇰🇹🇼 HK/TW 2 (v29.3 신규)
    "HSI": 300_000.0, "TWII": 300_000.0,
}
...
PHASE_A_RISK = {
    ...
    "NKD":  0.010,
    # 🇩🇪 EU 3 (v29.3 신규)
    "FDAX": 0.010, "FESX": 0.010, "FTSE": 0.010,
    # 🇭🇰🇹🇼 HK/TW 2 (v29.3 신규)
...
PHASE_B_RISK = {
    ...
    "NKD":  0.005,
    # 🇩🇪 EU 3 (v29.3 신규)
    "FDAX": 0.005, "FESX": 0.005, "FTSE": 0.005,
    # 🇭🇰🇹🇼 HK/TW 2 (v29.3 신규)
...
# 34 atom 으로만 trade record 구성.
# Risk 는 모든 mult 1.0 평탄화 (순수 alpha 측정).

SCORE_ONLY_GATE  = True   # ← 핵심: True 면 score 만 통과하면 모든 거래
USE_FLAT_RISK_MULT = True  # 모든 risk mult 1.0

# ============================================================
# ★★★ v29.3 패치: 시나리오 B 차단 + RULE mult + 자산별 mult ★★★
# v29.2 → v29.3 변경:
#   - 자산 정의 변경: KS 제거, FTSE 추가 (10자산)
#   - AXJO 제거 (데이터 미수신)
#   - mult 정책: v29.2 그대로 (단, KS 자산mult 제거)
# ============================================================
USE_V29_1_PATCH = True       # v29.1 base 차단 (시나리오 B 12종)
USE_V29_2_PATCH = True       # v29.2 신규: 자산별 mult 활성화
USE_V29_3_PATCH = True       # v29.3 신규: 10자산 + FTSE 추가

# v29.2 RULE mult (v29.1 → v29.2 mult 조정 반영)
V29_1_RULE_MULT = {
    # S++ 핵심
    "S++_MSS_OVL":          5.0,   # n=10  Win 90%   PF 579  (v29.1 운용 무패)
    "S++_SC24_FVG_WICK":    4.5,   # n=23  Win 87%   PF 105
    "S++_SC24_WICK_RFVG":   4.5,   # 도달불가 (참고)
    "S++_SC24_SCORE_WICK":  2.5,   # n=5   Win 60%   PF 2.63
    # S 핵심 (승격급)
    "S_PRE4_WICK_MAX":      4.0,   # n=58  Win 75.9% PF 6.96 ⭐ 승격
    "S_FVG_MAX_S":          2.5,   # ★ v29.2: 3.0 → 2.5 (큰 avg_loss 완화)
    # A+ 중 승격
    "A+_FVG_D_RP3":         3.5,   # n=8   Win 62.5% PF 5.61 ⭐ 승격
    # 나머지 S 계열
    "S_PRE4_SCORE":         2.0,   # ★ v29.2: 2.5 → 2.0 (큰 avg_loss 완화)
    "S_SCORE_S":            2.5,   # rp=3 만 살아남음 (n=8 Win 62.5%)
    "S_SCORE_MAX_WICK":     2.5,   # 도달불가
    # A+ 나머지 (★ v29.2: 1.2 → 1.0 통일)
    "A+_MAX":               1.0,
    "A+_SCORE_FVG":         1.0,
    # A 전부 (★ v29.2: 1.2 → 1.0 통일)
    "A_HIGH":               1.0,
    "A_RP3":                1.0,
    "A_RP2":                1.0,
    # S+ 5종 (코드상 도달 불가, 안전망)
    "S+_MAX_RFVG_S":        3.0, "S+_MAX_RFVG_WICK": 3.0,
    "S+_SCORE_RFVG_S":      3.0, "S+_MAX_HSV_S":     3.0, "S+_FVG_HSV_S": 3.0,
    # A- / OTHER (안전망)
    "A-_ROOM_ONLY":         1.0, "A-_OUT": 1.0, "OTHER": 1.0,
}

# ★ v29.3: 자산별 mult (KS 제거, ES 1.3x 만 유지, 신규 자산은 1.0 starts)
#   향후 v29.3 백테 결과 따라 신규 자산도 mult 조정 예정
V29_2_ASSET_MULT = {
    # 기존 (v29.2 검증)
    "ES":   1.3,   # PF 16.27, Win 79.5% — 두번째 강함
    # KS:  1.5  ← v29.3 에서 제거 (한투 매매 X)
    # 기본 (v29.3 추가 자산은 일단 1.0, 백테 후 조정)
    "NQ":   1.0,
    "YM":   1.0,
    "RTY":  1.0,
    "NKD":  1.0,
    "FDAX": 1.0,   # ⭐ v29.3 신규 자산 (백테 후 조정)
    "FESX": 1.0,
    "FTSE": 1.0,
    "HSI":  1.0,
    "TWII": 1.0,
...
# v27 reference (차단 안함, 분석 후 v29 에서 고려)
ALLOWED_TIER_COMBOS = {
    ("ALPHA_MAX",  "S"),
    ("ALPHA_MAX",  "A"),
    ("ALPHA_HIGH", "S"),
...
ATOMIZED_MODE = USE_COMBO_WHITELIST  # 호환 alias
...
OUTDIR = Path("yahoo_index_v29_3_outputs")
...
print("=" * 80)
print("YAHOO INDEX v29 — XB BLOCK (new RULE, XB skip, risk flat 1.0)")
print("=" * 80)
...
def _load_csv_1h(key, tz=TIMEZONE):
    """v29.2: tv_data/{key}_1h.csv 에서 로드. 컬럼/타임존을 yfinance 형식으로 변환"""
    csv_path = TV_CSV_DIR / f"{key}_1h.csv"
    if not csv_path.exists():
        raise FileNotFoundError(f"CSV cache not found: {csv_path}")

    df = pd.read_csv(csv_path)

    # 첫 컬럼 = datetime (download_tv_data.py 가 'Datetime' 으로 저장)
    ts_col = None
    for c in df.columns:
        if str(c).lower() in ["datetime", "date", "time", "timestamp", "unnamed: 0"]:
            ts_col = c
            break
    if ts_col is None:
        ts_col = df.columns[0]
    ...
    # 컬럼 정규화 (Open/High/Low/Close/Volume → open/high/low/close/volume)
    rename_lower = {c: c.lower() for c in df.columns if c != "timestamp"}
    df = df.rename(columns=rename_lower)

    needed = ["open", "high", "low", "close", "volume"]
    for c in needed:
        if c not in df.columns:
            raise ValueError(f"CSV {csv_path} missing column: {c}. cols={list(df.columns)}")
    ...
    if len(df) == 0:
        raise ValueError(f"CSV {csv_path} loaded empty after parse")

    return df


def load_yahoo_1h(symbol, period=PERIOD_1H, interval=INTERVAL_1H, tz=TIMEZONE):
    """v29.2: csv 우선, fallback 으로 yfinance"""
    # 1) CSV 캐시 우선 (USE_TV_CSV_CACHE=True 면)
    if USE_TV_CSV_CACHE:
        key = YAHOO_TO_KEY.get(symbol, symbol)
        try:
            df = _load_csv_1h(key, tz=tz)
            print(f"  [CSV] {key} loaded {len(df)} bars from {TV_CSV_DIR / f'{key}_1h.csv'}")
            return df
        except FileNotFoundError as e:
            if not ALLOW_YFINANCE_FALLBACK:
                raise FileNotFoundError(
                    f"v29.2 CSV cache mode: {e}. "
                    f"먼저 download_tv_data.py 를 실행하세요. "
                    f"또는 ALLOW_YFINANCE_FALLBACK=True 로 yfinance fallback 허용."
                )
            print(f"  [CSV miss] {symbol} → yfinance fallback")
        except Exception as e:
            if not ALLOW_YFINANCE_FALLBACK:
                raise
            print(f"  [CSV error] {symbol}: {e} → yfinance fallback")

    # 2) yfinance (legacy 또는 fallback)
    df = yf.download(
...
def simulate_portfolio_v25(candidates_packs, max_total_risk=0.1325, scenario_name="BOOST15"):
    ...
    for current_time in event_times:
        ...
        if current_time in entry_map:
            ...
            for idx, row in entry_events:
                ...
                # SCORE_ONLY_GATE=True 면 score 만 통과한 모든 거래 받음
                
                # 4J 차단 (v28: SCORE_ONLY_GATE 시 우회)
                ...
                # tier_4h 차단 (SKIP_MSS, COMPLETE_OUT, ROOM_ONLY blacklist, GEM SHORT)
                # v28: SCORE_ONLY_GATE 시 우회
                ...
                # RP filter (RP6+ SKIP) - v28: SCORE_ONLY_GATE 시 우회
                ...
                # ★★★ v26 신규: v19b RP_BOOST tier (별도 layer) ★★★
                ...
                # tier_v19b D 차단 - v28: SCORE_ONLY_GATE 시 우회
                ...
                # whitelist 차단 - v28: SCORE_ONLY_GATE 시 우회
                ...
                # ============================================================
                # ★★★ v29: 신규 INDEX RULE 분류 + XB 차단 ★★★
                # 모든 atom 조합 통계 분석 결과로 정의된 sub-tier
                # 우선순위 매칭 (위에서 아래로 첫 매치)
                # XB 만 차단, 다른 등급은 모두 거래 (mult 는 1.0 평탄)
                # ============================================================
                # candidate row 에서 atoms_a_* 컬럼 읽기 (simulate_portfolio 단계)
                _score_ge13 = bool(row.get('atoms_a_score_ge13', False))
                _fvg        = bool(row.get('atoms_a_fvg', False))
                _wick_q1    = bool(row.get('atoms_a_wick_le_q1', False))
                _pre_ge4    = bool(row.get('atoms_a_pre_total_ge4', False))
                _overlap    = bool(row.get('atoms_a_overlap', False))
                _mss        = bool(row.get('atoms_a_mss', False))
                _sweep_count_2_4 = bool(row.get('atoms_a_sweep_count_2_4', False))
                _trend_align     = bool(row.get('atoms_a_trend_align', False))
                _high_sweep_vol  = bool(row.get('t4h_high_sweep_vol', False))
                ...
                _rp = int(rp_v)
                _t4h_max_score_pre   = (tier_label == "ALPHA_MAX")
                _t4h_high_vol_overlap= (tier_label == "ALPHA_HIGH" and _overlap)  # 근사
                _t4h_sweep_room_fvg  = (tier_label == "SWEEP_ROOM_FVG")
                _t4h_sweep_room_only = (tier_label == "SWEEP_ROOM_ONLY")
                _t4h_high_sweep_vol  = (tier_label in ("ALPHA_HIGH", "ALPHA_MAX")) and _sweep_count_2_4
                _t4h_skip_mss        = (tier_label == "SKIP_MSS")
                _t4h_alpha_high      = (tier_label == "ALPHA_HIGH")
                _t4h_alpha_med       = (tier_label == "ALPHA_MED")
                _t4h_complete_out    = (tier_label == "COMPLETE_OUT")

                # ----- RULE 매칭 (priority high → low) -----
                v25idx_rule  = "OTHER"
                ...
                # 백테 분석 결과 12개 패턴 모두 NetPnL 음수 확정
                # ============================================================
                if USE_V29_1_PATCH and v25idx_subtier != "XB":
                    if v25idx_rule == "A_HIGH":
                        ...
                        elif side == "long" and symbol_key == "SSE":
                            v25idx_rule = "XB_AH_LONG_SSE"; v25idx_subtier = "XB"
                        elif side == "short" and symbol_key == "SZSE":
                            v25idx_rule = "XB_AH_SHORT_SZSE"; v25idx_subtier = "XB"
                ...
                # ----- v25idx_rule 별 mult 결정 (분기 흐름과 무관하게 항상 정의) -----
                if USE_V29_1_PATCH and v25idx_subtier != "XB":
                    v25idx_rule_mult = V29_1_RULE_MULT.get(v25idx_rule, 1.0)
                else:
                    v25idx_rule_mult = 1.0  # XB 거래(차단) 또는 패치 OFF

                # ★ v29.3: 자산별 mult (RULE mult 와 곱셈)
                if USE_V29_2_PATCH and v25idx_subtier != "XB":
                    v29_2_asset_mult = V29_2_ASSET_MULT.get(symbol_key, 1.0)
                else:
                    v29_2_asset_mult = 1.0
                v25idx_combined_mult = v25idx_rule_mult * v29_2_asset_mult

                # ----- XB 차단 -----
                if v25idx_subtier == "XB":
                    skipped_entries.append({
                        "entry_time": row["entry_time"], "symbol_key": symbol_key,
                        "reason": f"v25idx_xb_{v25idx_rule}",
                        "tier_label": tier_label, "tier_v19b": tier_v19b_label,
                        "phase_at_entry": current_phase_for_entry,
                ...
                # ★★★ v29.3: RULE mult × 자산 mult 결합 적용 ★★★
                if USE_V29_1_PATCH:
                    risk_pct_effective = risk_pct_base * v25idx_combined_mult
                    tier_v19b_mult_applied = 1.0
                    tier_mult_applied = 1.0
                    rp_mult_applied = 1.0
                    stage4j_mult_applied = 1.0
                    sentiment_mult_applied = v25idx_combined_mult  # 추적용 (실제 적용 mult)
                elif USE_FLAT_RISK_MULT:
                    # 모든 mult 1.0 → risk = base 만
                    risk_pct_effective = risk_pct_base
                    # 분석용 기록은 유지 (실제 적용은 1.0 으로)
                    tier_v19b_mult_applied = 1.0
                    tier_mult_applied = 1.0
                    rp_mult_applied = 1.0
                    stage4j_mult_applied = 1.0
                ...
                open_positions[idx] = {
                    ...
                    "v25idx_rule": v25idx_rule,
                    "v25idx_subtier": v25idx_subtier,
                    # v29.1: RULE 별 차등 mult
                    "v25idx_rule_mult": v25idx_rule_mult,
                    # v29.3: 자산 mult + 결합 mult
                    "v29_2_asset_mult": v29_2_asset_mult,
                    "v25idx_combined_mult": v25idx_combined_mult,
                    ...
                    # ★★★ v27 atom 컬럼 (atom OR search 용) ★★★
                    # ★ v28: 카테고리 1 - atomic 12 (4K base, 12개) ★
                    ...
                    # ★ v28: 카테고리 2 - v19b decision raw (3개) ★
                    ...
                    "rp_eq_4": rp_v == 4,
                    "rp_eq_5": rp_v == 5,
                    # ★ v28: 카테고리 4 - tier_4h decision atom (9개, 분해된 진짜 atom) ★
                    "t4h_max_overlap": (
                    ...
                    # ★ v28: 카테고리 5 - tier_v19b match atom (4개, 분해) ★
                    "v19b_match_S": (
    ...
    return {
        ...
        "strategy_variant": "v29_3_RULE_ASSET_MULT",
        "scenario": scenario_name,
        "trades": trades_df,
        "skipped": skipped_df,
        "equity": equity_df,
...
if __name__ == "__main__":
    ...
    try:
        res["trades"].to_csv(outdir / "v29_3_trades.csv", index=False)
        res["equity"].to_csv(outdir / "v29_3_equity.csv", index=False)
        res["skipped"].to_csv(outdir / "v29_3_skipped.csv", index=False)
        res["phase_transitions"].to_csv(outdir / "v29_3_phase_transitions.csv", index=False)
        res["excess_log"].to_csv(outdir / "v29_3_excess_log.csv", index=False)
        res["tax_log"].to_csv(outdir / "v29_3_tax_log.csv", index=False)
        all_cand.to_csv(outdir / "v29_3_all_candidates.csv", index=False)
        print(f"\n💾 CSV 저장 완료: {outdir}/")
    ...
    if len(tdf) == 0:
        ...
        summary_df.to_csv(outdir / "v29_3_summary.csv", index=False)
        ...
        monthly_summary.to_csv(outdir / "v29_3_monthly_summary.csv", index=False)
        ...
        asset_summary.to_csv(outdir / "v29_3_asset_summary.csv", index=False)
        ...
        if "tier_label" in tdf.columns:
            ...
            tier_summary.to_csv(outdir / "v29_3_tier_summary.csv", index=False)

        # ============================================================
        # v29: v25idx_subtier 별 + v25idx_rule 별 summary
        # ============================================================
        if "v25idx_subtier" in tdf.columns:
            print("\n" + "=" * 110)
            print("  [v29] v25idx_subtier 별 stat")
            print("=" * 110)
            subtier_summary = tdf.groupby("v25idx_subtier").apply(
                lambda x: pd.Series({
                    "n": len(x),
                    "winrate_pct": (x["net_pnl"] > 0).mean() * 100,
                    "avg_r": x["r_multiple"].mean(),
                    "net_pnl_usd": x["net_pnl"].sum(),
                    "PF": (x.loc[x["net_pnl"] > 0, "net_pnl"].sum() /
                           abs(x.loc[x["net_pnl"] < 0, "net_pnl"].sum()))
                          if (x["net_pnl"] < 0).any() else float("inf"),
                })
            ).reset_index()
            subtier_summary["pnl_contribution_%"] = (
                subtier_summary["net_pnl_usd"] / subtier_summary["net_pnl_usd"].sum() * 100
            )
            subtier_summary = subtier_summary.round({
                "winrate_pct": 2, "avg_r": 3, "net_pnl_usd": 2,
                "PF": 3, "pnl_contribution_%": 2,
            }).sort_values("net_pnl_usd", ascending=False)
            print(subtier_summary.to_string(index=False))
            subtier_summary.to_csv(outdir / "v29_3_v25idx_subtier_summary.csv", index=False)

            print("\n" + "=" * 110)
            print("  [v29] v25idx_rule 별 stat")
            print("=" * 110)
            rule_summary = tdf.groupby("v25idx_rule").apply(
                lambda x: pd.Series({
                    "n": len(x),
                    "subtier": x["v25idx_subtier"].iloc[0] if len(x) > 0 else "",
                    "winrate_pct": (x["net_pnl"] > 0).mean() * 100,
                    "avg_r": x["r_multiple"].mean(),
                    "net_pnl_usd": x["net_pnl"].sum(),
                    "PF": (x.loc[x["net_pnl"] > 0, "net_pnl"].sum() /
            ...
            rule_summary = rule_summary.round({
                "winrate_pct": 2, "avg_r": 3, "net_pnl_usd": 2, "PF": 3,
            }).sort_values("net_pnl_usd", ascending=False)
            print(rule_summary.to_string(index=False))
            rule_summary.to_csv(outdir / "v29_3_v25idx_rule_summary.csv", index=False)

        # 차트
        try:
            plot_equity_v25(res, save_path=str(outdir / "v29_3_equity_chart.png"))
        except Exception as e:
