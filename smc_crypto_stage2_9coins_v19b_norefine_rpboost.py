# =========================================================
# smc_crypto_9coins  STAGE 2: v19b_norefine_rpboost_9coins
#
# [Stage 2 변경사항 (Stage 1 대비)]
#
#   Stage 1 + BNB + ADA 추가 확장
#
#   1) 심볼 추가: BNBUSDT, ADAUSDT
#      - PHASE_A_RISK, PHASE_B_RISK 추가
#      - SLIPPAGE_LIMIT_PCT, DIRECT_ENTRY_THRESHOLD_USDT, SPLIT_MAX_TRANCHES 추가
#      - SCENARIO_MULTI.assets 에 추가
#
#   2) 실전 업데이트용 최종 버전
#      - 7코인 Stage 1 검증 완료 후 이 버전 사용
#      - BNB/ADA 추가 효과 검증 목적
#
# [Stage 1 과 동일한 로직]
#   - MIN_SCORE = 7.5 (단순 PF 필터, 교집합 아님!)
#   - REFINE 제거 (LONG_FRAC=0.10, SHORT_FRAC=0.90)
#   - RP_TIER_MULT_TABLE = {0:1.5, 1:1.5, 2:1.0, 3:1.0, 4:1.0, 5:1.0}, else=skip
#   - threshold 0.2300
...
# ★★★ 셀 3: Stage 1 백테스트 실행 ★★★
# =========================================================
OUTDIR = "smc_crypto_stage2_9coins_outputs"
import os
...
res["trades"].to_csv(f"{OUTDIR}/stage2_trades.csv", index=False)
res["equity"].to_csv(f"{OUTDIR}/stage2_equity.csv", index=False)
res["skipped"].to_csv(f"{OUTDIR}/stage2_skipped.csv", index=False)
res["phase_transitions"].to_csv(f"{OUTDIR}/stage2_phase_transitions.csv", index=False)
res["excess_log"].to_csv(f"{OUTDIR}/stage2_excess_log.csv", index=False)
monthly_report.to_csv(f"{OUTDIR}/stage2_monthly_pnl.csv", index=False)
...
if all_no_fill:
    ...
    no_fill_all.to_csv(f"{OUTDIR}/stage2_no_fill.csv", index=False)

print(f"\n💾 CSV 저장 완료: {OUTDIR}/")
print(f"\n🎯 Stage 2 백테스트 완료! (v19b_norefine_rpboost_9coins)")
print(f"   threshold: {WICK_RATIO_5_Q1_THRESHOLD}")
rp_table_str = " | ".join([f"rp{k}={v}" for k, v in sorted(RP_TIER_MULT_TABLE.items())])
print(f"   RP table: {rp_table_str} | else=skip")
print(f"   설정: 9coins (BTC+ETH+SOL+XRP+DOGE+AVAX+LINK+BNB+ADA)")
print(f"         단일 포지션, REFINE 제거, RP_BOOST 적용")
