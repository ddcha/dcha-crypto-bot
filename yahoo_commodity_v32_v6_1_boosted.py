# =========================================================
# YAHOO COMMODITY FUTURES BACKTEST — v3.2 V6.1 + DATA-DRIVEN RISK
...
OUTDIR = Path(f"yahoo_commodity_v32_v6_1_boosted_{V6_SCENARIO}{_TF_SUFFIX}_outputs")
...
print("=" * 80)
print(f"YAHOO COMMODITY v32 — V6.1 + DATA-DRIVEN ASSET MULT {V6_SCENARIO} (S/S+/A+/A/A- best-match, 14 commodities)")
print("=" * 80)
...
def simulate_portfolio_v25(candidates_packs, max_total_risk=0.1325, scenario_name="BOOST15"):
    ...
    return {
        ...
        "strategy_variant": f"v32_{V6_SCENARIO}{_TF_SUFFIX}_BOOST15",
        "scenario": scenario_name,
        "trades": trades_df,
        "skipped": skipped_df,
        "equity": equity_df,
...
if __name__ == "__main__":
    ...
    try:
        res["trades"].to_csv(outdir / f"v32_{V6_SCENARIO}{_TF_SUFFIX}_trades.csv", index=False)
        res["equity"].to_csv(outdir / f"v32_{V6_SCENARIO}{_TF_SUFFIX}_equity.csv", index=False)
        res["skipped"].to_csv(outdir / f"v32_{V6_SCENARIO}{_TF_SUFFIX}_skipped.csv", index=False)
        res["phase_transitions"].to_csv(outdir / f"v32_{V6_SCENARIO}{_TF_SUFFIX}_phase_transitions.csv", index=False)
        res["excess_log"].to_csv(outdir / f"v32_{V6_SCENARIO}{_TF_SUFFIX}_excess_log.csv", index=False)
        res["tax_log"].to_csv(outdir / f"v32_{V6_SCENARIO}{_TF_SUFFIX}_tax_log.csv", index=False)
        all_cand.to_csv(outdir / f"v32_{V6_SCENARIO}{_TF_SUFFIX}_all_candidates.csv", index=False)
        print(f"\n💾 CSV 저장 완료: {outdir}/")
    ...
    if len(tdf) == 0:
        ...
        summary_df.to_csv(outdir / f"v32_{V6_SCENARIO}{_TF_SUFFIX}_summary.csv", index=False)
        ...
        monthly_summary.to_csv(outdir / f"v32_{V6_SCENARIO}{_TF_SUFFIX}_monthly_summary.csv", index=False)
        ...
        asset_summary.to_csv(outdir / f"v32_{V6_SCENARIO}{_TF_SUFFIX}_asset_summary.csv", index=False)
        ...
        if "tier_label" in tdf.columns:
            ...
            tier_summary.to_csv(outdir / f"v32_{V6_SCENARIO}{_TF_SUFFIX}_tier_summary.csv", index=False)
        ...
        if "tier_v5_final" in tdf.columns:
            ...
            v5_summary.to_csv(outdir / f"v32_{V6_SCENARIO}{_TF_SUFFIX}_v5_tier_summary.csv", index=False)
        ...
        if "sub_tier_v6" in tdf.columns:
            ...
            v6_summary.to_csv(outdir / f"v32_{V6_SCENARIO}{_TF_SUFFIX}_v6_sub_tier_summary.csv", index=False)

        # 차트
        try:
            plot_equity_v25(res, save_path=str(outdir / f"v32_{V6_SCENARIO}{_TF_SUFFIX}_equity_chart.png"))
        except Exception as e:
