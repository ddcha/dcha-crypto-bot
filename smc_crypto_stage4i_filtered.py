# =========================================================
# smc_crypto_9coins  STAGE 4I: FILTERED (1.0× 평평 + GEM/ROOM_ONLY 차단)
#
# [배경]
#   Stage 4H Scen B (모두 1.0×) 분석 결과:
#     - SWEEP_GEM   88t : SHORT 50t (PF 2.88, R outlier 의존도 ↑) vs LONG 38t (PF 9.88) → LONG only
#     - SWEEP_ROOM_FVG 303t : LONG/SHORT 대칭 PF 2.0+, 전 심볼 PF≥1.0 → 그대로 유지 (주력 엔진)
#     - SWEEP_ROOM_ONLY 120t : 심볼별 극단 — 수익(ADA/AVAX/XRP/DOGE/ETH PF≥1.0) vs 적자(BTC/SOL/BNB/LINK PF<1.0)
#       → BTC/SOL/BNB/LINK 차단으로 50t 제거, PF 1.20 → 1.88
#   복리 미반영 단순 합산 +$27,458 (ALPHA 계열 합치면 1,679K → 1,706K)
#
#   Runner time exit 는 그대로 풀어둠 — long-hold runner (BTC R=167, AVAX R=33 등)
#   는 백테스트 종료까지 청산되지 않은 의도된 거래. 실전에서는 수동 청산.
#
# [Stage 4I 차단 규칙]
#   GEM_LONG_ONLY              = True
#   ROOM_ONLY_SYMBOL_BLACKLIST = {BTCUSDT, SOLUSDT, BNBUSDT, LINKUSDT}
#   risk_mult 단계에서 0 처리 → skipped 에 stage4i_skip_* 사유 기록
#
# [Tier 체계 (4G/4H 와 동일)]
#   ALPHA_MAX  (2.0×), ALPHA_HIGH (1.5×)
#   ALPHA_MED  (LONG 0.8, SHORT 1.2)
#   SWEEP_GEM (1.0× LONG only) / SWEEP_ROOM_FVG (1.0×) / SWEEP_ROOM_ONLY (1.0× ALLOW 5심볼)
#   COMPLETE_OUT (skip), SKIP_MSS (0)
#   RP_MULT: {0: 2.0, 1~5: 1.0}
#
# [산출물]
#   stage4i_filtered_out_skip_trades.csv 등
#   tier_stage4i_filtered_distribution.csv
#   tier_x_side_stage4i_filtered.csv
#   rp_x_tier_stage4i_filtered.csv
#   tier_x_symbol_stage4i_filtered.csv ← 신규: 심볼 분포 검증
#   stage4i_summary.csv
# =========================================================
#
# (아래 주석은 4G 원본 — 참고용 유지)
...
# =========================================================
# ★★★ Stage 4I: FILTERED (1.0× 평평) ★★★
# =========================================================
# Stage 4H Scen B 분석 결과 반영 — 신규 3 SWEEP 필터에 추가 차단:
#   1) SWEEP_GEM    : LONG only (SHORT 50t 제거 — outlier 의존성 ↓)
#   2) SWEEP_ROOM_FVG : 전체 유지 (PF 2.17 건전한 주력 엔진)
#   3) SWEEP_ROOM_ONLY: BTC/SOL/BNB/LINK 심볼 BLOCK (50t 적자 제거)
# 리스크 배수: 모두 1.0× (= Scen B 평평)
# Runner time exit: 그대로 풀어둠 (수동 청산 의도)

SCENARIO = "FILTERED"   # Stage 4I 단일 시나리오
...
# Stage 4I: 단일 시나리오 (모두 1.0×) — 분석 결과 1.5× 부스터는
# outlier 의존성이 너무 큼 (BTC R=167 단건 의존). 평평 1.0× 로 baseline.
    ...
    for current_time in event_times:
        ...
        if current_time in entry_map:
            for idx, row in entry_map[current_time]:
                ...
                if tier_mult_4e <= 0.0:
                    # Stage 4I 차단 사유 분리
                    if tier_4e == "SWEEP_GEM" and GEM_LONG_ONLY and _side_4g == "short":
...
print()
print("=" * 75)
print("[Stage 4I] candidates 생성 — SWEEP 세분화 + 필터 차단")
print("=" * 75)
...
print(f"  SWEEP_RISK (Stage 4I 단일 시나리오, 1.0× 평평):")
for sc, mult in SWEEP_RISK_BY_SCENARIO.items():
    print(f"    {sc}: {mult}")
print(f"  ── Stage 4I 차단 규칙 ──")
...
try:
    candidates_results_stage4e = _parallel_map(
        ...
        task_name="candidates Stage 4I", io_bound=False,
    )
...
print(f"\n  TOTAL candidates (Stage 4I): {total_stage4e}")
print("\n[OK] candidates 생성 완료 (Stage 4I). 단일 시나리오로 simulate.")
...
# =========================================================
# ★★★ 셀 3: Stage 4I — 단일 시나리오 백테스트 (FILTERED) ★★★
# =========================================================
import os
OUTDIR = "stage4i_outputs"
os.makedirs(OUTDIR, exist_ok=True)
print(f"\n[OUTDIR] {os.path.abspath(OUTDIR)}")
print(f"[CONFIG] OUT_BEHAVIOR = {OUT_BEHAVIOR!r}")
...
SCENARIO_DESCRIPTIONS = {
    "FILTERED": "Stage 4I FILTERED (모두 1.0× + GEM LONG only + ROOM_ONLY 심볼블랙)",
}

# 결과 누적 (단일 키지만 구조 유지)
scenario_results = {}

SCENARIO_CURRENT = "FILTERED"
SCENARIO = SCENARIO_CURRENT
globals()["SCENARIO"] = SCENARIO_CURRENT

print()
print("#" * 80)
print(f"# Stage 4I {SCENARIO_CURRENT}: {SCENARIO_DESCRIPTIONS[SCENARIO_CURRENT]}")
print(f"#   SWEEP_GEM       × {SWEEP_RISK_BY_SCENARIO[SCENARIO_CURRENT]['SWEEP_GEM']}  (LONG only)")
print(f"#   SWEEP_ROOM_FVG  × {SWEEP_RISK_BY_SCENARIO[SCENARIO_CURRENT]['SWEEP_ROOM_FVG']}  (전체 유지)")
print(f"#   SWEEP_ROOM_ONLY × {SWEEP_RISK_BY_SCENARIO[SCENARIO_CURRENT]['SWEEP_ROOM_ONLY']}  (BLOCK: BTC/SOL/BNB/LINK)")
print("#" * 80)

STAGE4I_CASE = {
    "name":  f"stage4i_{SCENARIO_CURRENT.lower()}",
    "label": f"Stage 4I [{SCENARIO_CURRENT}]: {SCENARIO_DESCRIPTIONS[SCENARIO_CURRENT]}",
    "risk_mult": 1.0,
...
res = simulate_scenario_v19b_rpboost(
    ...
    candidates_dict=STAGE4I_CASE["candidates"],
    risk_multiplier=STAGE4I_CASE["risk_mult"],
)

monthly_report = print_scenario_summary(res)

prefix = f"stage4i_{SCENARIO_CURRENT.lower()}_out_{OUT_BEHAVIOR}"
...
print()
print("=" * 75)
print(f"[Stage 4I {SCENARIO_CURRENT}] 전체 성과")
print("=" * 75)
...
if len(tdf) > 0 and "tier_4e" in tdf.columns:
    ...
    pd.DataFrame(tier_rows).round(4).to_csv(
        f"{OUTDIR}/tier_stage4i_{SCENARIO_CURRENT.lower()}_distribution.csv", index=False)
    ...
    pd.DataFrame(tx_rows).round(4).to_csv(
        f"{OUTDIR}/tier_x_side_stage4i_{SCENARIO_CURRENT.lower()}.csv", index=False)
    ...
    pd.DataFrame(rt_rows).round(4).to_csv(
        f"{OUTDIR}/rp_x_tier_stage4i_{SCENARIO_CURRENT.lower()}.csv", index=False)
    ...
    pd.DataFrame(ts_rows).round(4).to_csv(
        f"{OUTDIR}/tier_x_symbol_stage4i_{SCENARIO_CURRENT.lower()}.csv", index=False)
    
    # [5] Stage 4I 신규: 차단된 거래 사유 분포
    print()
    print("-" * 100)
    print(f"[5] Skipped 거래 사유 분포 (Stage 4I 차단 효과)")
    print("-" * 100)
    sk = res["skipped"]
    if len(sk) > 0 and "reason" in sk.columns:
        ...
        print(f"\n  ── Stage 4I 신규 차단: {len(stage4i_blocks)}건 ──")
...
print()
print("#" * 85)
print("# Stage 4I — FILTERED 결과 요약")
print("#" * 85)

comp_df = pd.DataFrame([scenario_results[SCENARIO_CURRENT]])
comp_df.to_csv(f"{OUTDIR}/stage4i_summary.csv", index=False)
...
print()
print("-" * 100)
print("Stage 4I 차단 규칙:")
print(f"  GEM_LONG_ONLY              = {GEM_LONG_ONLY}")
print(f"  ROOM_ONLY_SYMBOL_BLACKLIST = {sorted(ROOM_ONLY_SYMBOL_BLACKLIST)}")
print(f"  SWEEP risk multipliers     = all 1.0× (평평)")

print()
print("=" * 75)
print("[Stage 4I FILTERED] 백테스트 완료")
print("=" * 75)
print(f"  산출물: {OUTDIR}/")
print(f"    stage4i_filtered_out_skip_*.csv  (trades/equity/monthly/summary/skipped)")
print(f"    tier_stage4i_filtered_distribution.csv")
print(f"    tier_x_side_stage4i_filtered.csv")
print(f"    rp_x_tier_stage4i_filtered.csv")
print(f"    tier_x_symbol_stage4i_filtered.csv  ← 신규: 심볼 분포 검증용")
print(f"    stage4i_summary.csv")
