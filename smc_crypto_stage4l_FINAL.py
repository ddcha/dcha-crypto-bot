# =========================================================
# ★★★ smc_crypto_stage4l_FINAL.py ★★★
#
# FINAL FIX: ALPHA_MAX tier_mult 4.0 + REVIVE_CUTS=ON + BOOST15 only
#
# 기준선 (2023-07 ~ 2026-04, 9 coins, 1310 trades):
#   PF       2.997
#   Win%     61.91%
#   avg_R    0.359
#   MDD      -11.155%
#   Return   25,275%
#   Peak     $2.47M (33.4억 KRW)
#   퇴사     2024-01 (7개월)
#   Calmar   2,266 (최적점)
#
# 환경변수로 override 가능:
#   ALPHA_MAX_TIER_MULT=5.0   # ALPHA_MAX risk multiplier 변경
#   REVIVE_CUTS=0             # CUT 라벨 부활 비활성화
#   SCENARIOS_OVERRIDE=BOOST15,BOOST25,BOOST35  # 3 시나리오 비교
#   OUTDIR_OVERRIDE=my_test   # 결과 폴더 변경
#   RISK_MULT_OVERRIDE=1.5    # 모든 trade 에 전역 risk mult
#
# 산출물 (OUTDIR=stage4l_FINAL_outputs):
#   stage4l_boost15_out_skip_trades.csv      — 1310 trades 전체 상세
#   stage4l_boost15_out_skip_equity.csv      — 시간별 자본 곡선
#   stage4l_boost15_out_skip_monthly.csv     — 월별 PnL/winrate
#   stage4l_boost15_out_skip_skipped.csv     — 차단된 trade 686건
#   stage4l_boost15_out_skip_overall_summary.csv
#   tier_stage4l_boost15_distribution.csv    — tier 별 분포
#   sentiment_distribution_stage4l_boost15.csv
#   stage4l_comparison_summary.csv
#
# VS Code 실행: F5 또는 우상단 ▶ 버튼
# CLI 실행:    python smc_crypto_stage4l_FINAL.py
# =========================================================
#
# 원본: smc_crypto_stage4l_redist_h1choch_v3_conservative_mtf_fill.py
# 분석 기능은 원본과 100% 동일 (trades/equity/monthly/skipped/tier/sentiment CSV 모두 산출)
#
# =========================================================
...
# ★★★ [PATCHED — H1 bar-by-bar fill, 2026-05-28] ★★★
# H1 봉을 하나씩 시간순으로 forward 하며 SL/TP 를 현실적으로 판정.
# 진입 신호는 H4 그대로, fill(체결/SL/TP/runner/trailing) 만 H1 단위.
# 미래 참조 없음 (entry 이후 봉만 순차 진행).
#
# H4→H1 전환 시 "봉 수"에 의존하는 파라미터는 ×4 해야 시간 의미가 보존됨:
#   max_hold_bars  : 호출부에서 ×4 (기존 코드 존재)
#   FAST_2R_BARS_MAX, ABOVE_2R_BARS_MIN : 아래 H1 전용 상수로 ×4
# RR 기반 상수(RUNNER_PROXY_MAX_RR, MAX_RR_AFTER_2R_MIN 등)는 봉 무관 → 그대로.
H1_BARS_PER_H4 = 4

# ★★★ [PATCHED — Multi-TF fill (1h/15m 일반화), 2026-05-28] ★★★
# fill 판정 TF 를 선택. 진입 신호는 H4 고정, fill(SL/TP/runner/trailing)만 이 TF.
#   FILL_TF="1h"  → H4의 1/4   (BARS_PER_H4=4)
#   FILL_TF="15m" → H4의 1/16  (BARS_PER_H4=16)
#   FILL_TF="4h"  → H4와 동일  (BARS_PER_H4=1, 사실상 기존 H4 fill)
# 봉 수 의존 파라미터(FAST_2R/ABOVE_2R/max_hold)는 BARS_PER_H4 배수로 자동 환산.
FILL_TF = "15m"   # 1h | 15m | 4h
_FILL_TF_TO_BARS = {"4h": 1, "1h": 4, "15m": 16, "5m": 48}
BARS_PER_H4 = _FILL_TF_TO_BARS.get(FILL_TF, 4)
FAST_2R_BARS_MAX_FILL = FAST_2R_BARS_MAX * BARS_PER_H4    # 1h:12, 15m:48
ABOVE_2R_BARS_MIN_FILL = ABOVE_2R_BARS_MIN * BARS_PER_H4  # 1h:12, 15m:48
print(f"[MTF-FILL] FILL_TF={FILL_TF}  BARS_PER_H4={BARS_PER_H4}  "
      f"(FAST_2R={FAST_2R_BARS_MAX_FILL}, ABOVE_2R={ABOVE_2R_BARS_MIN_FILL})")

# 하위호환: 기존 H1 전용 상수명 유지 (다른 곳에서 참조할 경우)
FAST_2R_BARS_MAX_H1 = FAST_2R_BARS_MAX_FILL
ABOVE_2R_BARS_MIN_H1 = ABOVE_2R_BARS_MIN_FILL

# 봉 내 TP/SL 동시 터치 시 처리:
#   "heuristic" : open/close 로 양봉/음봉 판단해 순서 추정 (기존 H4 방식)
#   "pessimistic": 항상 SL 먼저 맞은 것으로 가정 (가장 보수적, 실전 하한)
# H1 으로 쪼개면 동시 터치 케이스 자체가 크게 줄지만, 남은 것 처리 방식.
H1_FILL_TIE_BREAK = _os.environ.get("H1_FILL_TIE_BREAK", "heuristic")  # heuristic | pessimistic
...
TIER_RISK_MULT_COMMON = {
    # ★★★ FINAL FIX ★★★ ALPHA_MAX 2.5 → 4.0
    # sweep 결과 (REVIVE_CUTS=1 + BOOST15 baseline):
    #   2.5: MDD -11.155%, Return 19,443%, peak $1.91M (25.7억) ← original
    #   3.5: MDD -11.155%, Return 23,612%, peak $2.31M (31.2억)
    #   4.0: MDD -11.155%, Return 25,275%, peak $2.47M (33.4억) ← 채택 (Calmar 최고 2,266)
    #   5.0: MDD -12.557%, Return 28,142%, peak $2.75M (37.2억) — 마지노선 초과
    # → 4.0 이 MDD 마지노선(-11.15%) 정확히 유지하는 한계점
    # env ALPHA_MAX_TIER_MULT 로 override 가능
    "ALPHA_MAX":    float(_os.environ.get("ALPHA_MAX_TIER_MULT", "4.0")),
...
# ★★★ FINAL FIX ★★★ REVIVE_CUTS default ON
# CUT_NeutFalling_S (n=42, Win 71%, PF 5.25) 와 CUT_ShortStable_L_DISABLED (n=10, Win 80%, PF 16.4)
# 실 데이터 분석 결과 "CUT" 가정과 정반대로 우수 → BOOST 로 재분류 (BOOSTER mult 적용)
# → 단독 효과 +$198k (2.67억원), MDD 추가 0%
# env REVIVE_CUTS=0 으로 비활성화 가능
_REVIVE_CUTS = _os.environ.get("REVIVE_CUTS", "1") == "1"
if _REVIVE_CUTS:
    print("[REVIVE_CUTS=ON] CUT_NeutFalling_S, CUT_ShortStable_L_DISABLED → BOOST 로 재분류 (booster mult 적용)")
else:
    print("[REVIVE_CUTS=OFF] CUT 라벨들 원래대로 0.5× 적용")
...
def get_stage4k_sentiment_mult(target_time, side, executed_trades_so_far, scenario):
    ...
    # ── 약점 3 CUT ──
    # env REVIVE_CUTS=1 일 때 NeutFalling_S, ShortStable_L 을 BOOST 로 재분류
    #   근거 (Stage 4L v3 conservative 백테스트, 1310 trades):
    #     CUT_NeutFalling_S         : n=42  Win 71.4%  PF 5.25  avg_R 0.39  → cut인데 우수
    #     CUT_ShortStable_L_DISABLED: n=10  Win 80%   PF 16.4  avg_R 0.49  → disabled 분류인데 폭발
...
#   Stage 4L: LONG 0.6 / SHORT 1.0  → 평균 0.8, side-aware 비율 유지
# ⚠ [v3 PATCH ③] ALPHA_MED SHORT risk × 0.3 (강한 cut)
#   분석 근거 (h1choch BOOST25 trades):
#     - IS  (~2024-12): n=45, Win 48.9%, PF 0.78, pnl -$6,178
#     - OOS (2025+):    n=47, Win 36.2%, PF 0.81, pnl -$13,202
#     - IS/OOS 둘 다 PF<1, 12분기 중 PF>1.5 인 분기는 4개뿐 → 일관된 약점
#     - stop loss 55건 -$41K, time exit 37건 +$22K → 순 -$19K
#   대안 비교:
#     - risk × 0.3: IS pnl -$6K → -$2K, OOS pnl -$13K → -$4K (총 +$13K 회복)
#     - 진입 차단:  총 +$19K 회복 (정보 잃음)
#   → risk × 0.3 채택 (보수적 + 진입 정보 유지)
ALPHA_MED_RISK_LONG  = 0.6   # ★ 4K 0.8 → 0.6 (Stage 4L)
ALPHA_MED_RISK_SHORT = 0.3   # ⚠ v3 patch: was 1.0 (IS/OOS 둘 다 PF<1)
...
# ⭐⭐⭐ Option 2B: Multi-TF bar-by-bar Simulator ⭐⭐⭐
# trade simulator를 fill TF(1h/15m) 봉 단위로 호출 (SL/TP/runner 모두 해당 TF wick 기준)
# FILL_TF != "4h" 이면 자동 활성. USE_H1_FILL=0 으로 강제 OFF(H4) 가능.
# generate_candidates와 execute_and_resimulate_trade 둘 다 영향
_force_h4 = _os.environ.get("USE_H1_FILL", "auto") == "0"
USE_H1_BAR_BY_BAR_SIMULATOR = (FILL_TF != "4h") and (not _force_h4)
print(f"[MTF-FILL] bar-by-bar simulator = {USE_H1_BAR_BY_BAR_SIMULATOR}  "
      f"(FILL_TF={FILL_TF}, tie_break={H1_FILL_TIE_BREAK})")
...
PATTERN_MULTS = {
    ...
    # ✓ [v3 RESTORED] strong_T2aligned_h1comp_short = 1.5 복원
    #   - v2 에서 1.0 으로 중립화했으나 OOS 도 PF 18.78 (n=9) 로 강하게 검증됨
    #   - n=16(IS)/n=9(OOS) 작아도 IS PF 19, OOS PF 19 일관 → 우연 아닌 진짜 알파
    #   - v2 에서 1.0 적용 시 OOS pnl -$187K 손실 발생 → v3 에서 복원
    "strong_T2aligned_h1comp_short": 1.5,   # ✓ v3 restored from v2 neutralization
}
...
_DATA_CACHE_DIR = _os.path.join(_os.path.dirname(_os.path.abspath(__file__)), "data_cache")
_os.makedirs(_DATA_CACHE_DIR, exist_ok=True)


def _download_data_uncached(symbol="BTCUSDT", interval="4h"):
    frames = []
...
def download_data(symbol="BTCUSDT", interval="4h"):
    """parquet 캐시 우선 사용. 캐시 있으면 그대로 반환(증분 다운로드 X)."""
    cache_fp = _os.path.join(_DATA_CACHE_DIR, f"{symbol}_{interval}.parquet")
    if _os.path.exists(cache_fp):
        try:
            df_cache = pd.read_parquet(cache_fp)
            if "timestamp" in df_cache.columns and len(df_cache) > 0:
                if df_cache["timestamp"].dt.tz is None:
                    df_cache["timestamp"] = df_cache["timestamp"].dt.tz_localize("UTC")
                else:
                    df_cache["timestamp"] = df_cache["timestamp"].dt.tz_convert("UTC")
                df_cache = df_cache.sort_values("timestamp").reset_index(drop=True)
                print(f"[cache] {symbol} {interval}: {len(df_cache)} rows  "
                      f"[{df_cache['timestamp'].iloc[0]} ~ {df_cache['timestamp'].iloc[-1]}]")
                return df_cache
        except Exception as e:
            print(f"[cache READ FAIL] {symbol} {interval}: {e} — falling back to download")
    out = _download_data_uncached(symbol=symbol, interval=interval)
    try:
        out.to_parquet(cache_fp, index=False)
        print(f"[cache WRITE] {symbol} {interval}: {len(out)} rows -> {cache_fp}")
    except Exception as e:
        print(f"[cache WRITE FAIL] {symbol} {interval}: {e}")
    return out
...
def _simulate_trade_with_plan_core(df_local, entry_idx, side, entry, sl, qty, fee_rate, grade, plan,
    ...
    # ★ [PATCHED MTF-fill] fill 모드면 "봉 수" 의존 runner 조건을 BARS_PER_H4 배수로.
    # (max_hold_bars 는 호출부에서 이미 ×BARS_PER_H4 되어 plan 으로 들어옴)
    _fast_2r_max = FAST_2R_BARS_MAX_FILL if h1_mode else FAST_2R_BARS_MAX
    _above_2r_min = ABOVE_2R_BARS_MIN_FILL if h1_mode else ABOVE_2R_BARS_MIN
    ...
    def register_rr_stats(j, favorable_rr):
        ...
        cond_fast_2r = (pd.notna(bars_to_2r) and bars_to_2r <= _fast_2r_max)
        cond_hold_above_2r = (bars_spent_above_2r >= _above_2r_min)
    ...
    for j in range(entry_idx + 1, loop_end):
        ...
        # ★ [PATCHED H1-fill] tie-break:
        #   heuristic  : 양봉/음봉으로 봉 내 가격 경로 추정 (기존)
        #   pessimistic: 항상 불리한 쪽(SL 방향)을 먼저 평가 → 가장 보수적
        if h1_mode and H1_FILL_TIE_BREAK == "pessimistic":
            if side == "long":
                event_order = ["low", "high"]   # low=SL 방향 먼저
            else:
                event_order = ["high", "low"]   # high=SL 방향 먼저
        elif side == "long":
...
def simulate_trade_with_plan_original(df_local, entry_idx, side, entry, sl, qty, fee_rate, grade, plan,
                                       use_seq_runner_protection=False, h1_mode=False):
...
def download_symbol_data(symbol):
    ...
    # ★ [PATCHED MTF-fill] fill 전용 TF 데이터 준비
    #   FILL_TF=="1h"  → df_h1 재사용 (추가 다운로드 X)
    #   FILL_TF=="15m" → 15m 별도 다운로드
    #   FILL_TF=="4h"  → df(H4) 재사용 (fill 비활성 시)
    if FILL_TF == "1h":
        df_fill = df_h1.copy()
    elif FILL_TF == "4h":
        df_fill = df.copy()
    else:
        df_fill = download_data(symbol=symbol, interval=FILL_TF)
        df_fill = df_fill[df_fill["timestamp"] >= cutoff].reset_index(drop=True)
    print(f"{symbol} H4: {len(df)} rows, H1: {len(df_h1)} rows, FILL({FILL_TF}): {len(df_fill)} rows")
    return {"symbol": symbol, "df_raw": df, "df_h1_raw": df_h1, "df_fill_raw": df_fill}
...
def apply_indicators_and_build(raw):
    ...
    # ★ [PATCHED MTF-fill] fill 전용 df 준비 (trailing stop 이 atr 사용하므로 basic indicators 필요)
    #   FILL_TF=="1h" 면 df_h1 재사용 (이미 지표 적용됨)
    #   FILL_TF=="4h" 면 df_struct 재사용
    #   그 외(15m 등)는 별도 df_fill 에 basic indicators 적용
    if FILL_TF == "1h":
        df_fill = df_h1   # 이미 모든 지표 적용된 H1
    elif FILL_TF == "4h":
        df_fill = df_struct
    else:
        df_fill = raw.get("df_fill_raw")
        if df_fill is not None:
            df_fill = df_fill.copy()
            df_fill = apply_basic_indicators(df_fill)   # atr 등 (trailing 에 필요)
        else:
            df_fill = df_h1  # fallback

    df_struct, structures, structures_by_zone_created = build_structures(df)
    df_d1 = build_d1_trend(df_struct)

    print(f"{symbol} total structures: {len(structures)} | D1 bars: {len(df_d1)} | fill_rows({FILL_TF}): {len(df_fill)}")
...
def generate_candidates_from_prepared(prepared):
    ...
    df_fill = prepared.get("df_fill", df_h1)   # ★ [MTF-fill] fill 전용 TF (없으면 H1)
    ...
    # ⭐ [LEAK-1 진단] same-bar entry 카운터 (print-only, 로직 영향 0)
    #   확정 거래에서 진입봉 i 가 zone 생성봉과 같은지 집계.
    #   전역 대신 return dict 로 부모에 전달 (loky 멀티프로세스 안전).
    _leak1_same = 0
    _leak1_total = 0
    _leak1_dist = {}

    # ⭐ [패치①] env 토글 — PATCH1=1 이면 zone 생성봉 당봉 진입 금지 (off-by-one 누수 제거)
    import os as _os_p1
    _PATCH1_ON = _os_p1.environ.get("PATCH1") == "1"
    ...
    while i < len(df_struct) - 1:
        ...
        for s, eff_score, fresh_reasons in scored_active:
            if _PATCH1_ON and i <= s["zone_created_idx"]:  # ⭐ [패치①] off-by-one: zone 생성봉 당봉 진입 금지 (env PATCH1=1)
                continue
        ...
        # ⭐⭐⭐ Multi-TF bar-by-bar simulator (1h/15m) ⭐⭐⭐
        if USE_H1_BAR_BY_BAR_SIMULATOR:
            _h1_entry_idx = find_h1_entry_idx(df_fill, row["timestamp"])
            if _h1_entry_idx is not None:
                # max_hold_bars × BARS_PER_H4 (H4 → fill TF 단위 변환)
                _plan_h1 = {**tp_plan, "max_hold_bars": tp_plan["max_hold_bars"] * BARS_PER_H4}
                sim = simulate_trade_with_plan_original(
                    df_local=df_fill, entry_idx=_h1_entry_idx, side=entry_side,
                ...
                # cooldown 비교용 — fill exit_idx → H4 인덱스 환산 (i와 같은 단위로)
                _sim_exit_idx_h4 = i + max(1, (sim["exit_idx"] - _h1_entry_idx) // BARS_PER_H4)
            else:
                # fallback to H4
                sim = simulate_trade_with_plan_original(
        ...
        # ⭐ [LEAK-1 진단] 확정 거래 1건 — 진입봉 i vs zone 생성봉 거리
        _leak1_total += 1
        _d_leak1 = i - used_structure["zone_created_idx"]
        if _d_leak1 == 0:
            _leak1_same += 1
        _leak1_dist[_d_leak1] = _leak1_dist.get(_d_leak1, 0) + 1
        ...
        # ★ [PATCHED H1-fill] H1 모드면 sim["exit_idx"]가 H1 인덱스이므로
        # H4 메인 루프 i 진행에는 환산된 _sim_exit_idx_h4 를 사용해야 함.
        # (H4 모드면 _sim_exit_idx_h4 == sim["exit_idx"] 라 동작 동일)
        i = _sim_exit_idx_h4 + 1
    ...
    return {
        ...
        "df_h4": df_struct,
        "df_h1": df_h1,
        "df_fill": df_fill,            # ★ [MTF-fill] fill 전용 TF (1h/15m)
        "df_d1": df_d1,                # ⭐ Step 1 모니터링: simulate 시 D1 ema_state/slope lookup용
        "candidates": candidates_df,
        "no_fill": no_fill_df,
        # ⭐ [LEAK-1 진단] 부모 프로세스 집계용
        "_leak1_same": _leak1_same,
        "_leak1_total": _leak1_total,
        "_leak1_dist": _leak1_dist,
    }
...
def execute_and_resimulate_trade(row, qty, df_h4, df_h1=None, df_fill=None):
    ...
    # ⭐⭐⭐ Multi-TF bar-by-bar simulator (1h/15m) ⭐⭐⭐
    # df_fill 우선 사용 (없으면 df_h1 로 폴백 — 하위호환)
    _df_fill = df_fill if df_fill is not None else df_h1
    _tp_plan = get_tp_plan(row["expansion_state"])
    if USE_H1_BAR_BY_BAR_SIMULATOR and _df_fill is not None:
        _h1_entry_idx = find_h1_entry_idx(_df_fill, row["entry_time"])
        if _h1_entry_idx is not None:
            _plan_h1 = {**_tp_plan, "max_hold_bars": _tp_plan["max_hold_bars"] * BARS_PER_H4}
            sim = simulate_trade_with_plan_runner_no_time_exit(
                df_local=_df_fill, entry_idx=_h1_entry_idx, side=row["side"],
        ...
        if current_time in entry_map:
            for idx, row in entry_map[current_time]:
                ...
                exec_adj = execute_and_resimulate_trade(
                    ...
                    df_fill=candidates_dict[symbol].get("df_fill"),
                )
...
# =========================================================
# ⭐⭐ [Phase 1-1 진단] LEAK-1 + ZONE-PARAM 집계 출력 (print-only)
# =========================================================
print("\n" + "=" * 75)
print("[PHASE 1-1 진단]")
print("=" * 75)
print(f"[ZONE-PARAM] MAX_AGE={H4_ZONE_MAX_AGE} MSS={H4_MSS_LOOKBACK} "
      f"OB={H4_OB_LOOKBACK} PD={H4_PD_LOOKBACK}")
_leak1_same_all = 0
_leak1_total_all = 0
_leak1_dist_all = {}
for _s in SYMBOLS_TO_PREPARE:
    _r = candidates_dict_stage4e[_s]
    _leak1_same_all += _r.get("_leak1_same", 0)
    _leak1_total_all += _r.get("_leak1_total", 0)
    for _k, _v in _r.get("_leak1_dist", {}).items():
        _leak1_dist_all[_k] = _leak1_dist_all.get(_k, 0) + _v
_pct_leak1 = 100 * _leak1_same_all / max(_leak1_total_all, 1)
print(f"[LEAK-1] same-bar entry: {_leak1_same_all}/{_leak1_total_all} = {_pct_leak1:.2f}%")
print("[LEAK-1-DIST] (i - zone_created_idx) -> count:")
for _k in sorted(_leak1_dist_all.keys())[:15]:
    print(f"    Δ={_k:>3}봉: {_leak1_dist_all[_k]}")
print("=" * 75)

# ⭐⭐ [Phase 1-1 측정] 후보 덤프 (before/after 대조용) + 선택적 조기종료
import os as _os_diag, sys as _sys_diag
_run_tag = _os_diag.environ.get("RUN_TAG", "run")
_dump_rows = []
for _s in SYMBOLS_TO_PREPARE:
    _cdf = candidates_dict_stage4e[_s]["candidates"]
    if len(_cdf) == 0:
        continue
    _sub = _cdf[["zone_created_idx", "entry_idx", "side", "entry", "sl",
                 "score", "r_multiple", "result", "exit_reason", "hold_bars"]].copy()
    _sub.insert(0, "symbol", _s)
    _sub["delta"] = _sub["entry_idx"] - _sub["zone_created_idx"]
    _dump_rows.append(_sub)
if _dump_rows:
    _dump_df = pd.concat(_dump_rows, ignore_index=True)
    _dump_fp = f"diag_candidates_{_run_tag}.parquet"
    _dump_df.to_parquet(_dump_fp, index=False)
    _rpos = _dump_df.loc[_dump_df["r_multiple"] > 0, "r_multiple"].sum()
    _rneg = _dump_df.loc[_dump_df["r_multiple"] < 0, "r_multiple"].sum()
    _wr = 100.0 * (_dump_df["r_multiple"] > 0).mean()
    _pf = (_rpos / abs(_rneg)) if _rneg != 0 else float("inf")
    print(f"[DUMP] {_dump_fp}: n_trades={len(_dump_df)} "
          f"R-PF={_pf:.3f} winrate={_wr:.1f}% sumR={_dump_df['r_multiple'].sum():.1f}")
if _os_diag.environ.get("DIAG_STOP_AFTER_CANDIDATES") == "1":
    print("[DIAG] STOP_AFTER_CANDIDATES=1 — simulate 단계 스킵하고 종료")
    _sys_diag.exit(0)
...
import os
# ★★★ FINAL FIX ★★★ 결과 폴더 분리
OUTDIR = _os.environ.get("OUTDIR_OVERRIDE", "stage4l_FINAL_outputs")
os.makedirs(OUTDIR, exist_ok=True)
print(f"\n[OUTDIR] {os.path.abspath(OUTDIR)}")
print(f"[CONFIG] OUT_BEHAVIOR = {OUT_BEHAVIOR!r}")
print(f"[V3 CONSERVATIVE PATCHES]")
print(f"  ① CUT_ShortStable_L → DISABLED (kept from v2 — IS 0.47 vs OOS 12.4 flip 확인)")
print(f"  ② strong_T2aligned_h1comp_short → 1.5× RESTORED (v2 에서 1.0 했으나 OOS PF 18.78 검증)")
print(f"  ③ ALPHA_MED_RISK_SHORT: 1.0 → 0.3 (IS/OOS 모두 PF<1, 12분기 중 PF>1.5 분기 4개뿐)")
...
# ★★★ FINAL FIX ★★★ BOOST15 만 기본 실행 (AM4.0 + REVIVE_CUTS 와 매칭되는 best 조합)
# 3 시나리오 다 비교하려면: SCENARIOS_OVERRIDE=BOOST15,BOOST25,BOOST35 환경변수 또는 아래 줄 수정
_scenarios_env = _os.environ.get("SCENARIOS_OVERRIDE", "").strip()
if _scenarios_env:
    SCENARIOS_LIST = [s.strip() for s in _scenarios_env.split(",") if s.strip()]
    print(f"[SCENARIOS OVERRIDE] {SCENARIOS_LIST}")
else:
    SCENARIOS_LIST = ["BOOST15"]   # ★ FINAL: BOOST15 만 (sentiment mult 1.5/0.5)
    print(f"[SCENARIOS] {SCENARIOS_LIST}  (전체 비교는 SCENARIOS_OVERRIDE=BOOST15,BOOST25,BOOST35)")
scenario_results = {}
...
for SCENARIO_CURRENT in SCENARIOS_LIST:
    ...
    STAGE4K_CASE = {
        ...
        "risk_mult": float(_os.environ.get("RISK_MULT_OVERRIDE", "1.0")),
        "candidates": candidates_dict_stage4e,
    }
    if STAGE4K_CASE["risk_mult"] != 1.0:
        print(f"[RISK_MULT OVERRIDE] risk_mult = {STAGE4K_CASE['risk_mult']}")
