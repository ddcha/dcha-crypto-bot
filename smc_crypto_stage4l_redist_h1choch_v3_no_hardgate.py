        ...
        for s, eff_score, fresh_reasons in scored_active:
            ...
            if s["type"] == "short":
                if i - last_short_exit_idx < SAME_SIDE_COOLDOWN_BARS:
                    continue
                # ⭐ [NO-HARDGATE 실험 — 2026-05-22] trend=up 차단 제거
                #   원래: SHORT 시 trend=up 이면 continue (hard gate 차단)
                #   제거 이유: trend 차단 효과 정량 측정 위해
            ...
            if s["type"] == "long":
                if i - last_long_exit_idx < SAME_SIDE_COOLDOWN_BARS:
                    continue
                # ⭐ [NO-HARDGATE 실험 — 2026-05-22] trend=down 차단 제거
                #   원래: LONG 시 trend=down 이면 continue (hard gate 차단)
                #   제거 이유: trend 차단 효과 정량 측정 위해
...
# =========================================================
import os
# ⭐ [V3 NO-HARDGATE 실험 — 2026-05-22] 결과를 별도 폴더에 저장
OUTDIR = "stage4l_redist_outputs_v3_no_hardgate"
os.makedirs(OUTDIR, exist_ok=True)
print(f"\n[OUTDIR] {os.path.abspath(OUTDIR)}")
print(f"[CONFIG] OUT_BEHAVIOR = {OUT_BEHAVIOR!r}")
print(f"[V3 PATCHES — kept]")
print(f"  ① CUT_ShortStable_L → DISABLED")
print(f"  ② strong_T2aligned_h1comp_short → 1.5× (restored)")
print(f"  ③ ALPHA_MED_RISK_SHORT: 1.0 → 0.3")
print(f"[V3 NO-HARDGATE — 실험적 변경]")
print(f"  ④ Hard gate REMOVED (line 3276-3277, 3362-3363)")
print(f"     - 원래: SHORT 시 trend=up 이면 차단, LONG 시 trend=down 이면 차단")
print(f"     - 제거 후: 모든 trend 에서 양방향 진입 가능")
print(f"     - 목적: trend 차단 효과 정량 측정")
