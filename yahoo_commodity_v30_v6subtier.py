# =========================================================
# YAHOO COMMODITY FUTURES BACKTEST — v3.0 V6 SUB-TIER (FLAT 1.0)
#
# [v30 변경점 (vs v29)]
#   ★★★ V6 SUB-TIER 분류 도입 (S, S+, A+, A, A-, B+, B, B-, X-block, X) ★★★
#       - 각 rule 의 raw stat 으로 sub-tier 자동 부여
#       - 거래의 매치 rule 들 중 가장 강한 sub-tier 선택 (best-match)
#       - V5 의 hierarchical 한계 보완 (B rule 의 raw alpha 회복)
#
#   ★★★ Risk multiplier 모두 1.0 통일 (분류 검증용) ★★★
#       - sub-tier 분류 자체의 가치 평가
#       - X-block / X 만 SKIP, 나머지 모두 동일 risk
#
# [V6 RULE → SUB-TIER 매핑 (raw stat 기반 자동)]
#   S    : V5 S rule 매치 (그대로)
#   S+   : A7 (trend_align & d_cond & high_sweep_vol) - Win 87.8%, AvgR 1.06
#   A+   : A1, A2, A3, A4, A5, A8, A9, A10, A11, A12 (Win 80%+ raw)
#   A    : B2 (wick_q1 & rp_eq_3), B8 (sweep & d_cond & rp_eq_2)
#   A-   : A6 (volume & trend_align & v19b_match_A),
#          B1, B3, B4, B5, B6, B7, B9, B10
#   X    : 어느 rule 도 매치 안됨 (= V5 의 X)
# =========================================================
#
# =========================================================
...
# Rule → Sub-tier 매핑 (commodity v29 백테 trades.csv 분석 결과 기반 자동 분류)
# index 1-based: A_RULES[0] → "A1", B_RULES[0] → "B1"
V6_RULE_TO_SUBTIER = {
    # S rules (V5 S 그대로)
    "S1": "S", "S2": "S",
    # A rules
    ...
    # B rules - 사실 raw 에선 모두 A- 급
    "B1":  "A-",   # volume                         Win 68.3%, AvgR +0.51
    "B2":  "A",    # wick_q1 & rp_eq_3              Win 71.3%, AvgR +0.52, PF 6.31
    "B3":  "A-",   # match_S                        Win 69.2%, AvgR +0.51, PF 6.64
    "B4":  "A-",   # trend & fvg & rp_eq_2 & match_C Win 69%, AvgR +0.41
    "B5":  "A-",   # pre_ge1 & fvg & rp_eq_3        Win 68.8%, AvgR +0.53
    "B6":  "A-",   # sweep & pre_ge1 & mss          Win 72.7%, AvgR +0.37
    "B7":  "A-",   # mss & rp_eq_3                  Win 69.4%, AvgR +0.37
    "B8":  "A",    # sweep & d_cond & rp_eq_2       Win 71%, AvgR +0.63, PF 14.93 ⭐
    "B9":  "A-",   # wick_q1 & pre_ge1 & mss & fvg  Win 72.9%, AvgR +0.35
    "B10": "A-",   # pre_ge1 & trend & mss          Win 68.8%, AvgR +0.41
}
...
V6_SCENARIO = _os.environ.get("V6_SCENARIO", "FLAT")
...
OUTDIR = Path(f"yahoo_commodity_v30_v6_{V6_SCENARIO}_outputs")
...
print("=" * 80)
print(f"YAHOO COMMODITY v30 — V6 SUB-TIER {V6_SCENARIO} (S/S+/A+/A/A- best-match, 14 commodities)")
print("=" * 80)
...
def simulate_portfolio_v25(candidates_packs, max_total_risk=0.1325, scenario_name="BOOST15"):
        ...
        if current_time in entry_map:
            ...
            for idx, row in entry_events:
                ...
                if USE_V6_SUB_TIER:
                    ...
                    # 6. risk_pct = base × V6 sub-tier mult
                    risk_pct_effective = risk_pct_base * v6_mult
    ...
    return {
        ...
        "strategy_variant": f"v30_{V6_SCENARIO}_BOOST15",
        "scenario": scenario_name,
        "trades": trades_df,
        "skipped": skipped_df,
        "equity": equity_df,
...
if __name__ == "__main__":
    ...
    try:
        res["trades"].to_csv(outdir / f"v30_{V6_SCENARIO}_trades.csv", index=False)
        res["equity"].to_csv(outdir / f"v30_{V6_SCENARIO}_equity.csv", index=False)
        res["skipped"].to_csv(outdir / f"v30_{V6_SCENARIO}_skipped.csv", index=False)
        res["phase_transitions"].to_csv(outdir / f"v30_{V6_SCENARIO}_phase_transitions.csv", index=False)
        res["excess_log"].to_csv(outdir / f"v30_{V6_SCENARIO}_excess_log.csv", index=False)
        res["tax_log"].to_csv(outdir / f"v30_{V6_SCENARIO}_tax_log.csv", index=False)
        all_cand.to_csv(outdir / f"v30_{V6_SCENARIO}_all_candidates.csv", index=False)
        print(f"\n💾 CSV 저장 완료: {outdir}/")
    ...
    if len(tdf) == 0:
        ...
        summary_df.to_csv(outdir / f"v30_{V6_SCENARIO}_summary.csv", index=False)
        ...
        monthly_summary.to_csv(outdir / f"v30_{V6_SCENARIO}_monthly_summary.csv", index=False)
        ...
        asset_summary.to_csv(outdir / f"v30_{V6_SCENARIO}_asset_summary.csv", index=False)
        ...
        if "tier_label" in tdf.columns:
            ...
            tier_summary.to_csv(outdir / f"v30_{V6_SCENARIO}_tier_summary.csv", index=False)
        ...
        if "tier_v5_final" in tdf.columns:
            ...
            v5_summary.to_csv(outdir / f"v30_{V6_SCENARIO}_v5_tier_summary.csv", index=False)
        ...
        if "sub_tier_v6" in tdf.columns:
            ...
            v6_summary.to_csv(outdir / f"v30_{V6_SCENARIO}_v6_sub_tier_summary.csv", index=False)

        # 차트
        try:
            plot_equity_v25(res, save_path=str(outdir / f"v30_{V6_SCENARIO}_equity_chart.png"))
        except Exception as e:
