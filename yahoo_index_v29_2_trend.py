...
# ============================================================
# ★★★ v29.2 패치: 시나리오 B 차단 + RULE mult 조정 + 자산별 mult ★★★
# v29.1 → v29.2 변경:
#   - S_FVG_MAX_S: 3.0 → 2.5 (avg_loss -$1,409 위험 완화)
#   - S_PRE4_SCORE: 2.5 → 2.0 (avg_loss -$1,015 위험 완화)
#   - 모든 1.2x → 1.0x 통일 (A+_MAX, A+_SCORE_FVG, A_HIGH, A_RP3, A_RP2)
#   - 자산별 mult 신규: KS=1.5, ES=1.3
# ============================================================
USE_V29_1_PATCH = True       # v29.1 베이스 차단 활성화 (시나리오 B 12종 차단)
USE_V29_2_PATCH = True       # v29.2 신규: 자산별 mult 활성화

# v29.2: RULE 별 차등 mult (조정됨)
V29_1_RULE_MULT = {
    # S++ 핵심
...
# ★ v29.2: 자산별 mult (둠챠 지정)
V29_2_ASSET_MULT = {
    "KS":   1.5,   # PF 134.95, Win 83.3% — 가장 강한 자산
...
OUTDIR = Path("yahoo_index_v29_2_trend_outputs")
...
def compute_trade_tags(df_struct, entry_idx, zone_created_idx,
    ...
    # ★ v29.2-trend: HTF trend 원본값 보존 ("up" / "down" / "neutral")
    tags["htf_trend_raw"] = str(h4_trend)
...
def generate_trade_candidates_for_symbol(pack):
    ...
    while i < len(df_htf) - 1:
        ...
        candidates.append({
            ...
            # ★ v29.2-trend: HTF trend 원본값 ("up" / "down" / "neutral")
            "htf_trend_raw":          str(atoms_dict.get("htf_trend_raw", "neutral"))  if atoms_dict else "neutral",
        })
        i = sim["exit_idx"] + 1
...
def simulate_portfolio_v25(candidates_packs, max_total_risk=0.1325, scenario_name="BOOST15"):
    ...
    for current_time in event_times:
        ...
        if current_time in entry_map:
            ...
            for idx, row in entry_events:
                ...
                # ★ v29.2: 자산별 mult 추가 (RULE mult 와 곱셈)
                ...
                open_positions[idx] = {
                    ...
                    # ★ v29.2-trend: HTF trend 원본값 ("up" / "down" / "neutral")
                    "htf_trend_raw":           str(row.get("htf_trend_raw", "neutral")),
    ...
    return {
        ...
        "strategy_variant": "v29_2_trend_RULE_MULT",
        "scenario": scenario_name,
        "trades": trades_df,
        "skipped": skipped_df,
        "equity": equity_df,
...
if __name__ == "__main__":
    ...
    try:
        res["trades"].to_csv(outdir / "v29_2_trend_trades.csv", index=False)
        res["equity"].to_csv(outdir / "v29_2_trend_equity.csv", index=False)
        res["skipped"].to_csv(outdir / "v29_2_trend_skipped.csv", index=False)
        res["phase_transitions"].to_csv(outdir / "v29_2_trend_phase_transitions.csv", index=False)
        res["excess_log"].to_csv(outdir / "v29_2_trend_excess_log.csv", index=False)
        res["tax_log"].to_csv(outdir / "v29_2_trend_tax_log.csv", index=False)
        all_cand.to_csv(outdir / "v29_2_trend_all_candidates.csv", index=False)
        print(f"\n💾 CSV 저장 완료: {outdir}/")
    ...
    if len(tdf) == 0:
        ...
        summary_df.to_csv(outdir / "v29_2_trend_summary.csv", index=False)
        ...
        monthly_summary.to_csv(outdir / "v29_2_trend_monthly_summary.csv", index=False)
        ...
        asset_summary.to_csv(outdir / "v29_2_trend_asset_summary.csv", index=False)
        ...
        if "tier_label" in tdf.columns:
            ...
            tier_summary.to_csv(outdir / "v29_2_trend_tier_summary.csv", index=False)
        ...
        if "v25idx_subtier" in tdf.columns:
            ...
            subtier_summary.to_csv(outdir / "v29_2_trend_v25idx_subtier_summary.csv", index=False)
            ...
            rule_summary.to_csv(outdir / "v29_2_trend_v25idx_rule_summary.csv", index=False)

        # 차트
        try:
            plot_equity_v25(res, save_path=str(outdir / "v29_2_trend_equity_chart.png"))
        except Exception as e:
