# =========================================================
# smc_crypto_9coins  STAGE 4F: PF-BASED TIER (Option A 재설계)
#
# [배경 — Stage 4E v2 문제점 분석 결과]
...
#   2) tier_stage4f_distribution.csv   — Tier 별 trades/Win%/PF/risk 적용 이후 $
...
# =========================================================
# ★★★ Stage 4F: NEW TIER SYSTEM (PF-based re-rank) ★★★
# =========================================================
# v2 (SSS/SS/S/A/B) → v3 (ALPHA_MAX/HIGH/MED/BASE) 재편
# 이유: v2 의 Tier 순서가 PF 와 엇갈림 → Option A (PF 기반 재정렬)

# ─── Tier risk multiplier ───
TIER_RISK_MULT_4F = {
    "ALPHA_MAX":  2.0,   # PF 챔피언: overlap-only 또는 score+p1-only
    "ALPHA_HIGH": 1.5,   # Win 고/안정: volume+overlap 또는 sweep+volume
    "ALPHA_MED":  1.2,   # union 중 위 미해당
    "BASE":       1.0,   # union 밖 (OUT_BEHAVIOR 따라 적용 여부)
    "SKIP_MSS":   0.0,   # mss 단독 — 진입 차단
}

# ─── RP mult (Stage 4D 발견 유지: RP0 만 특별) ───
RP_MULT_4F = {
    0: 1.5,
    1: 1.0, 2: 1.0, 3: 1.0, 4: 1.0, 5: 1.0,
}

# ─── OUT 처리 (첫 실행 skip — v2 와 동일 조건 비교) ───
OUT_BEHAVIOR = "skip"

# ─── Notional cap ───
# ALPHA_MAX/HIGH 는 cap FREE (큰 배팅 허용), 나머지 3.0
TIER_CAP_4F = {
    "ALPHA_MAX":  999.0,
    "ALPHA_HIGH": 999.0,
    "ALPHA_MED":  3.0,
...
def classify_tier_stage4f(atoms_dict):
    """
    12 atomic 태그로부터 Stage 4F Tier 판정.
    
    Tier 체계 (Option A, PF 기반):
      ALPHA_MAX  (2.0x): (1) a_overlap AND NOT a_volume
                         (2) a_score_ge13 AND a_pre_total_ge1 AND NOT a_overlap
      ALPHA_HIGH (1.5x): (3) a_volume AND a_overlap AND (p1 or p4 or wq1 or s13)
                         (4) a_sweep AND a_volume AND NOT a_overlap
      ALPHA_MED  (1.2x): union(319 canonical) 중 위 미해당
      BASE       (1.0x): union 밖
      SKIP_MSS:          a_mss 단독 (any alpha 없음)
    
    Args:
        atoms_dict: 12 원자 boolean dict
    ...
    # ── ALPHA_MAX (2.0x): PF 챔피언 2 경로 ──
    # (1) a_overlap AND NOT a_volume — 현재 SS (PF 9.45)
    if ovl and not vol:
        return "ALPHA_MAX"
    # (2) a_score_ge13 AND a_pre_total_ge1 AND NOT a_overlap — 현재 S (PF 4.21)
    if s13 and p1 and not ovl:
        return "ALPHA_MAX"
    
    # ── ALPHA_HIGH (1.5x): Win 고, 안정 ──
    # (3) a_volume AND a_overlap AND 보조 — 현재 SSS (PF 3.52)
    if vol and ovl and (p1 or p4 or wq1 or s13):
        return "ALPHA_HIGH"
    # (4) a_sweep AND a_volume AND NOT a_overlap — 현재 A (PF 3.02)
    if swp and vol and not ovl:
        return "ALPHA_HIGH"
    
    # ── ALPHA_MED (1.2x): union(319 canonical) 매칭 but 위 미해당 ──
    if passes_win69_union(atoms_dict):
        return "ALPHA_MED"
    
    # ── BASE (1.0x): union 밖 (OUT_BEHAVIOR 따라 처리) ──
    return "BASE"


def get_tier_risk_mult_4f(tier_label):
    """Tier 별 risk mult 반환. BASE 는 OUT_BEHAVIOR 에 따라."""
    ...
    if tier_label == "BASE":
        if OUT_BEHAVIOR == "skip":
            ...
            return 0.0
    return TIER_RISK_MULT_4F.get(tier_label, 1.0)


def get_tier_cap_4f(tier_label):
    return TIER_CAP_4F.get(tier_label, 3.0)


# ─── 하위 호환: 기존 v2 심볼도 유지 (로그/CSV 스키마) ───
# classify_tier_stage4e() 호출 지점이 있으면 4F 로 위임
TIER_RISK_MULT_4E = TIER_RISK_MULT_4F
RP_MULT_4E = RP_MULT_4F
classify_tier_stage4e = classify_tier_stage4f
get_tier_risk_mult_4e = get_tier_risk_mult_4f
get_tier_cap_4e = get_tier_cap_4f
...
# =========================================================
# ★★★ Stage 4F: candidates 생성 (PF-based Tier 재정렬) ★★★
# =========================================================
print()
print("=" * 75)
print("[Stage 4F] candidates 생성 — PF-based Tier 재설계 (Option A)")
print("=" * 75)
print("  진입 조건 = MIN_SCORE 7.5 만 (Tier 판정은 simulate 단계)")
print(f"  OUT_BEHAVIOR = {OUT_BEHAVIOR!r}")
print(f"  TIER_RISK_MULT_4F = {TIER_RISK_MULT_4F}")
print(f"  RP_MULT_4F = {RP_MULT_4F}")
print(f"  Tier 체계: ALPHA_MAX(2.0) > ALPHA_HIGH(1.5) > ALPHA_MED(1.2) > BASE")
...
try:
    candidates_results_stage4e = _parallel_map(
        ...
        task_name="candidates Stage 4F", io_bound=False,
    )
...
print(f"\n  TOTAL candidates (Stage 4F): {total_stage4e}")
print("\n[OK] candidates 생성 완료 (Stage 4F). Tier 4F + RP4F 는 simulate 단계에서 적용됨.")




# =========================================================
# ★★★ 셀 3: 단일 시나리오 백테스트 실행 (Stage 4F) ★★★
# =========================================================
import os
OUTDIR = "stage4f_outputs"
...
STAGE4F_CASE = {
    "name":  "stage4f",
    "label": f"Stage 4F: PF-based Tier (OUT={OUT_BEHAVIOR})",
    "risk_mult": 1.0,
...
print()
print("#" * 75)
print(f"# Stage 4F 백테스트 시작: {STAGE4F_CASE['label']}")
print("#" * 75)

res = simulate_scenario_v19b_rpboost(
    scenario=SCENARIO_MULTI,
    candidates_dict=STAGE4F_CASE["candidates"],
    risk_multiplier=STAGE4F_CASE["risk_mult"],
)

monthly_report = print_scenario_summary(res)

prefix = f"stage4f_out_{OUT_BEHAVIOR}"
...
summary_df = pd.DataFrame([{
    "scenario":  STAGE4F_CASE["name"],
    "label":     STAGE4F_CASE["label"],
    "out_behavior": OUT_BEHAVIOR,
    "trades":    len(trades),
    "win%":      wr_overall,
    "PF":        pf_overall,
...
summary_df.to_csv(f"{OUTDIR}/stage4f_out_{OUT_BEHAVIOR}_overall_summary.csv", index=False)

print()
print("=" * 75)
print(f"[Stage 4F | OUT={OUT_BEHAVIOR}] 전체 성과")
print("=" * 75)
...
if len(tdf) == 0:
    ...
    print()
    print("=" * 95)
    print("[1] tier_stage4f_distribution — Tier 별 성과")
    print("=" * 95)
    print(f"{'tier':<8} {'trades':>7} {'win%':>7} {'avg_R':>7} {'PF':>7} "
          f"{'$/trade':>10} {'total $':>14}")
    print("-" * 95)

    tier_order = ["ALPHA_MAX", "ALPHA_HIGH", "ALPHA_MED", "BASE", "SKIP_MSS"]
    tier_rows = []
    for t in tier_order:
    ...
    tier_df.to_csv(f"{OUTDIR}/tier_stage4f_distribution_out_{OUT_BEHAVIOR}.csv", index=False)
...
print()
print("=" * 75)
print(f"[SAVED] Stage 4F 분석 산출물 (OUT={OUT_BEHAVIOR})")
print("=" * 75)
print(f"  {OUTDIR}/stage4f_out_{OUT_BEHAVIOR}_trades.csv")
print(f"  {OUTDIR}/stage4f_out_{OUT_BEHAVIOR}_equity.csv")
print(f"  {OUTDIR}/stage4f_out_{OUT_BEHAVIOR}_skipped.csv")
print(f"  {OUTDIR}/stage4f_out_{OUT_BEHAVIOR}_monthly.csv")
print(f"  {OUTDIR}/stage4f_out_{OUT_BEHAVIOR}_overall_summary.csv")
print(f"  {OUTDIR}/tier_stage4f_distribution_out_{OUT_BEHAVIOR}.csv")
print(f"  {OUTDIR}/tier_x_side_out_{OUT_BEHAVIOR}.csv")
print(f"  {OUTDIR}/rp_x_tier_out_{OUT_BEHAVIOR}.csv")

print()
print("=" * 75)
print("[Stage 4F] 실험 완료 (Option A — PF-based Tier 재정렬)")
print("=" * 75)
print(f"  OUT_BEHAVIOR: {OUT_BEHAVIOR}")
print(f"  Tier 체계: ALPHA_MAX(2.0x) > ALPHA_HIGH(1.5x) > ALPHA_MED(1.2x) > BASE")
print(f"  RP Boost:  RP0 only (1.5x), RP1~5 = 1.0x")
print(f"  예상 분포: ALPHA_MAX ~102 / ALPHA_HIGH ~428 / ALPHA_MED ~166 / BASE ~926 / SKIP ~2")
print(f"  다음: Tier 검증 후 OUT_BEHAVIOR 실험 예정")
