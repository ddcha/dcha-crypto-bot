# =========================================================
# smc_crypto_9coins  STAGE 2 COMPARE: 4-case experiment
#
# [4가지 케이스 비교]
#   Case A: Risk × 1.0 + Volume Filter ON   (baseline, 보수적)
#   Case B: Risk × 2.0 + Volume Filter ON
#   Case C: Risk × 1.0 + Volume Filter OFF
#   Case D: Risk × 2.0 + Volume Filter OFF
#
# [✨ Stage 2 Compare 핵심 차별점]
#   🎯 실전 근사 fill (보수적) 적용
#      - SHORT: zone_low 에서 체결 (상승 중 zone 진입 순간)
#      - LONG:  zone_high 에서 체결 (하락 중 zone 진입 순간)
#      - 이게 실전 엔진의 current_price 시장가 체결과 가장 유사
#      - 백테스트가 실전보다 덜 낙관적 (심적 안전마진)
#
# [구조]
#   - candidates VOL ON 생성 (1회) → A, B 시나리오에 사용
#   - candidates VOL OFF 생성 (1회) → C, D 시나리오에 사용
#   - 각 candidates 에 risk_multiplier 1.0, 2.0 시뮬레이션 실행 (총 4회)
#   - 결과 비교 테이블 자동 출력
#
# [유지] Stage 2 와 동일
#   - 9코인 (BTC+ETH+SOL+XRP+DOGE+AVAX+LINK+BNB+ADA)
#   - MIN_SCORE = 7.5 / REFINE 제거 / RP_BOOST
#   - S/A cap FREE (Tier S, A 는 notional 상한 해제)
    ...
    for current_time in event_times:
        ...
        if current_time in entry_map:
            for idx, row in entry_map[current_time]:
                ...
                tier_label, tier_mult, rp_action = classify_tier_v19b_rp_boost(
                ...
                if tier_mult <= 0.0:
                    skipped_entries.append({
                        "scenario_name": scenario["name"], "entry_time": row["entry_time"],
                        "symbol": symbol, 
                        "reason": f"tier_{tier_label}_skip_{rp_action}",
                ...
                risk_pct = risk_pct_base * tier_mult * risk_multiplier  # ⭐ risk_multiplier 곱셈

                # ⭐⭐ S/A cap FREE — 이전 세션 v19g 와 동일
                # Tier S, A 는 notional 상한 해제 (고품질 신호에 공격 position)
                # Tier B, C 는 기본 3.0 배 상한 유지
...
# =========================================================
# ★★★ VOL ON 으로 candidates 생성 (Case A, B 용) ★★★
# =========================================================
print()
print("=" * 75)
print("🎯 candidates 생성 — VOL ON (Case A, B 용)")
print("=" * 75)

USE_VOLUME_FILTER = True  # ⭐ VOL ON

try:
    candidates_results_vol_on = _parallel_map(
        _generate_candidates_single, SYMBOLS_TO_PREPARE,
        task_name="candidates VOL ON", io_bound=False,
    )
    candidates_dict_vol_on = dict(candidates_results_vol_on)
except Exception as e:
    print(f"  ⚠️ candidates 병렬 실패: {e}, 순차 처리")
    candidates_dict_vol_on = {}
    for sym in SYMBOLS_TO_PREPARE:
        candidates_dict_vol_on[sym] = generate_candidates_from_prepared(prepared_data[sym])

# VOL ON 집계
total_vol_on = sum(len(candidates_dict_vol_on[s]["candidates"]) for s in SYMBOLS_TO_PREPARE)
print(f"\n  TOTAL candidates (VOL ON): {total_vol_on}")


# =========================================================
# ★★★ VOL OFF 으로 candidates 생성 (Case C, D 용) ★★★
# =========================================================
print()
print("=" * 75)
print("🎯 candidates 생성 — VOL OFF (Case C, D 용)")
print("=" * 75)

USE_VOLUME_FILTER = False  # ⭐ VOL OFF

try:
    candidates_results_vol_off = _parallel_map(
        _generate_candidates_single, SYMBOLS_TO_PREPARE,
        task_name="candidates VOL OFF", io_bound=False,
    )
    candidates_dict_vol_off = dict(candidates_results_vol_off)
except Exception as e:
    print(f"  ⚠️ candidates 병렬 실패: {e}, 순차 처리")
    candidates_dict_vol_off = {}
    for sym in SYMBOLS_TO_PREPARE:
        candidates_dict_vol_off[sym] = generate_candidates_from_prepared(prepared_data[sym])

total_vol_off = sum(len(candidates_dict_vol_off[s]["candidates"]) for s in SYMBOLS_TO_PREPARE)
print(f"\n  TOTAL candidates (VOL OFF): {total_vol_off}")
print(f"  → VOL OFF 가 VOL ON 보다 {total_vol_off-total_vol_on}건 더 많음 (VOL 필터 통과 못 한 거래)")

print("\n✅ candidates 생성 완료 (2가지 버전).")


# =========================================================
# ★★★ 셀 3: 4 케이스 백테스트 실행 ★★★
# =========================================================
OUTDIR = "smc_crypto_stage2_compare_outputs"
import os
os.makedirs(OUTDIR, exist_ok=True)

# 4 케이스 정의
CASES = [
    {"name": "A", "label": "Risk×1.0 / VOL ON",  "risk_mult": 1.0, "candidates": candidates_dict_vol_on},
    {"name": "B", "label": "Risk×2.0 / VOL ON",  "risk_mult": 2.0, "candidates": candidates_dict_vol_on},
    {"name": "C", "label": "Risk×1.0 / VOL OFF", "risk_mult": 1.0, "candidates": candidates_dict_vol_off},
    {"name": "D", "label": "Risk×2.0 / VOL OFF", "risk_mult": 2.0, "candidates": candidates_dict_vol_off},
]
...
for case in CASES:
    ...
    res = simulate_scenario_v19b_rpboost(
        ...
        candidates_dict=case["candidates"],
        risk_multiplier=case["risk_mult"],
    )
    
    monthly_report = print_scenario_summary(res)
    results[case["name"]] = {"result": res, "monthly": monthly_report, "case": case}
    
    # CSV 저장
    prefix = f"case_{case['name']}"
...
for name in ["A", "B", "C", "D"]:
    ...
    pf = gp / max(gl, 1e-9)
    wr = (trades['net_pnl']>0).mean() * 100 if len(trades)>0 else 0
    avgr = trades['r_multiple'].mean() if len(trades)>0 else 0
    mdd = eq['dd_pct'].min() if len(eq)>0 else 0
    ...
    # 퇴사 시점
    qualified = monthly[monthly['rolling_3m_avg_krw'] >= RETIREMENT_MONTHLY_TARGET_KRW]
    retirement_month = qualified.iloc[0]['month'] if len(qualified)>0 else "미달성"
    months_taken = monthly[monthly['month'] == retirement_month].index[0] + 1 if len(qualified)>0 else None
...
# 예쁘게 출력
print(f"{'Case':<5} {'구성':<22} {'Trades':>7} {'Win%':>6} {'PF':>6} {'avg_R':>7} {'MDD%':>7} {'Total억':>10} {'Excess억':>10} {'Return%':>11} {'퇴사':>8} {'개월':>5}")
print("-" * 140)
for _, r in summary_df.iterrows():
    months_str = str(r["퇴사_개월"]) if r["퇴사_개월"] != "X" else "X"
    print(f"{r['case']:<5} {r['label']:<22} {r['trades']:>7} {r['win%']:>5.2f}% {r['PF']:>6.2f} {r['avg_R']:>7.3f} {r['MDD%']:>6.2f}% {r['Total_억KRW']:>10.2f} {r['Excess_억KRW']:>10.2f} {r['Return_%']:>10.1f}% {str(r['퇴사']):>8} {months_str:>5}")

# CSV 저장
summary_df.to_csv(f"{OUTDIR}/case_comparison.csv", index=False)
...
print("💎 해석 가이드")
print("=" * 75)
print("  • Risk ×2.0 = qty 2배 = 더 공격적 진입")
print("    → 이익 2배, 손실 2배, MDD 증가 예상")
print("  • VOL OFF = 더 많은 거래 진입 (필터 없음)")
print("    → 거래 수 증가, 품질 하락 가능")
print()
print("  🎯 최적 케이스 선정 기준:")
print("    1) MDD 한자리수 (< 10%)")
print("    2) PF > 5")
...
print()
print(f"💾 모든 CSV 저장 완료: {OUTDIR}/")
print(f"   (각 case별 trades, equity, skipped, monthly + 종합 case_comparison.csv)")
