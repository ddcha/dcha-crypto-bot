...
def main():
    ...
    # ── BTC 레짐 태깅 (룩어헤드-free, strict <) ──
    print("\n[BTC레짐] 태깅 + 룩어헤드 검증")
    btc = pd.read_parquet("data_cache/BTCUSDT_4h.parquet")
    ...
    btc["btc_dist"] = (btc["ma10"] - btc["ma30"]) / btc["ma30"] * 100.0
    btc["btc_up"] = (btc["ma10"] > btc["ma30"])
    bts = btc["timestamp"].values
    et = pd.to_datetime(trades["entry_time"], utc=True)
    pos = np.searchsorted(bts, et.values, side="left") - 1   # entry_time 이전(strict <) 마지막 BTC봉
    pos = np.clip(pos, 0, len(btc) - 1)
    trades["btc_match_ts"] = btc["timestamp"].values[pos]
    trades["btc_dist"] = btc["btc_dist"].values[pos]
    trades["btc_up"] = btc["btc_up"].values[pos]
    # ★룩어헤드 검증: 매칭 BTC봉 timestamp < entry_time 전수
    valid = pos >= 0
    future = int(((trades["btc_match_ts"].values >= et.values) & valid).sum())
    early = int((pos < 0).sum())  # entry 이전 BTC봉 자체가 없는 경우(초반)
    print(f"  매칭 BTC봉 미래참조(>= entry_time): {future}건 | entry이전봉없음(초반): {early}건")
    if future != 0:
        raise SystemExit(f"[ABORT] BTC 룩어헤드 — 미래참조 {future}건")
    print("  ✅ BTC 미래참조 0건 — 룩어헤드 통과")

    os.makedirs(OUTDIR, exist_ok=True)
    trades.to_csv(os.path.join(OUTDIR, "trades.csv"), index=False)
    print(f"[저장] {OUTDIR}/trades.csv ({len(trades)}행, +btc_dist +btc_up +side)")
    ...
    # ── BTC 레짐 참고표 (이격도 구간 × 방향) ──
    rg = rl.copy(); rg["exit_time"] = pd.to_datetime(rg["exit_time"], utc=True)

    def _bin(x):
        if x < -3: return "1_강하락<-3"
        if x < -1: return "2_약하락-3~-1"
        if x <= 1: return "3_횡보-1~1"
        if x <= 3: return "4_약상승1~3"
        return "5_강상승>3"

    rg["btc_bin"] = rg["btc_dist"].astype(float).apply(_bin)
    print("\n=== BTC 이격도구간 × 방향 (참고 — 거름X) ===")
    print(f"{'btc구간':<14}{'side':<6}{'n':>5}{'win%':>7}{'PF':>8}{'OOS':>8}{'totR':>8}")
    brows = []
    for b in ["1_강하락<-3", "2_약하락-3~-1", "3_횡보-1~1", "4_약상승1~3", "5_강상승>3"]:
        for sd in ["long", "short"]:
            g = rg[(rg["btc_bin"] == b) & (rg["side"] == sd)].sort_values("exit_time")
            if len(g) == 0:
                continue
            pnl = g["net_pnl"].astype(float).values; r = g["r_multiple"].astype(float).values
            n = len(g); k = int(n * 0.7)
            row = {"btc_bin": b, "side": sd, "n": n, "win%": round(float((pnl > 0).mean() * 100), 1),
                   "PF": round(_pf(pnl), 3), "OOS": round(_pf(pnl[k:]), 3) if n - k else float("nan"),
                   "totR": round(float(r.sum()), 2)}
            brows.append(row)
            print(f"{b:<14}{sd:<6}{n:>5}{row['win%']:>7}{row['PF']:>8}{str(row['OOS']):>8}{row['totR']:>8}")
    pd.DataFrame(brows).to_csv(os.path.join(OUTDIR, "btc_regime_table.csv"), index=False)
