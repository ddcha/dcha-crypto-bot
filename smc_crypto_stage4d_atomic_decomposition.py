...
FEE_RATE = 0.0005
SL_BUFFER_MULT = 0.08
import os as _os_minscore
MIN_SCORE = float(_os_minscore.environ.get("STAGE4D_MIN_SCORE", "7.5"))  # ⭐ 진입 문턱 (교집합 아님! 단순 PF 필터). env로 하향 가능.
...
# ─── 신규 원자 (배치1) 상수 ───  모두 인덱스 ≤ entry_idx 만 읽음(룩어헤드 0)
KILLZONE_HOURS = (8, 12)          # UTC H4봉 open 시각: 08=런던, 12=뉴욕 킬존
ATR_PCTL_WINDOW = 100             # atr 백분위 회귀(변동성 레짐) 윈도우
ATR_EXPANSION_PCTL = 0.50         # 현재 atr 이 직전 윈도우 중앙값 이상이면 확장레짐
DISP_STRENGTH_MIN = 1.50          # 셋업 디스플레이스먼트 강도(range/atr) 임계
DISP_SCAN_BACK = 6                # zone_created_idx 직전 N봉 내 디스플레이스먼트 탐색
VOLUME_STRICT_MULT = 1.80         # 엄격 거래량: zone봉 vol / 직전20봉 중앙값
...
def compute_trade_tags(df_struct, entry_idx, zone_created_idx,
                       ...
                       override_trend=None, override_pd_high=None, override_pd_low=None):
    """
    ...
    Args:
        ...
        override_trend: None 이 아니면 a_trend_align 의 H4 trend 를 이 값으로 대체
                        (둠챠 현재봉 진행분 H1/15m 재구성용; None→기존 df_struct.loc[entry_idx,"trend"])
        override_pd_high/override_pd_low: None 이 아니면 a_room 의 pd_high/pd_low 를 대체
                        (None→기존 df_struct.loc[entry_idx,"pd_high"/"pd_low"])
    ※ 세 override 모두 None 이면 기존 동작과 바이트 동일. 나머지 10원자는 불변(과교정 금지).
    ...
    if override_trend is not None:
        h4_trend = override_trend
    else:
    ...
    if pd.notna(atr_val) and atr_val > 0:
        if side == "long":
            ...
            if override_pd_high is not None:
                room_target = override_pd_high
            else:
            ...
            if override_pd_low is not None:
                room_target = override_pd_low
            else:
    ...
    # ─── 신규 원자 (배치1) ─── 전부 인덱스 ≤ entry_idx 만 참조(룩어헤드 0)
    # a_killzone: 신호봉 timestamp 의 UTC 시각이 킬존(런던/뉴욕)인가. 시각은 봉 시작에 이미 확정 → 미래정보 아님.
    a_killzone = False
    if "timestamp" in df_struct.columns:
        _ts = df_struct.loc[entry_idx, "timestamp"]
        if pd.notna(_ts):
            a_killzone = (pd.Timestamp(_ts).hour in KILLZONE_HOURS)
    tags["a_killzone"] = bool(a_killzone)

    # a_atr_expansion: 현재 atr 이 직전 ATR_PCTL_WINDOW 봉 중 백분위 >= 0.5 (변동성 확장레짐).
    #   atr[k]=과거TR rolling(14) → 모두 k 이하 정보. 윈도우 상한 = entry_idx.
    a_atr_expansion = False
    if "atr" in df_struct.columns:
        _w0 = max(0, entry_idx - ATR_PCTL_WINDOW + 1)
        _aw = pd.to_numeric(df_struct.loc[_w0:entry_idx, "atr"], errors="coerce").dropna()
        _cur = df_struct.loc[entry_idx, "atr"]
        if len(_aw) >= 10 and pd.notna(_cur):
            a_atr_expansion = (float((_aw < _cur).mean()) >= ATR_EXPANSION_PCTL)
    tags["a_atr_expansion"] = bool(a_atr_expansion)

    # a_disp_strength: 셋업 디스플레이스먼트 강도(range/atr) >= 1.5 (방향 일치).
    #   zone 생성 직전 DISP_SCAN_BACK 봉 내에서 탐색 → 상한=zone_created_idx (≤ entry_idx).
    a_disp_strength = False
    if "disp_strength" in df_struct.columns and pd.notna(zone_created_idx):
        _zc = int(zone_created_idx)
        _d0 = max(0, _zc - DISP_SCAN_BACK)
        _seg = df_struct.loc[_d0:_zc]
        _dcol = "bull_disp" if side == "long" else "bear_disp"
        if _dcol in _seg.columns:
            _strong = pd.to_numeric(_seg.loc[_seg[_dcol].astype(bool), "disp_strength"], errors="coerce")
            a_disp_strength = bool((_strong >= DISP_STRENGTH_MIN).any())
    tags["a_disp_strength"] = bool(a_disp_strength)

    # a_volume_strict: zone봉 거래량 / 직전 20봉 중앙값 >= 1.8 (a_volume 1.1×평균 보다 엄격·로버스트).
    a_volume_strict = False
    if "volume" in df_struct.columns:
        _vs = max(0, zone_created_idx - VOLUME_AVG_WINDOW)
        _ve = zone_created_idx
        if _ve > _vs:
            _med = pd.to_numeric(df_struct.loc[_vs:_ve - 1, "volume"], errors="coerce").median()
            _zv = df_struct.loc[zone_created_idx, "volume"]
            if pd.notna(_med) and _med > 0 and pd.notna(_zv):
                a_volume_strict = (float(_zv) / float(_med)) >= VOLUME_STRICT_MULT
    tags["a_volume_strict"] = bool(a_volume_strict)

    return tags


def doomcha_current_bar_override(df_struct, df_15m, entry_idx_sim, entry_price,
                                 side, pd_lookback):
    """둠챠 현재봉 재구성: 진입봉 i 의 '진행 중' 상태를 15m 누적으로 복원.

    철학: i-1(직전 종가확정봉)까지는 그대로 두고, 현재봉 i 의 OHLC 를
          '진입가에 처음 닿는 15m 봉(시점 t)'까지만 누적해서 근사.
          → t 이후 데이터는 일절 읽지 않음(미래참조 0).

    Returns: (override_trend, override_pd_high, override_pd_low)
             재구성 불가 시 (None, None, None) → 호출부가 기존 i-1 동작으로 폴백.
    """
    if df_15m is None or len(df_15m) == 0:
        return (None, None, None)
    if entry_idx_sim < 1 or entry_idx_sim >= len(df_struct):
        return (None, None, None)
    try:
        bar_open_ts = df_struct.loc[entry_idx_sim, "timestamp"]
    except Exception:
        return (None, None, None)
    bar_end_ts = bar_open_ts + pd.Timedelta(hours=4)

    m = df_15m[(df_15m["timestamp"] >= bar_open_ts) &
               (df_15m["timestamp"] < bar_end_ts)]
    if len(m) == 0:
        return (None, None, None)
    m_hi = m["high"].to_numpy(float)
    m_lo = m["low"].to_numpy(float)
    m_cl = m["close"].to_numpy(float)

    # 진입가에 처음 닿는 15m 봉(시점 t) 탐색 — t 이후는 보지 않음
    touch_pos = None
    for j in range(len(m)):
        if m_lo[j] <= entry_price <= m_hi[j]:
            touch_pos = j
            break
    if touch_pos is None:
        return (None, None, None)

    acc_high = float(np.max(m_hi[:touch_pos + 1]))
    acc_low = float(np.min(m_lo[:touch_pos + 1]))
    close_t = float(m_cl[touch_pos])

    # ── override_trend: i-1 확정 EMA 를 close_t 한 스텝 전진 (ewm adjust=False 재현)
    override_trend = None
    try:
        e20 = float(df_struct.loc[entry_idx_sim - 1, "ema20"])
        e50 = float(df_struct.loc[entry_idx_sim - 1, "ema50"])
        e200 = float(df_struct.loc[entry_idx_sim - 1, "ema200"])
        if not (np.isnan(e20) or np.isnan(e50) or np.isnan(e200)):
            a20, a50, a200 = 2 / 21, 2 / 51, 2 / 201
            n20 = a20 * close_t + (1 - a20) * e20
            n50 = a50 * close_t + (1 - a50) * e50
            n200 = a200 * close_t + (1 - a200) * e200
            if n20 < n50 < n200:
                override_trend = "down"
            elif n20 > n50 > n200:
                override_trend = "up"
            else:
                override_trend = "neutral"
    except Exception:
        override_trend = None

    # ── override_pd_high/low: i-1 까지의 rolling 창 + 현재봉 진행분(acc) 결합
    lo = max(0, entry_idx_sim - pd_lookback + 1)
    override_pd_high = acc_high
    override_pd_low = acc_low
    if entry_idx_sim - 1 >= lo:
        prior_hi = df_struct.loc[lo:entry_idx_sim - 1, "high"].max()
        prior_lo = df_struct.loc[lo:entry_idx_sim - 1, "low"].min()
        if pd.notna(prior_hi):
            override_pd_high = max(acc_high, float(prior_hi))
        if pd.notna(prior_lo):
            override_pd_low = min(acc_low, float(prior_lo))

    return (override_trend, override_pd_high, override_pd_low)
...
def download_symbol_data(symbol):
    ...
    out = {"symbol": symbol, "df_raw": df, "df_h1_raw": df_h1}
    # 둠챠 현재봉 정밀화: 15m raw OHLC (게이트 켜진 경우에만 다운로드)
    import os
    if os.environ.get("DOOMCHA_15M", "0") == "1":
        df_15m = download_data(symbol=symbol, interval="15m")
        df_15m = df_15m[df_15m["timestamp"] >= cutoff].reset_index(drop=True)
        out["df_15m_raw"] = df_15m
        print(f"{symbol} H4: {len(df)} rows, H1: {len(df_h1)} rows, 15m: {len(df_15m)} rows")
    else:
...
def apply_indicators_and_build(raw):
    ...
    # 둠챠 현재봉 정밀화: 15m raw 전파 (재구성에는 raw OHLC+timestamp 만 필요)
    if "df_15m_raw" in raw:
        out["df_15m"] = raw["df_15m_raw"].copy()
    return out
...
def generate_candidates_from_prepared(prepared):
    ...
    df_15m = prepared.get("df_15m")  # 둠챠 현재봉 정밀화용 (게이트 off면 None)
    ...
    # ⭐ [ATOM_AND] 12원자 AND 진입게이트 — 실제 백테스트(슬롯재배치 반영, post-hoc 아님).
    #   ATOM_AND_LIST="a_volume,a_fvg,~a_room" → 모든 리터럴 True 여야 진입(교집합).
    #   "~atom"=음극(NOT atom). 빈값=AND게이트 없음. OR/AND 둘 다 켜면 (OR통과) AND (AND통과).
    ...
    _ATOM_GATE_ON = bool(_ATOM_OR_RAW or _ATOM_AND_RAW)
    def _atom_gate_pass(tags):
        return _atom_or_pass(tags) and _atom_and_pass(tags)
    ...
    while i < len(df_struct) - 1:
        ...
        for s, eff_score, fresh_reasons in scored_active:
            ...
            if s["type"] == "short":
                ...
                if _ATOM_GATE_ON:  # 12원자 OR/AND 진입게이트 (실값 atoms, _ti=정직봉)
                    if not _atom_gate_pass(compute_trade_tags(
        ...
        if current_time in entry_map:
            for idx, row in entry_map[current_time]:
                ...
                if _df_struct_here is None:
                    ...
                    try:
                        ...
                        if len(_match) > 0:
                            ...
                            # ── 둠챠 현재봉 정밀화(게이트): a_trend_align/a_room 만 15m 재구성 override.
                            #    나머지 10원자 불변(과교정 금지). off 면 (None,None,None)→기존 i-1 동작.
                            _ov_trend = _ov_pdh = _ov_pdl = None
                            if _os_atom.environ.get("DOOMCHA_15M", "0") == "1":
                                _df_15m_here = candidates_dict[symbol].get("df_15m")
                                _ov_trend, _ov_pdh, _ov_pdl = doomcha_current_bar_override(
                                    df_struct=_df_struct_here,
                                    df_15m=_df_15m_here,
                                    entry_idx_sim=_entry_idx_sim,
                                    entry_price=float(row["entry"]),
                                    side=row["side"],
                                    pd_lookback=H4_PD_LOOKBACK,
                                )
                            ...
                            _atoms_final = compute_trade_tags(
                                ...
                                override_trend=_ov_trend,
                                override_pd_high=_ov_pdh,
                                override_pd_low=_ov_pdl,
                            )
                        else:
                ...
                open_positions[idx] = {
                    ...
                    # ⭐ 신규 원자 (배치1)
                    "a_killzone":         bool(_atoms_final.get("a_killzone", False)),
                    "a_atr_expansion":    bool(_atoms_final.get("a_atr_expansion", False)),
                    "a_disp_strength":    bool(_atoms_final.get("a_disp_strength", False)),
                    "a_volume_strict":    bool(_atoms_final.get("a_volume_strict", False)),
                    # 하위 호환 (기존 CSV 스키마 유지)
