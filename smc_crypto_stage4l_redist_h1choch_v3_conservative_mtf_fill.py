...
TIER_RISK_MULT_COMMON = {
    "ALPHA_MAX":    float(_os.environ.get("ALPHA_MAX_TIER_MULT", "2.5")),  # default 2.5, override 가능 (예: 3.5 hybrid)
...
_REVIVE_CUTS = _os.environ.get("REVIVE_CUTS", "0") == "1"
if _REVIVE_CUTS:
    print("[REVIVE_CUTS] CUT_NeutFalling_S, CUT_ShortStable_L_DISABLED → BOOST 로 재분류 (booster mult 적용)")
...
def build_structures(df):
    # ⭐ [ZONE-PARAM 진단] 실사용 zone 파라미터 1회 출력 (print-only)
    if not getattr(build_structures, "_printed", False):
        print(f"[ZONE-PARAM] MAX_AGE={H4_ZONE_MAX_AGE} MSS={H4_MSS_LOOKBACK} "
              f"OB={H4_OB_LOOKBACK} PD={H4_PD_LOOKBACK}")
        build_structures._printed = True
    recent_sweep_n = 10
    df = df.copy()
...
print("[PHASE 1-1 진단]")
print("=" * 75)
# [B] 실사용 zone 파라미터 (부모 전역 = 자식과 동일, 리바인딩 반영)
print(f"[ZONE-PARAM] MAX_AGE={H4_ZONE_MAX_AGE} MSS={H4_MSS_LOOKBACK} "
      f"OB={H4_OB_LOOKBACK} PD={H4_PD_LOOKBACK}")
# [A] 누수① 정량화 — 심볼별 return dict 에서 집계 (loky 멀티프로세스 안전)
_leak1_same_all = 0
_leak1_total_all = 0
_leak1_dist_all = {}
...
# (i - zone_created_idx) 분포 — same-bar 가 0봉에만 몰렸는지 1~2봉도 섞였는지
...
#   RUN_TAG 로 파일명 구분. DIAG_STOP_AFTER_CANDIDATES=1 이면 simulate 전 종료.
...
# =========================================================
import os
# ✓ [V3 CONSERVATIVE] 결과를 별도 폴더에 저장 (h1choch / v2 와 비교 가능)
OUTDIR = _os.environ.get("OUTDIR_OVERRIDE", "stage4l_redist_outputs_v3_conservative")
os.makedirs(OUTDIR, exist_ok=True)
print(f"\n[OUTDIR] {os.path.abspath(OUTDIR)}")
print(f"[CONFIG] OUT_BEHAVIOR = {OUT_BEHAVIOR!r}")
