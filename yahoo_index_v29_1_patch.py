...
# ============================================================
# ★★★ v29.1 패치: 시나리오 B 차단 + RULE 별 차등 mult ★★★
# ============================================================
USE_V29_1_PATCH = True       # True 면 v29.1 적용 (시나리오 B XB 차단 + RULE mult)

# v29.1: RULE 별 차등 mult (둠챠 지정)
V29_1_RULE_MULT = {
    # S++ 핵심
    "S++_MSS_OVL":          5.0,   # n=10  Win 90%   PF 579
    ...
    "S_FVG_MAX_S":          3.0,   # n=25  Win 76.0% PF 2.17
    # A+ 중 승격
    "A+_FVG_D_RP3":         3.5,   # n=8   Win 62.5% PF 5.61 ⭐ 승격
    # 나머지 S 계열
    "S_PRE4_SCORE":         2.5,   # n=42  Win 59.5% PF 2.28
    "S_SCORE_S":            2.5,   # rp=3 만 살아남음 (n=8 Win 62.5%)
    "S_SCORE_MAX_WICK":     2.5,   # 도달불가
    # A+ 나머지
    "A+_MAX":               1.2,   # n=6   Win 66.7% PF 2.48
    "A+_SCORE_FVG":         1.2,   # n=28  Win 53.6% PF 1.75
    # A 전부
    "A_HIGH":               1.2,   # n=131 (살아남은 부분만)
    "A_RP3":                1.2,   # n=9 (살아남은 부분만)
    "A_RP2":                1.2,   # n=34
    # S+ 5종 (코드상 도달 불가, 안전망)
...
OUTDIR = Path("yahoo_index_v29_1_patch_outputs")
...
def simulate_portfolio_v25(candidates_packs, max_total_risk=0.1325, scenario_name="BOOST15"):
    ...
    for current_time in event_times:
        ...
        if current_time in entry_map:
            ...
            for idx, row in entry_events:
                ...
                # ★★★ v29.1: RULE 별 차등 mult 적용 ★★★
                if USE_V29_1_PATCH:
                    risk_pct_effective = risk_pct_base * v25idx_rule_mult
                    tier_v19b_mult_applied = 1.0
                    tier_mult_applied = 1.0
                    rp_mult_applied = 1.0
                    stage4j_mult_applied = 1.0
                    sentiment_mult_applied = v25idx_rule_mult  # 추적용 (실제 v25idx_rule_mult)
    ...
    return {
        ...
        "strategy_variant": "v29_1_RULE_MULT",
        "scenario": scenario_name,
        "trades": trades_df,
        "skipped": skipped_df,
        "equity": equity_df,
...
if __name__ == "__main__":
    ...
    try:
        res["trades"].to_csv(outdir / "v29_1_trades.csv", index=False)
        res["equity"].to_csv(outdir / "v29_1_equity.csv", index=False)
        res["skipped"].to_csv(outdir / "v29_1_skipped.csv", index=False)
        res["phase_transitions"].to_csv(outdir / "v29_1_phase_transitions.csv", index=False)
        res["excess_log"].to_csv(outdir / "v29_1_excess_log.csv", index=False)
        res["tax_log"].to_csv(outdir / "v29_1_tax_log.csv", index=False)
        all_cand.to_csv(outdir / "v29_1_all_candidates.csv", index=False)
        print(f"\n💾 CSV 저장 완료: {outdir}/")
    ...
    if len(tdf) == 0:
        ...
        summary_df.to_csv(outdir / "v29_1_summary.csv", index=False)
        ...
        monthly_summary.to_csv(outdir / "v29_1_monthly_summary.csv", index=False)
        ...
        asset_summary.to_csv(outdir / "v29_1_asset_summary.csv", index=False)
        ...
        if "tier_label" in tdf.columns:
            ...
            tier_summary.to_csv(outdir / "v29_1_tier_summary.csv", index=False)
        ...
        if "v25idx_subtier" in tdf.columns:
            ...
            subtier_summary.to_csv(outdir / "v29_1_v25idx_subtier_summary.csv", index=False)
            ...
            rule_summary.to_csv(outdir / "v29_1_v25idx_rule_summary.csv", index=False)

        # 차트
        try:
            plot_equity_v25(res, save_path=str(outdir / "v29_1_equity_chart.png"))
        except Exception as e:
