# =========================================================
# smc_crypto_7coins  STAGE 1: v19b_norefine_rpboost
#
# [Stage 1 변경사항 (v1.9b 대비)]
#
#   1) REFINE 제거
#      - LONG_BASE_ENTRY_FRAC: 0.40 → 0.10 (보수적 진입)
#      - SHORT_BASE_ENTRY_FRAC: 0.60 → 0.90 (보수적 진입)
#      - refine_entry_with_h1_fvg 호출 bypass
#      - → 실전 엔진과 동일한 진입 로직
#
#   2) RP_BOOST 추가
#      - RP_TIER_MULT_TABLE = {0:1.5, 1:1.5, 2:1.0, 3:1.0, 4:1.0, 5:1.0}
#      - RP 0/1 에 1.5× boost, 2~5 그대로, 6+ skip
#      - classify_tier_v19b_rp_boost() 신규 함수
#
#   3) 병렬화 추가
#      - joblib(loky) / ProcessPool / Thread / 순차 fallback
#      - 7코인 기준 25분 → 7~10분 예상
#
#   4) strategy_variant = "v19b_norefine_rpboost"
#
# [유지 사항 (v1.9b 와 동일)]
#   - 7 심볼 (BTC, ETH, SOL, XRP, DOGE, AVAX, LINK)
#   - MIN_SCORE = 7.5 (단순 PF 필터, 교집합 아님!)
#   - 티어 판정 threshold = 0.2300
#   - 단일 포지션 (symbol_already_open)
#   - VOLUME_FILTER = True (유일한 활성 필터)
#   - H4 exit 시뮬레이션 (LTF 변환 없음)
#   - Phase A/B + Excess relay
# =========================================================
...
SCENARIO_MULTI = {
    "name": "BTC+ETH+SOL+XRP+DOGE+AVAX+LINK v19b norefine rpboost",
    "assets": {
...
def generate_candidates_from_prepared(prepared):
    ...
    while i < len(df_struct) - 1:
        ...
        for s, eff_score, fresh_reasons in scored_active:
            ...
            # ★ v1.9b: H4 open 을 zone clamp 한 것을 fill_entry 로 사용
...
# =========================================================
def simulate_scenario_v19b_rpboost(scenario, candidates_dict):
        ...
        if current_time in entry_map:
            for idx, row in entry_map[current_time]:
                ...
                open_positions[idx] = {
                    ...
                    "strategy_variant": "v19b_norefine_rpboost",
...
print("=" * 75)
print("🔍 실행 환경 (Stage 1: v19b_norefine_rpboost / 7코인)")
print("=" * 75)
...
try:
    candidates_results = _parallel_map(
        ...
        task_name="candidates", io_bound=False,
    )
...
print(f"{'  TOTAL':<12}: candidates={total_candidates:>4} | no_fill={total_no_fill:>4}")
...
# ★★★ 셀 3: Stage 1 백테스트 실행 ★★★
# =========================================================
OUTDIR = "smc_crypto_stage1_7coins_outputs"
import os
...
res["trades"].to_csv(f"{OUTDIR}/stage1_trades.csv", index=False)
res["equity"].to_csv(f"{OUTDIR}/stage1_equity.csv", index=False)
res["skipped"].to_csv(f"{OUTDIR}/stage1_skipped.csv", index=False)
res["phase_transitions"].to_csv(f"{OUTDIR}/stage1_phase_transitions.csv", index=False)
res["excess_log"].to_csv(f"{OUTDIR}/stage1_excess_log.csv", index=False)
monthly_report.to_csv(f"{OUTDIR}/stage1_monthly_pnl.csv", index=False)
...
if all_no_fill:
    ...
    no_fill_all.to_csv(f"{OUTDIR}/stage1_no_fill.csv", index=False)

print(f"\n💾 CSV 저장 완료: {OUTDIR}/")
print(f"\n🎯 Stage 1 백테스트 완료! (v19b_norefine_rpboost, 7코인)")
print(f"   threshold: {WICK_RATIO_5_Q1_THRESHOLD}")
rp_table_str = " | ".join([f"rp{k}={v}" for k, v in sorted(RP_TIER_MULT_TABLE.items())])
print(f"   RP table: {rp_table_str} | else=skip")
print(f"   설정: 단일 포지션 (symbol_already_open), REFINE 제거, RP_BOOST 적용")
