# =========================================================
# smc_crypto_9coins  STAGE 4H: SWEEP SPLIT — 3 시나리오 동시 비교
#
# [배경]
#   Stage 4G 분석 결과:
#     - SWEEP_BASE 511t 의 98.6% 수익이 TOP 25 에 집중
#     - TOP 25 의 24건이 a_room 포함 (sweep+room+fvg 지배 패턴)
#     - 하지만 SWEEP_NO_ROOM (88t) 이 PF 9.89 로 숨은 gem 발견
#     - SWEEP_ROOM+NO_FVG (120t) 는 PF 1.16 로 거의 break-even
#   R 계산은 버그 아님 (tight SL + big move = 정상 대박)
#
# [실험 목적]
#   SWEEP_BASE 세분화 방식 3가지를 동시 실행하여 비교:
#
#   시나리오 A (sweep_split_3tier):
#     SWEEP_GEM       (1.5×) — sweep + NOT room + (기타 alpha 없음)
#     SWEEP_ROOM_FVG  (1.0×) — sweep + room + fvg
#     SWEEP_ROOM_ONLY (0.5×) — sweep + room + NOT fvg
#
#   시나리오 B (sweep_flat_2tier):
#     SWEEP_ROOM      (1.0×) — sweep + room (fvg 상관없음)
#     SWEEP_NO_ROOM   (1.0×) — sweep + NOT room
#
#   시나리오 C (sweep_gem_only):
#     SWEEP_GEM       (1.5×) — sweep + NOT room
#     SWEEP_ROOM      (1.0×) — sweep + room + NOT fvg
#     SWEEP_ROOM_FVG  (1.0×) — sweep + room + fvg
#     (A 와 같은 구조, risk mult 만 다름: GEM 1.5, ROOM_FVG 1.0, ROOM_ONLY 0.5 → GEM 1.5, ROOM 1.0, ROOM_FVG 1.0)
#
# [공통 Tier 유지 (Stage 4G 에서)]
...
#
# [산출물 — 시나리오별 각각]
#   stage4h_<scenario>_out_skip_trades.csv 등
#   scenario_comparison.csv — A/B/C 한 표에 요약
# =========================================================
#
# (아래 주석은 4G 원본 — 참고용 유지)
...
# =========================================================
# ★★★ Stage 4H: 3 SCENARIO COMPARISON ★★★
# =========================================================
# SCENARIO 변수로 A/B/C 선택 — 순차 실행 루프에서 덮어씀
# default 는 "A" 지만 실행부에서 3번 override 해서 돌림

SCENARIO = "A"   # 실행부에서 재설정됨 ("A" | "B" | "C")
...
# Sub-tier 구성:
#   SWEEP_GEM       = sweep + NOT room + not in ALPHA_*  (88t, PF 9.89)
#   SWEEP_ROOM_FVG  = sweep + room + fvg + not in ALPHA_* (303t, PF 2.06)
#   SWEEP_ROOM_ONLY = sweep + room + NOT fvg + not in ALPHA_* (120t, PF 1.16)
SWEEP_RISK_BY_SCENARIO = {
    ...
    "B": {   # 2-Tier 평평 (네 원안)
        "SWEEP_GEM":       1.0,   # = SWEEP_NO_ROOM
        "SWEEP_ROOM_FVG":  1.0,   # room 포함 전체 1.0
        "SWEEP_ROOM_ONLY": 1.0,
    },
    "C": {   # 혼합 (GEM 승격 + 네 원안 room)
        "SWEEP_GEM":       1.5,
        "SWEEP_ROOM_FVG":  1.0,
        "SWEEP_ROOM_ONLY": 1.0,
    },
...
def get_tier_risk_mult_4h(tier_label, side="long"):
    """Tier 별 risk mult — scenario 설정 반영."""
...
def classify_tier_stage4e(atoms_dict):
    return classify_tier_stage4h(atoms_dict, side="long")
def get_tier_risk_mult_4f(tier_label):
    return get_tier_risk_mult_4h(tier_label, side="long")
    ...
    for current_time in event_times:
        ...
        if current_time in entry_map:
            for idx, row in entry_map[current_time]:
                ...
                tier_mult_4e = get_tier_risk_mult_4h(tier_4e, side=_side_4g)
                ...
                if tier_mult_4e <= 0.0:
                    skipped_entries.append({
                        "scenario_name": scenario["name"], "entry_time": row["entry_time"],
                        "symbol": symbol,
                        "reason": f"stage4g_skip_{tier_4e}_out_behavior_{OUT_BEHAVIOR}",
...
print()
print("=" * 75)
print("[Stage 4H] candidates 생성 — SWEEP 세분화 (GEM/ROOM_FVG/ROOM_ONLY)")
print("=" * 75)
...
print(f"  SWEEP_RISK_BY_SCENARIO:")
for sc, mult in SWEEP_RISK_BY_SCENARIO.items():
    print(f"    시나리오 {sc}: {mult}")
...
try:
    candidates_results_stage4e = _parallel_map(
        ...
        task_name="candidates Stage 4H", io_bound=False,
    )
...
print(f"\n  TOTAL candidates (Stage 4H): {total_stage4e}")
print("\n[OK] candidates 생성 완료 (Stage 4H). SCENARIO 루프에서 3번 simulate.")
...
SCENARIO_DESCRIPTIONS = {
    "A": "3-Tier 차등   (GEM 1.5 / ROOM_FVG 1.0 / ROOM_ONLY 0.5)",
    "B": "2-Tier 평평   (GEM 1.0 / ROOM_FVG 1.0 / ROOM_ONLY 1.0)",
    "C": "혼합          (GEM 1.5 / ROOM_FVG 1.0 / ROOM_ONLY 1.0)",
}

# 시나리오별 결과 누적
scenario_results = {}

for SCENARIO_CURRENT in ["A", "B", "C"]:
    # 전역 SCENARIO 를 override — get_tier_risk_mult_4h() 가 이 전역을 읽음
    SCENARIO = SCENARIO_CURRENT
    globals()["SCENARIO"] = SCENARIO_CURRENT   # 안전하게 전역에도 반영
    
    print()
    print("#" * 80)
    print(f"# Stage 4H SCENARIO {SCENARIO_CURRENT}: {SCENARIO_DESCRIPTIONS[SCENARIO_CURRENT]}")
    print(f"#   SWEEP_GEM       × {SWEEP_RISK_BY_SCENARIO[SCENARIO_CURRENT]['SWEEP_GEM']}")
    print(f"#   SWEEP_ROOM_FVG  × {SWEEP_RISK_BY_SCENARIO[SCENARIO_CURRENT]['SWEEP_ROOM_FVG']}")
    print(f"#   SWEEP_ROOM_ONLY × {SWEEP_RISK_BY_SCENARIO[SCENARIO_CURRENT]['SWEEP_ROOM_ONLY']}")
    print("#" * 80)
    
    STAGE4H_CASE = {
        "name":  f"stage4h_scen{SCENARIO_CURRENT}",
        "label": f"Stage 4H [{SCENARIO_CURRENT}]: {SCENARIO_DESCRIPTIONS[SCENARIO_CURRENT]}",
        "risk_mult": 1.0,
    ...
    res = simulate_scenario_v19b_rpboost(
        ...
        candidates_dict=STAGE4H_CASE["candidates"],
        risk_multiplier=STAGE4H_CASE["risk_mult"],
    )
    
    monthly_report = print_scenario_summary(res)
    
    prefix = f"stage4h_scen{SCENARIO_CURRENT}_out_{OUT_BEHAVIOR}"
    ...
    print()
    print("=" * 75)
    print(f"[Stage 4H SCENARIO {SCENARIO_CURRENT}] 전체 성과")
    print("=" * 75)
    ...
    if len(tdf) > 0 and "tier_4e" in tdf.columns:
        ...
        pd.DataFrame(tier_rows).round(4).to_csv(
            f"{OUTDIR}/tier_stage4h_scen{SCENARIO_CURRENT}_distribution.csv", index=False)
        ...
        pd.DataFrame(tx_rows).round(4).to_csv(
            f"{OUTDIR}/tier_x_side_stage4h_scen{SCENARIO_CURRENT}.csv", index=False)
        ...
        pd.DataFrame(rt_rows).round(4).to_csv(
            f"{OUTDIR}/rp_x_tier_stage4h_scen{SCENARIO_CURRENT}.csv", index=False)
...
print()
print("#" * 85)
print("# Stage 4H — 3 시나리오 비교 요약")
print("#" * 85)

comp_df = pd.DataFrame([scenario_results[s] for s in ["A","B","C"]])
comp_df.to_csv(f"{OUTDIR}/scenario_comparison.csv", index=False)

# 한 표로 출력
print()
print(f"{'Scenario':<10} {'trades':>7} {'win%':>7} {'PF':>7} {'avg_R':>7} "
...
for s in ["A", "B", "C"]:
    ...
    print(f"SCEN_{s:<4} {r['trades']:>7} {r['win%']:>6.2f}% {r['PF']:>7.3f} {r['avg_R']:>7.3f} "
          f"{r['MDD%']:>6.2f}% {r['Return_%']:>9.1f}% {months_str:>9} ${r['total_assets_peak']:>12,.0f}")

print()
print("-" * 95)
print("구성 (SWEEP 계열 risk mult):")
for s in ["A", "B", "C"]:
    r = scenario_results[s]
    print(f"  {s}: GEM × {r['sweep_gem_mult']:.1f}, ROOM_FVG × {r['sweep_room_fvg']:.1f}, ROOM_ONLY × {r['sweep_room_only']:.1f}")

print()
print("=" * 75)
print("[Stage 4H] 3 시나리오 비교 완료")
print("=" * 75)
print(f"  산출물: {OUTDIR}/")
print(f"    stage4h_scen{{A,B,C}}_out_skip_*.csv  (각 시나리오 trades/equity/monthly/summary)")
print(f"    tier_stage4h_scen{{A,B,C}}_distribution.csv")
print(f"    tier_x_side_stage4h_scen{{A,B,C}}.csv")
print(f"    rp_x_tier_stage4h_scen{{A,B,C}}.csv")
print(f"    scenario_comparison.csv  ← 3개 나란히 비교")
