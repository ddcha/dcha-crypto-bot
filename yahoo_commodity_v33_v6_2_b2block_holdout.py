...
# ★★★ [PATCHED v33-holdout — Hold-Out WF 분할, 2026-05-22] ★★★
# 학계의 Walk-Forward 중 single hold-out 방식.
# 룰은 코드 그대로(atom search / V6 sub-tier / BOOSTED 다 고정),
# 거래 시기만 train/test 분할해서 OOS PF/MDD 검증.
#
# TRAIN_END_DATE 이전 = Train (룰이 fit됐다고 가정)
# TRAIN_END_DATE 이후 = Test (진짜 OOS)
#
# 원자재는 세금 있어서 MDD 3종 모두 train/test 분리 출력:
#   ① MDD_strategy : equity + excess + tax 복원
#   ② MDD_afterTax : equity + excess (세금만 반영)
#   ③ MDD_actual   : equity (세금+인출 전부 반영)
#
# 기본 분할: 2020~2025.05 / 2025.06~ (yahoo 데이터 기간에 맞춰 자동)
# 환경변수: TRAIN_END_DATE=2024-12-31 python ...
HOLDOUT_TRAIN_END = _os.environ.get("TRAIN_END_DATE", "2025-05-31")
HOLDOUT_ENABLED = _os.environ.get("HOLDOUT_ENABLED", "1") == "1"
print(f"[v33-HOLD-OUT] TRAIN_END_DATE = {HOLDOUT_TRAIN_END}  (이전=train, 이후=test)")
print(f"[v33-HOLD-OUT] ENABLED = {HOLDOUT_ENABLED}")
...
def simulate_portfolio_v25(candidates_packs, max_total_risk=0.1325, scenario_name="BOOST15"):
    ...
    if len(equity_df) > 0:
        # ★★★ [PATCHED — MDD 3종 분리, 2026-05-22] ★★★
        # 기존 total_assets_usd 는 excess(인출)만 복원하고 tax(세금)는 미복원이라
        # 연말 세금 내는 날이 MDD 로 잡혀 실제 전략 손실을 과대평가함.
        # → 3가지 기준으로 분리 계산해서 "세금/인출 vs 진짜 손실" 구분.
        #
        #   1) strategy : equity + excess + tax 전부 복원
        #                 = 세금도 안 내고 인출도 안 했다면? → 순수 전략 손실폭
        #   2) after_tax: equity + excess  (= 기존 total_assets_usd)
        #                 = 세금은 반영, 인출은 복원 → 세금 후 자산 기준
        #   3) actual   : equity 그대로 (= 실제 통장 잔고)
        #                 = 세금 + 인출 전부 빠진 실제 잔고 변동

        # --- (1) 순수 전략 MDD: 세금 + 인출 둘 다 복원 ---
        equity_df["assets_strategy"] = (
            equity_df["equity"]
            + equity_df["cumulative_excess"]
            + equity_df["cumulative_tax"]
        )
        ...
        # --- (2) 세금 후 MDD: 인출만 복원 (기존 방식 = total_assets_usd) ---
        equity_df["total_assets_usd"] = equity_df["equity"] + equity_df["cumulative_excess"]
        equity_df["cummax_after_tax"] = equity_df["total_assets_usd"].cummax()
        equity_df["dd_after_tax"] = equity_df["total_assets_usd"] - equity_df["cummax_after_tax"]
        equity_df["dd_pct_after_tax"] = np.where(
            equity_df["cummax_after_tax"] > 0,
            equity_df["dd_after_tax"] / equity_df["cummax_after_tax"] * 100, 0
        )

        # --- (3) 실제 통장 MDD: 아무것도 복원 안 함 (세금+인출 다 빠진 balance) ---
        ...
        # 하위 호환: 기존 dd/dd_pct 컬럼은 after_tax 기준 유지
        equity_df["cummax"] = equity_df["cummax_after_tax"]
        equity_df["dd"] = equity_df["dd_after_tax"]
        equity_df["dd_pct"] = equity_df["dd_pct_after_tax"]
...
if __name__ == "__main__":
    ...
    if len(tdf) == 0:
        ...
        # ★★★ [PATCHED — MDD 3종, 2026-05-22] ★★★
        mdd_pct = float(edf["dd_pct"].min()) if len(edf) > 0 else 0  # 하위호환(after_tax)
        mdd_strategy = float(edf["dd_pct_strategy"].min()) if len(edf) > 0 and "dd_pct_strategy" in edf else 0
        mdd_after_tax = float(edf["dd_pct_after_tax"].min()) if len(edf) > 0 and "dd_pct_after_tax" in edf else 0
        mdd_actual = float(edf["dd_pct_actual"].min()) if len(edf) > 0 and "dd_pct_actual" in edf else 0
        ...
        summary_row = {
            ...
            "MDD_strategy_%": round(mdd_strategy, 3),   # 순수 전략 (세금/인출 복원)
            "MDD_afterTax_%": round(mdd_after_tax, 3),  # 세금 반영, 인출 복원
            "MDD_actual_%": round(mdd_actual, 3),       # 실제 통장 (세금+인출 다)
            "MDD_%": round(mdd_pct, 3),                 # 하위호환 = afterTax
        }
        ...
        print(f"  ─────────────────────────────────────────────")
        print(f"  📉 MDD 분해 (★ 세금/인출 vs 진짜 손실 구분)")
        print(f"    ① MDD 순수전략    : {mdd_strategy:.2f}%   (세금·인출 둘 다 복원 = 진짜 트레이딩 손실폭)")
        print(f"    ② MDD 세금후      : {mdd_after_tax:.2f}%   (세금 반영, 인출 복원)")
        print(f"    ③ MDD 실제통장    : {mdd_actual:.2f}%   (세금+인출 다 빠진 실잔고 변동)")
        print(f"    → ①과 ③ 차이가 크면 = MDD 대부분이 세금/인출 때문 (손실 아님)")
        print(f"  ─────────────────────────────────────────────")

        # ★★★ [PATCHED v33-holdout — Hold-Out WF 분할 출력, 2026-05-22] ★★★
        if HOLDOUT_ENABLED and len(tdf) > 0 and "exit_time" in tdf.columns:
            try:
                train_end_ts = pd.Timestamp(HOLDOUT_TRAIN_END, tz="UTC")

                # trades 분할 (exit_time 기준)
                tr_t = pd.to_datetime(tdf["exit_time"], utc=True, errors="coerce")
                train_trades = tdf[tr_t <= train_end_ts]
                test_trades = tdf[tr_t > train_end_ts]

                # equity 분할 (time 기준)
                eq_t = pd.to_datetime(edf["time"], utc=True, errors="coerce") if len(edf) > 0 else None
                if eq_t is not None and len(eq_t) > 0:
                    eq_train = edf[eq_t <= train_end_ts]
                    eq_test = edf[eq_t > train_end_ts]
                else:
                    eq_train = pd.DataFrame(); eq_test = pd.DataFrame()

                def _ho_stats(seg_trades, seg_eq, label):
                    """구간별 PF/WR/Avg_R/MDD(3종) 계산. equity 시리즈 기준으로 MDD를 재 cummax."""
                    n = len(seg_trades)
                    if n == 0:
                        return dict(label=label, n=0, wr=np.nan, pf=np.nan, avg_r=np.nan,
                                    mdd_strat=np.nan, mdd_aftertax=np.nan, mdd_actual=np.nan, total_r=np.nan)
                    pnls = seg_trades["net_pnl"]
                    wins = pnls[pnls > 0]; losses = pnls[pnls <= 0]
                    gw = float(wins.sum()); gl = float(abs(losses.sum()))
                    pf = gw / gl if gl > 0 else float("inf")
                    wr = len(wins) / n * 100
                    avg_r = float(seg_trades["r_multiple"].mean()) if "r_multiple" in seg_trades.columns else np.nan
                    total_r = float(seg_trades["r_multiple"].sum()) if "r_multiple" in seg_trades.columns else np.nan

                    # 구간 시작 시점을 peak 0 으로 리셋해서 MDD 재계산 (3종)
                    mdd_s = np.nan; mdd_at = np.nan; mdd_a = np.nan
                    if len(seg_eq) > 0:
                        s = seg_eq.copy()
                        if "assets_strategy" in s.columns:
                            s["_cmx"] = s["assets_strategy"].cummax()
                            s["_dd"] = np.where(s["_cmx"] > 0,
                                                (s["assets_strategy"] - s["_cmx"]) / s["_cmx"] * 100, 0)
                            mdd_s = float(s["_dd"].min())
                        if "total_assets_usd" in s.columns:
                            s["_cmx2"] = s["total_assets_usd"].cummax()
                            s["_dd2"] = np.where(s["_cmx2"] > 0,
                                                 (s["total_assets_usd"] - s["_cmx2"]) / s["_cmx2"] * 100, 0)
                            mdd_at = float(s["_dd2"].min())
                        if "equity" in s.columns:
                            s["_cmx3"] = s["equity"].cummax()
                            s["_dd3"] = np.where(s["_cmx3"] > 0,
                                                 (s["equity"] - s["_cmx3"]) / s["_cmx3"] * 100, 0)
                            mdd_a = float(s["_dd3"].min())

                    return dict(label=label, n=n, wr=wr, pf=pf, avg_r=avg_r,
                                mdd_strat=mdd_s, mdd_aftertax=mdd_at, mdd_actual=mdd_a, total_r=total_r)

                train_s = _ho_stats(train_trades, eq_train, "TRAIN")
                test_s = _ho_stats(test_trades, eq_test, "TEST")
                full_s = _ho_stats(tdf, edf, "FULL")

                print(f"  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
                print(f"  📊 HOLD-OUT WF 분석 (TRAIN_END_DATE = {HOLDOUT_TRAIN_END})")
                print(f"  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
                print(f"  구간   거래  승률   PF     Avg_R   총R    MDD순수  MDD세금후  MDD실제")
                for s in [train_s, test_s, full_s]:
                    if s["n"] == 0:
                        print(f"  {s['label']:<6} {s['n']:>4}  (no trades in segment)")
                    else:
                        def fmt(v, w=6, p=2, suf="%"):
                            return f"{v:>{w}.{p}f}{suf}" if pd.notna(v) else f"{'N/A':>{w+1}}"
                        print(f"  {s['label']:<6} {s['n']:>4}  {s['wr']:>4.1f}% "
                              f"{s['pf']:>5.2f}  {s['avg_r']:>5.2f}  {s['total_r']:>6.1f}  "
                              f"{fmt(s['mdd_strat'])}  {fmt(s['mdd_aftertax'])}  {fmt(s['mdd_actual'])}")
                print(f"  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")

                # 강건성 진단
                if train_s["n"] > 0 and test_s["n"] > 0 and pd.notna(train_s["pf"]) and pd.notna(test_s["pf"]):
                    pf_drop = (train_s["pf"] - test_s["pf"]) / train_s["pf"] * 100 if train_s["pf"] > 0 else 0
                    wr_drop = train_s["wr"] - test_s["wr"]
                    if pf_drop < 10 and abs(wr_drop) < 5:
                        print(f"  ✅ 강건성 양호: TEST PF가 TRAIN 대비 -{pf_drop:.1f}% (10% 이내)")
                        print(f"     → 원자재 v33 룰이 OOS에서도 유지됨. 진짜 alpha 가능성 높음.")
                        print(f"     → 한투 실전 진입 적합 (BOOSTED는 시드 작게부터)")
                    elif pf_drop < 30:
                        print(f"  🟡 약간 감소: TEST PF가 TRAIN 대비 -{pf_drop:.1f}%")
                        print(f"     → 부분 in-sample fit. atom search 룰 일부 과거 fit 가능성.")
                        print(f"     → 실전 적용 시 AGGRESSIVE(S=2.5배)부터 권장")
                    else:
                        print(f"  ⚠️  큰 감소: TEST PF가 TRAIN 대비 -{pf_drop:.1f}%")
                        print(f"     → in-sample 과최적화 강하게 의심.")
                        print(f"     → BOOSTED는 위험. FLAT 시나리오 재검증 + 룰 단순화 필요")
                    print(f"  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")

                # CSV 저장 (시나리오 + TF suffix 모두 포함, 파일명 충돌 방지)
                holdout_df = pd.DataFrame([train_s, test_s, full_s])
                holdout_csv = outdir / f"v33_{V6_SCENARIO}{_TF_SUFFIX}_holdout_summary.csv"
                holdout_df.to_csv(holdout_csv, index=False)
                print(f"  💾 hold-out 결과 저장: {holdout_csv.name}")
            except Exception as e:
