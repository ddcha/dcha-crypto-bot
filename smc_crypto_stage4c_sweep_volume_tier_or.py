# =========================================================
# smc_crypto_9coins  STAGE 4C: TIER + (SWEEP OR VOLUME) — OR GATE
#
# [실험 목적]
#   Stage 3 의 기존 진입 엔진(Tier + RP_BOOST + S/A cap FREE)을 그대로 복원하면서,
#   Stage 4A 에서 검증된 alpha (SWEEP, VOLUME) 를 게이트의 일원으로 승격.
#
#   Tier 엔진 / SWEEP / VOLUME 3개가 OR 로 묶여 작동 →
#   3개 중 하나라도 통과하면 진입. 모두 실패일 때만 skip.
#
#   D1_SELF / BTC_TREND 는 Stage 4A 에서 역효과로 판명 → 완전 제거.
#
# [진입 조건]
#   eff_score ≥ MIN_SCORE(7.5)   AND
#   (tier != "D"  OR  pass_sweep  OR  pass_volume)
#
# [Risk 분기]
#   tier ∈ {S, A, B, C}:
#     risk_pct = risk_pct_base × tier_mult × risk_multiplier
#     tier S/A  → notional cap 999  (S/A cap FREE 복원)
#     tier B/C  → notional cap 3.0
#   tier == "D" + (sweep OR volume) 으로 진입:
#     risk_pct = risk_pct_base × 1.0 × risk_multiplier   (기본 risk)
#     notional cap 3.0
#
# [복원 (Stage 3 Case C 와 동일)]
#   - classify_tier_v19b_rp_boost 호출 복원
#   - RP_TIER_MULT_TABLE 곱 (RP_BOOST)
#   - WICK_RATIO_5_Q1_THRESHOLD 0.2300
#   - TIER_RISK_S/A/B/C
#   - S/A cap FREE
#   - get_run_potential / classify_grade
#   - LONG/SHORT BASE_ENTRY_FRAC 0.10/0.90 (REFINE 제거)
#   - 단일 포지션, 실전 근사 fill, H4 exit, 병렬화
#
# [신규 tag 컬럼]
#   tag_sweep        : SWEEP 통과 여부
#   tag_volume       : VOLUME 통과 여부
#   tag_tier_ok      : tier != "D"  (tier 엔진 통과)
#   tag_entry_basis  : 진입 근거 조합 ("tier+sweep+volume", "sweep+volume", "tier", "sweep" 등 8가지)
#
# [제거 — Stage 4A 잔재]
#   - D1_SELF / BTC_TREND 태그 및 관련 계산 함수
#   - BTC D1 trend 사전 로드 블록
#   - set_btc_d1_trend_cache / get_btc_d1_trend_cache
#
# [분석 산출물 7개]
#   1) stage4c_overall_summary.csv
#   2) entry_basis_solo.csv      : tier_ok / sweep / volume 각각 PASS vs FAIL
#   3) entry_basis_combo.csv     : 8가지 조합 (2^3) 성과
#   4) tier_distribution.csv     : S/A/B/C tier별 trades/Win%/PF
#   5) side_x_basis_combo.csv    : LONG/SHORT × 8조합 (핵심)
#   6) side_x_tier.csv           : LONG/SHORT × tier
#   7) rp_boost_effect.csv       : RP0~RP5 × LONG/SHORT
# =========================================================
...
# =========================================================
# ★★★ Stage 4C: SWEEP / VOLUME 태그 계산 ★★★
# =========================================================
# Stage 4A 의 D1_SELF / BTC_TREND 태그는 제거 (역효과 확인됨)
# SWEEP / VOLUME 은 실제 진입 게이트로도 사용됨.
def compute_trade_tags(df_struct, entry_idx, zone_created_idx,
                       zone_low, zone_high, side):
    """
    SWEEP / VOLUME 2개 태그 계산.
    ...
    Returns:
        dict: { "tag_sweep": bool, "tag_volume": bool }
    """
    tags = {}
...
def generate_candidates_from_prepared(prepared):
    ...
    while i < len(df_struct) - 1:
        ...
        for s, eff_score, fresh_reasons in scored_active:
            ...
            # ⭐⭐ Stage 4C: SWEEP / VOLUME 태그 계산 (Tier OR 게이트 판정은 simulate 단계에서)
            ...
            # 하위 호환: 기존 _filter_passes / _pass_combo / _pass_count 유지
            _filter_passes = {
                "pass_vol":   _tags["tag_volume"],
                "pass_sweep": _tags["tag_sweep"],
                "pass_d1":    False,   # Stage 4C 에서 제거
                "pass_pull":  False,
                "pass_atr":   False,
            }
            _passed_names = []
            if _tags["tag_sweep"]:  _passed_names.append("SWEEP")
            if _tags["tag_volume"]: _passed_names.append("VOLUME")
        ...
        candidates.append({
            ...
            # ⭐⭐ Stage 4C: 2개 태그 기록 (SWEEP / VOLUME)
    ...
    for current_time in event_times:
        ...
        if current_time in entry_map:
            for idx, row in entry_map[current_time]:
                ...
                # ⭐⭐ Stage 4C: Tier 시스템 완전 복원 (Stage 3 Case C 와 동일)
                #   + SWEEP / VOLUME 태그와 OR 로 묶어서 진입 판단
                ...
                tier_label, tier_mult_raw, rp_action = classify_tier_v19b_rp_boost(
                    pre_total_v, sweep_v, score_v, wick_v, rp_v
                )

                # Stage 4C: OR 게이트 판정
                #   tier_ok     = (tier != "D")  — 기존 엔진 통과
                #   sweep_ok    = tag_sweep      — Stage 4A 에서 검증된 alpha
                #   volume_ok   = tag_volume     — Stage 4A 에서 검증된 alpha
                #   셋 중 하나라도 통과하면 진입 허용
                tier_ok    = (tier_label != "D") and (tier_mult_raw > 0.0)
                sweep_ok   = bool(row.get("tag_sweep", False))
                volume_ok  = bool(row.get("tag_volume", False))

                if not (tier_ok or sweep_ok or volume_ok):
                    skipped_entries.append({
                        "scenario_name": scenario["name"], "entry_time": row["entry_time"],
                        "symbol": symbol,
                        "reason": f"or_gate_fail_tier{tier_label}_sweep{sweep_ok}_vol{volume_ok}",
                        "tier_label": tier_label,
                        "rp_action": rp_action,
                        "run_potential": rp_v,
                        "tag_sweep": sweep_ok,
                        "tag_volume": volume_ok,
                ...
                # 진입 근거 조합 문자열
                _basis_names = []
                if tier_ok:   _basis_names.append("tier")
                if sweep_ok:  _basis_names.append("sweep")
                if volume_ok: _basis_names.append("volume")
                tag_entry_basis = "+".join(sorted(_basis_names))

                # Risk 계산 분기
                if tier_ok:
                    # Tier 엔진 경로: 기존 방식 그대로 (RP_BOOST 포함)
                    tier_mult = tier_mult_raw
                    risk_pct  = risk_pct_base * tier_mult * risk_multiplier
                    # S/A cap FREE 복원 (Stage 3 Case C 와 동일)
                    if tier_label in ("S", "A"):
                        effective_notional_cap = 999.0
                        sa_cap_tag = "saFree"
                    else:
                        effective_notional_cap = MAX_NOTIONAL_MULT  # 3.0
                        sa_cap_tag = "capNorm"
                else:
                    # Tier D 인데 SWEEP or VOLUME 으로 진입한 경우
                    # → 근거가 약하므로 기본 risk 적용, RP_BOOST 없음, cap 고정 3.0
                    tier_mult = 1.0
                    risk_pct  = risk_pct_base * 1.0 * risk_multiplier
                    effective_notional_cap = MAX_NOTIONAL_MULT  # 3.0
                    sa_cap_tag = "capTierD"
                ...
                open_positions[idx] = {
                    ...
                    # ⭐⭐ Stage 4C: 진입 근거 태그 (분석의 본체)
                    "tag_sweep":       bool(sweep_ok),
                    "tag_volume":      bool(volume_ok),
                    "tag_tier_ok":     bool(tier_ok),
                    "tag_entry_basis": tag_entry_basis,
                    # 하위 호환 (기존 CSV 스키마 유지)
...
# =========================================================
# ★★★ Stage 4C: candidates 생성 (단일 버전 — Tier OR SWEEP OR VOLUME) ★★★
# =========================================================
print()
print("=" * 75)
print("[Stage 4C] candidates 생성 — Tier + (SWEEP OR VOLUME) OR gate")
print("=" * 75)
print("  진입 조건 = MIN_SCORE 7.5  AND  (tier!=D  OR  sweep  OR  volume)")
print("  D1_SELF / BTC_TREND 태그는 Stage 4A 분석 결과 제거됨 (역효과)")

USE_FILTER_GROUPS = False    # FILTER_GROUPS 비활성
USE_VOLUME_FILTER = False    # 모든 진입 필터 개별 OFF (OR 게이트는 simulate 단계에서)
USE_LIQUIDITY_SWEEP_CONF = False
USE_D1_TREND_FILTER = False
USE_PULLBACK_DEPTH = False
USE_ATR_FILTER = False
...
try:
    candidates_results_stage4c = _parallel_map(
        _generate_candidates_single, SYMBOLS_TO_PREPARE,
        task_name="candidates Stage 4C", io_bound=False,
    )
    candidates_dict_stage4c = dict(candidates_results_stage4c)
except Exception as e:
    print(f"  candidates 병렬 실패: {e}, 순차 처리")
    candidates_dict_stage4c = {}
    for sym in SYMBOLS_TO_PREPARE:
        candidates_dict_stage4c[sym] = generate_candidates_from_prepared(prepared_data[sym])

total_stage4c = sum(len(candidates_dict_stage4c[s]["candidates"]) for s in SYMBOLS_TO_PREPARE)
print(f"\n  TOTAL candidates (Stage 4C 원본, OR 게이트 적용 전): {total_stage4c}")
print("\n[OK] candidates 생성 완료 (Stage 4C). OR 게이트 / Tier / Risk 는 simulate 단계에서 적용됨.")


# =========================================================
# ★★★ 셀 3: 단일 시나리오 백테스트 실행 (Stage 4C) ★★★
# =========================================================
import os
OUTDIR = "stage4c_outputs"
os.makedirs(OUTDIR, exist_ok=True)
print(f"\n[OUTDIR] {os.path.abspath(OUTDIR)}")

STAGE4C_CASE = {
    "name":  "stage4c",
    "label": "Stage 4C: Tier + (SWEEP OR VOLUME) OR gate",
    "risk_mult": 1.0,
    "candidates": candidates_dict_stage4c,
}

print()
print("#" * 75)
print(f"# Stage 4C 백테스트 시작: {STAGE4C_CASE['label']}")
print("#" * 75)

res = simulate_scenario_v19b_rpboost(
    scenario=SCENARIO_MULTI,
    candidates_dict=STAGE4C_CASE["candidates"],
    risk_multiplier=STAGE4C_CASE["risk_mult"],
)
...
summary_df = pd.DataFrame([{
    "scenario":  STAGE4C_CASE["name"],
    "label":     STAGE4C_CASE["label"],
    "trades":    len(trades),
    "win%":      wr_overall,
    "PF":        pf_overall,
    "avg_R":     avgr_overall,
    "MDD%":      mdd_overall,
    "Total_KRW_100M":  final_total_krw / 1e8,
    "Excess_KRW_100M": final_excess_krw / 1e8,
    "Return_%":  return_pct,
    "retirement":      retirement_month,
    "retirement_months": months_taken if months_taken else "X",
}])
summary_df.to_csv(f"{OUTDIR}/stage4c_overall_summary.csv", index=False)

print()
print("=" * 75)
print("[Stage 4C] 전체 성과")
print("=" * 75)
...
# =========================================================
# ★★★ Stage 4C: 심층 분석 (7개 산출물) ★★★
# =========================================================
print()
print("#" * 75)
print("# Stage 4C 심층 분석 — Tier / SWEEP / VOLUME 교차분석")
print("#" * 75)
...
if len(tdf) == 0:
    ...
    # 필수 컬럼 존재 보장
    for c in ["tag_sweep", "tag_volume", "tag_tier_ok", "tag_entry_basis", "tier", "run_potential"]:
        if c not in tdf.columns:
            tdf[c] = False if c.startswith("tag_") else ""
    ...
    # ─────────────────────────────────────────────
    # (1) entry_basis_solo.csv — 각 게이트 단독 효과 (PASS vs FAIL)
    # ─────────────────────────────────────────────
    print()
    print("=" * 75)
    print("[1] entry_basis_solo — Tier / SWEEP / VOLUME 각각 PASS vs FAIL")
    print("=" * 75)
    print(f"{'gate':<14} {'status':<6} {'trades':>7} {'win%':>7} {'avg_R':>7} {'PF':>7} {'total $':>14}")
    print("-" * 75)
    ...
    for c, gate_name in [
        ...
        for status, sub in [("PASS", tdf[tdf[c] == True]),
            ...
            st = _stats(sub)
            solo_rows.append({"gate": gate_name, "status": status, **st})
            print(f"{gate_name:<14} {status:<6} {st['trades']:>7} "
                  f"{st['win_pct']:>6.2f}% {st['avg_R']:>7.3f} {st['PF']:>7.3f} "
                  f"{st['total_pnl']:>14,.0f}")

    solo_df = pd.DataFrame(solo_rows).round(4)
    solo_df.to_csv(f"{OUTDIR}/entry_basis_solo.csv", index=False)

    # ─────────────────────────────────────────────
    # (2) entry_basis_combo.csv — 8가지 조합 (2^3)
    # ─────────────────────────────────────────────
    print()
    print("=" * 75)
    print("[2] entry_basis_combo — 8가지 진입 근거 조합 성과")
    print("=" * 75)

    combo_stat = tdf.groupby("tag_entry_basis").agg(
        trades=("symbol", "count"),
    ...
    combo_stat["PF"] = combo_stat["tag_entry_basis"].apply(
        lambda v: _pf_of(tdf[tdf["tag_entry_basis"] == v])
    )
    combo_stat = combo_stat.sort_values("total_pnl", ascending=False).reset_index(drop=True)
    combo_stat = combo_stat.round({"win_pct":2, "avg_R":3, "total_pnl":2,
                                    "pnl_per_trade":2, "PF":3})

    print(f"{'entry_basis':<30} {'trades':>7} {'win%':>7} {'avg_R':>7} {'PF':>7} {'$/trade':>10} {'total $':>14}")
    print("-" * 95)
    for _, r in combo_stat.iterrows():
        cstr = r["tag_entry_basis"][:28] if len(r["tag_entry_basis"]) > 28 else r["tag_entry_basis"]
        print(f"{cstr:<30} {int(r['trades']):>7} {r['win_pct']:>6.2f}% "
              f"{r['avg_R']:>7.3f} {r['PF']:>7.3f} "
              f"{r['pnl_per_trade']:>10,.0f} {r['total_pnl']:>14,.0f}")

    combo_stat.to_csv(f"{OUTDIR}/entry_basis_combo.csv", index=False)

    # ─────────────────────────────────────────────
    # (3) tier_distribution.csv — Tier S/A/B/C/D 분포 + 성과
    # ─────────────────────────────────────────────
    print()
    print("=" * 75)
    print("[3] tier_distribution — Tier 등급별 성과")
    print("=" * 75)

    tier_stat = tdf.groupby("tier").agg(
        trades=("symbol", "count"),
        win_pct=("net_pnl", lambda s: (s > 0).mean() * 100),
        avg_R=("r_multiple", "mean"),
        total_pnl=("net_pnl", "sum"),
    ...
    tier_stat["PF"] = tier_stat["tier"].apply(lambda t: _pf_of(tdf[tdf["tier"] == t]))
    # 순서: S > A > B > C > D > N/A
    _tier_order = {"S":0, "A":1, "B":2, "C":3, "D":4, "N/A":5}
    tier_stat["_ord"] = tier_stat["tier"].map(lambda t: _tier_order.get(t, 99))
    tier_stat = tier_stat.sort_values("_ord").drop(columns=["_ord"]).reset_index(drop=True)
    tier_stat = tier_stat.round({"win_pct":2, "avg_R":3, "total_pnl":2,
                                  "pnl_per_trade":2, "PF":3})

    print(f"{'tier':<6} {'trades':>7} {'win%':>7} {'avg_R':>7} {'PF':>7} {'$/trade':>10} {'total $':>14}")
    print("-" * 75)
    for _, r in tier_stat.iterrows():
        print(f"{r['tier']:<6} {int(r['trades']):>7} {r['win_pct']:>6.2f}% "
              f"{r['avg_R']:>7.3f} {r['PF']:>7.3f} "
              f"{r['pnl_per_trade']:>10,.0f} {r['total_pnl']:>14,.0f}")

    tier_stat.to_csv(f"{OUTDIR}/tier_distribution.csv", index=False)

    # ─────────────────────────────────────────────
    # (4) side_x_basis_combo.csv — LONG/SHORT × 8조합 (핵심)
    # ─────────────────────────────────────────────
    print()
    print("=" * 75)
    print("[4] side_x_basis_combo — LONG/SHORT x entry_basis 교차 (CORE)")
    print("=" * 75)

    sxb_rows = []
    for side_val in ("long", "short"):
        side_sub = tdf[tdf["side"] == side_val]
        sides = side_sub.groupby("tag_entry_basis").agg(
            trades=("symbol", "count"),
            win_pct=("net_pnl", lambda s: (s > 0).mean() * 100),
            avg_R=("r_multiple", "mean"),
            total_pnl=("net_pnl", "sum"),
        ...
        sides["PF"] = sides["tag_entry_basis"].apply(
            lambda v: _pf_of(side_sub[side_sub["tag_entry_basis"] == v])
        )
        sides["side"] = side_val
        sxb_rows.append(sides)

    sxb_df = pd.concat(sxb_rows, ignore_index=True) if sxb_rows else pd.DataFrame()
    if len(sxb_df) > 0:
        sxb_df = sxb_df[["side","tag_entry_basis","trades","win_pct","avg_R","PF",
                          "pnl_per_trade","total_pnl"]].sort_values(
            ["side","total_pnl"], ascending=[True, False]
        )
        sxb_df = sxb_df.round({"win_pct":2, "avg_R":3, "total_pnl":2,
                                "pnl_per_trade":2, "PF":3})
        sxb_df.to_csv(f"{OUTDIR}/side_x_basis_combo.csv", index=False)

        print(f"{'side':<6} {'entry_basis':<28} {'trades':>7} {'win%':>7} {'PF':>7} {'total $':>14}")
        print("-" * 85)
        for _, r in sxb_df.iterrows():
            cstr = r["tag_entry_basis"][:26] if len(r["tag_entry_basis"]) > 26 else r["tag_entry_basis"]
            print(f"{r['side']:<6} {cstr:<28} {int(r['trades']):>7} "
                  f"{r['win_pct']:>6.2f}% {r['PF']:>7.3f} {r['total_pnl']:>14,.0f}")

    # ─────────────────────────────────────────────
    # (5) side_x_tier.csv — LONG/SHORT × tier
    # ─────────────────────────────────────────────
    print()
    print("=" * 75)
    print("[5] side_x_tier — LONG/SHORT x Tier 등급")
    print("=" * 75)

    sxt_rows = []
    for side_val in ("long", "short"):
        side_sub = tdf[tdf["side"] == side_val]
        t_stat = side_sub.groupby("tier").agg(
            trades=("symbol", "count"),
            win_pct=("net_pnl", lambda s: (s > 0).mean() * 100),
            avg_R=("r_multiple", "mean"),
            total_pnl=("net_pnl", "sum"),
        ...
        t_stat["PF"] = t_stat["tier"].apply(lambda t: _pf_of(side_sub[side_sub["tier"] == t]))
        t_stat["side"] = side_val
        sxt_rows.append(t_stat)

    sxt_df = pd.concat(sxt_rows, ignore_index=True) if sxt_rows else pd.DataFrame()
    if len(sxt_df) > 0:
        sxt_df["_ord"] = sxt_df["tier"].map(lambda t: _tier_order.get(t, 99))
        sxt_df = sxt_df.sort_values(["side","_ord"]).drop(columns=["_ord"])
        sxt_df = sxt_df[["side","tier","trades","win_pct","avg_R","PF",
                          "pnl_per_trade","total_pnl"]]
        sxt_df = sxt_df.round({"win_pct":2, "avg_R":3, "total_pnl":2,
                                "pnl_per_trade":2, "PF":3})
        sxt_df.to_csv(f"{OUTDIR}/side_x_tier.csv", index=False)

        print(f"{'side':<6} {'tier':<6} {'trades':>7} {'win%':>7} {'PF':>7} {'total $':>14}")
        print("-" * 75)
        for _, r in sxt_df.iterrows():
            print(f"{r['side']:<6} {r['tier']:<6} {int(r['trades']):>7} "
                  f"{r['win_pct']:>6.2f}% {r['PF']:>7.3f} {r['total_pnl']:>14,.0f}")

    # ─────────────────────────────────────────────
    # (6) rp_boost_effect.csv — run_potential × LONG/SHORT
    # ─────────────────────────────────────────────
    print()
    print("=" * 75)
    print("[6] rp_boost_effect — run_potential (RP0~RP5) x LONG/SHORT")
    print("=" * 75)

    rp_rows = []
    for side_val in ("long", "short"):
        side_sub = tdf[tdf["side"] == side_val]
        for rp_v in sorted(side_sub["run_potential"].dropna().unique().tolist()):
            sub = side_sub[side_sub["run_potential"] == rp_v]
            st = _stats(sub)
            rp_rows.append({"side": side_val, "run_potential": int(rp_v), **st})

    # 전체 (side 무관)
    for rp_v in sorted(tdf["run_potential"].dropna().unique().tolist()):
        sub = tdf[tdf["run_potential"] == rp_v]
        st = _stats(sub)
        rp_rows.append({"side": "ALL", "run_potential": int(rp_v), **st})

    rp_df = pd.DataFrame(rp_rows).round(4)
    if len(rp_df) > 0:
        rp_df = rp_df[["side","run_potential","trades","win_pct","avg_R","PF",
                        "pnl_per_trade","total_pnl"]].sort_values(["side","run_potential"])
        rp_df.to_csv(f"{OUTDIR}/rp_boost_effect.csv", index=False)

        print(f"{'side':<6} {'RP':<4} {'trades':>7} {'win%':>7} {'PF':>7} {'total $':>14}")
        print("-" * 75)
        for _, r in rp_df.iterrows():
            print(f"{r['side']:<6} {int(r['run_potential']):<4} {int(r['trades']):>7} "
                  f"{r['win_pct']:>6.2f}% {r['PF']:>7.3f} {r['total_pnl']:>14,.0f}")


print()
print("=" * 75)
print("[SAVED] Stage 4C 분석 산출물")
print("=" * 75)
print(f"  {OUTDIR}/stage4c_trades.csv")
print(f"  {OUTDIR}/stage4c_equity.csv")
print(f"  {OUTDIR}/stage4c_skipped.csv")
print(f"  {OUTDIR}/stage4c_monthly.csv")
print(f"  {OUTDIR}/stage4c_overall_summary.csv")
print(f"  {OUTDIR}/entry_basis_solo.csv")
print(f"  {OUTDIR}/entry_basis_combo.csv")
print(f"  {OUTDIR}/tier_distribution.csv")
print(f"  {OUTDIR}/side_x_basis_combo.csv")
print(f"  {OUTDIR}/side_x_tier.csv")
print(f"  {OUTDIR}/rp_boost_effect.csv")

print()
print("=" * 75)
print("[Stage 4C] 실험 완료")
print("=" * 75)
print("  설정: MIN_SCORE 7.5 AND (tier!=D OR sweep OR volume)")
print("        Tier + RP_BOOST + S/A cap FREE 전부 복원")
print("        D1_SELF / BTC_TREND 완전 제거 (Stage 4A 에서 역효과)")
print("  핵심 산출물: entry_basis_combo.csv + side_x_basis_combo.csv")
