# =========================================================
# smc_crypto_9coins  STAGE 4G: SWEEP_BASE RESCUE + ALPHA_MED SIDE-AWARE + RP0 2.0x
#
# [Stage 4F → 4G 변경점]
...
# =========================================================
# ★★★ Stage 4G: SWEEP_BASE + ALPHA_MED SIDE-AWARE + RP0 2.0x ★★★
# =========================================================
# 4F → 4G 변경:
#   (1) SWEEP_BASE Tier 신설 (BASE 중 a_sweep 포함, risk × 1.0)
#   (2) ALPHA_MED LONG risk 낮춤 (0.8 vs SHORT 1.2)
#   (3) RP0 boost 1.5 → 2.0
#   (4) COMPLETE_OUT 만 skip (alpha 전혀 없는 415건)

# ─── Tier risk multiplier (4G) ───
TIER_RISK_MULT_4G = {
    "ALPHA_MAX":    2.0,
    "ALPHA_HIGH":   1.5,
    "ALPHA_MED":    1.2,   # default, side-aware 적용시 덮어씀
    "SWEEP_BASE":   1.0,   # 신규 Tier
    "COMPLETE_OUT": 1.0,   # OUT_BEHAVIOR 에 따라 (default "skip")
    "SKIP_MSS":     0.0,
}

# ─── Side-aware 추가 설정 (ALPHA_MED LONG 낮춤) ───
# ALPHA_MED LONG PF 1.46 / SHORT PF 1.55 — 미세한 차이지만 avg_R 은 LONG 0.30 vs SHORT 0.47 로 큼
ALPHA_MED_RISK_LONG  = 0.8   # LONG 은 보수적
ALPHA_MED_RISK_SHORT = 1.2   # SHORT 은 기본 (더 강함)

# ─── RP mult (RP0 가산 강화) ───
RP_MULT_4G = {
    0: 2.0,    # 기존 1.5 → 2.0 (RP0 전용 가산)
    1: 1.0, 2: 1.0, 3: 1.0, 4: 1.0, 5: 1.0,
}

# ─── OUT 처리 ───
# "skip"     : COMPLETE_OUT 진입 안 함 (default)
# "keep_r1"  : 모든 OUT 복구, risk × 1.0
# "keep_r05" : risk × 0.5
OUT_BEHAVIOR = "skip"

# ─── Notional cap ───
TIER_CAP_4G = {
    "ALPHA_MAX":    999.0,   # cap FREE
    "ALPHA_HIGH":   999.0,   # cap FREE
    "ALPHA_MED":    3.0,
    "SWEEP_BASE":   3.0,
    "COMPLETE_OUT": 3.0,
    "SKIP_MSS":     3.0,
...
def classify_tier_stage4g(atoms_dict, side="long"):
    """
    12 atomic + side 로 Stage 4G Tier 판정.
    
    Tier 체계:
      ALPHA_MAX     (2.0x): overlap-only 또는 score+p1-only
      ALPHA_HIGH    (1.5x): volume+overlap+보조 또는 sweep+volume
      ALPHA_MED     (LONG 0.8 / SHORT 1.2): union(319 canonical) 위 미해당
      SWEEP_BASE    (1.0x):  BASE 중 a_sweep 포함 (신규)
      COMPLETE_OUT  (skip):  alpha 전혀 없음
      SKIP_MSS     (0):      a_mss 단독
    """
    ...
    # ALPHA_MED: 319 canonical union 매칭, 위 미해당
    if passes_win69_union(atoms_dict):
        return "ALPHA_MED"
    
    # SWEEP_BASE: BASE 중 a_sweep 포함 (신규)
    if swp:
        return "SWEEP_BASE"
    
    # COMPLETE_OUT: alpha 전혀 없음
    return "COMPLETE_OUT"


def get_tier_risk_mult_4g(tier_label, side="long"):
    """Tier + side 별 risk mult 반환."""
    ...
    if tier_label == "ALPHA_MED":
        if side == "long":
            return ALPHA_MED_RISK_LONG
        else:
            return ALPHA_MED_RISK_SHORT
    return TIER_RISK_MULT_4G.get(tier_label, 1.0)


def get_tier_cap_4g(tier_label):
    return TIER_CAP_4G.get(tier_label, 3.0)


# ─── 하위 호환 alias (simulate 내부가 4F/4E 이름 호출 중) ───
TIER_RISK_MULT_4F = TIER_RISK_MULT_4G
TIER_RISK_MULT_4E = TIER_RISK_MULT_4G
RP_MULT_4F = RP_MULT_4G
RP_MULT_4E = RP_MULT_4G

# classify / get_mult 는 side 파라미터 때문에 wrapper 필요
def classify_tier_stage4f(atoms_dict):
    # side 없이 호출되면 long 으로 기본 — 실제 호출 지점에서 side 가공 필요
    return classify_tier_stage4g(atoms_dict, side="long")

def classify_tier_stage4e(atoms_dict):
    return classify_tier_stage4g(atoms_dict, side="long")

def get_tier_risk_mult_4f(tier_label):
    return get_tier_risk_mult_4g(tier_label, side="long")

def get_tier_risk_mult_4e(tier_label):
    return get_tier_risk_mult_4g(tier_label, side="long")

get_tier_cap_4f = get_tier_cap_4g
get_tier_cap_4e = get_tier_cap_4g
    ...
    for current_time in event_times:
        ...
        if current_time in entry_map:
            for idx, row in entry_map[current_time]:
                ...
                _side_4g = row["side"]
                tier_4e = classify_tier_stage4g(_atoms_final, side=_side_4g)
                tier_mult_4e = get_tier_risk_mult_4g(tier_4e, side=_side_4g)
...
# =========================================================
# ★★★ Stage 4G: candidates 생성 (SWEEP_BASE 신설) ★★★
# =========================================================
print()
print("=" * 75)
print("[Stage 4G] candidates 생성 — SWEEP_BASE + ALPHA_MED side-aware + RP0 2.0x")
print("=" * 75)
print("  진입 조건 = MIN_SCORE 7.5 만 (Tier 판정은 simulate 단계)")
print(f"  OUT_BEHAVIOR = {OUT_BEHAVIOR!r}")
print(f"  TIER_RISK_MULT_4G = {TIER_RISK_MULT_4G}")
print(f"  RP_MULT_4G = {RP_MULT_4G}")
print(f"  ALPHA_MED side-aware: LONG={ALPHA_MED_RISK_LONG}, SHORT={ALPHA_MED_RISK_SHORT}")
print(f"  Tier 체계: ALPHA_MAX(2.0) > ALPHA_HIGH(1.5) > ALPHA_MED(LONG 0.8/SHORT 1.2) > SWEEP_BASE(1.0) > COMPLETE_OUT")
...
try:
    candidates_results_stage4e = _parallel_map(
        ...
        task_name="candidates Stage 4G", io_bound=False,
    )
...
print(f"\n  TOTAL candidates (Stage 4G): {total_stage4e}")
print("\n[OK] candidates 생성 완료 (Stage 4G). Tier 4G + RP4G 는 simulate 단계에서 적용됨.")




# =========================================================
# ★★★ 셀 3: 단일 시나리오 백테스트 실행 (Stage 4G) ★★★
# =========================================================
import os
OUTDIR = "stage4g_outputs"
...
STAGE4G_CASE = {
    "name":  "stage4g",
    "label": f"Stage 4G: SWEEP_BASE Rescue (OUT={OUT_BEHAVIOR})",
    "risk_mult": 1.0,
...
print()
print("#" * 75)
print(f"# Stage 4G 백테스트 시작: {STAGE4G_CASE['label']}")
print("#" * 75)

res = simulate_scenario_v19b_rpboost(
    scenario=SCENARIO_MULTI,
    candidates_dict=STAGE4G_CASE["candidates"],
    risk_multiplier=STAGE4G_CASE["risk_mult"],
)

monthly_report = print_scenario_summary(res)

prefix = f"stage4g_out_{OUT_BEHAVIOR}"
...
summary_df = pd.DataFrame([{
    "scenario":  STAGE4G_CASE["name"],
    "label":     STAGE4G_CASE["label"],
    "out_behavior": OUT_BEHAVIOR,
    "trades":    len(trades),
    "win%":      wr_overall,
    "PF":        pf_overall,
...
summary_df.to_csv(f"{OUTDIR}/stage4g_out_{OUT_BEHAVIOR}_overall_summary.csv", index=False)

print()
print("=" * 75)
print(f"[Stage 4G | OUT={OUT_BEHAVIOR}] 전체 성과")
print("=" * 75)
...
if len(tdf) == 0:
    ...
    print()
    print("=" * 95)
    print("[1] tier_stage4g_distribution — Tier 별 성과")
    print("=" * 95)
    print(f"{'tier':<8} {'trades':>7} {'win%':>7} {'avg_R':>7} {'PF':>7} "
          f"{'$/trade':>10} {'total $':>14}")
    print("-" * 95)

    tier_order = ["ALPHA_MAX", "ALPHA_HIGH", "ALPHA_MED", "SWEEP_BASE", "COMPLETE_OUT", "SKIP_MSS"]
    tier_rows = []
    for t in tier_order:
    ...
    tier_df.to_csv(f"{OUTDIR}/tier_stage4g_distribution_out_{OUT_BEHAVIOR}.csv", index=False)
...
print()
print("=" * 75)
print(f"[SAVED] Stage 4G 분석 산출물 (OUT={OUT_BEHAVIOR})")
print("=" * 75)
print(f"  {OUTDIR}/stage4g_out_{OUT_BEHAVIOR}_trades.csv")
print(f"  {OUTDIR}/stage4g_out_{OUT_BEHAVIOR}_equity.csv")
print(f"  {OUTDIR}/stage4g_out_{OUT_BEHAVIOR}_skipped.csv")
print(f"  {OUTDIR}/stage4g_out_{OUT_BEHAVIOR}_monthly.csv")
print(f"  {OUTDIR}/stage4g_out_{OUT_BEHAVIOR}_overall_summary.csv")
print(f"  {OUTDIR}/tier_stage4g_distribution_out_{OUT_BEHAVIOR}.csv")
print(f"  {OUTDIR}/tier_x_side_out_{OUT_BEHAVIOR}.csv")
print(f"  {OUTDIR}/rp_x_tier_out_{OUT_BEHAVIOR}.csv")

print()
print("=" * 75)
print("[Stage 4G] 실험 완료 — SWEEP_BASE 511 구조 + side-aware")
print("=" * 75)
print(f"  OUT_BEHAVIOR: {OUT_BEHAVIOR}")
print(f"  Tier 체계: ALPHA_MAX(2.0x) > ALPHA_HIGH(1.5x) > ALPHA_MED(LONG 0.8x/SHORT 1.2x) > SWEEP_BASE(1.0x) > COMPLETE_OUT")
print(f"  RP Boost:  RP0 only (2.0x), RP1~5 = 1.0x")
print(f"  예상 분포: ALPHA_MAX ~102 / ALPHA_HIGH ~428 / ALPHA_MED ~166 / SWEEP_BASE ~511 / COMPLETE_OUT ~415 / SKIP ~2")
print(f"  IN ~1207 / OUT ~417 (COMPLETE_OUT skip)")
print(f"  다음: 결과 검증 후 ALPHA_MED LONG / COMPLETE_OUT 추가 튜닝")
