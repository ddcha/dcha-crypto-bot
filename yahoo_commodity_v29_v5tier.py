# =========================================================
# YAHOO COMMODITY FUTURES BACKTEST — v2.9 V5 TIER + RISK MULT
#
# [v29 변경점 (vs v28)]
#   ★★★ V5 TIER 분류 (S/A/B/X) 적용 ★★★
#       - V5 cutoff: S Win≥90% AvgR≥1.3 / A Win≥80% AvgR≥0.65 / B Win≥68% AvgR≥0.35
#       - Hierarchical: S 우선 → A → B → X (SKIP)
#       - X tier (UNCL) 모두 진입 차단
#
#   ★★★ RISK_MULT_SCENARIO 으로 3가지 옵션 선택 ★★★
#       AGGRESSIVE  : S=2.5,  A=1.2,  B=0.6   (보수적 정수배, 권장)
#       SQRT_PF     : S=4.42, A=1.30, B=0.81  (sqrt(PF/baseline) 비례)
#       AVGR_PROP   : S=11.15,A=4.41, B=1.0   (AvgR 비례, 매우 공격적)
#
#   ★★★ score 7.5 gate 유지, atom 34개 모두 기록 (v28 그대로) ★★★
#   ★★★ same_symbol/max_total_risk/volume_filter 우회 유지 ★★★
#
# [V5 TIER RULES - hierarchical match]
#   [S] (2 rules)
#     • atoms_a_score_ge13 & t4h_high_sweep_vol
#     • atoms_a_pre_total_ge4 & t4h_max_score_pre & t4h_sweep_room_fvg
#
#   [A] (12 rules) - score, score+fvg, sweep_vol+match_S/C, fvg+match_S, ...
#   [B] (10 rules) - volume, wick+rp3, match_S, mss+rp3, ...
# =========================================================
#
# =========================================================
...
OUTDIR = Path(f"yahoo_commodity_v29_{RISK_SCENARIO}_outputs")
...
print("=" * 80)
print(f"YAHOO COMMODITY v29 — V5 TIER + {RISK_SCENARIO} (S/A/B/X risk mult, 14 commodities)")
print("=" * 80)
...
print()
print(f"★ v29 SCORE_ONLY_GATE:    {SCORE_ONLY_GATE}  (True → score 만 통과한 모든 거래)")
print(f"★ v29 USE_V5_TIER_GATE:   {USE_V5_TIER_GATE}  (True → V5 tier S/A/B/X 분류 적용)")
print(f"★ v29 RISK_SCENARIO:      {RISK_SCENARIO}")
print(f"   → S × {V5_TIER_MULT['S']:.2f}, A × {V5_TIER_MULT['A']:.2f}, B × {V5_TIER_MULT['B']:.2f}, X = SKIP")
if SKIP_ASSETS:
    print(f"★ v29 SKIP_ASSETS:        {sorted(SKIP_ASSETS)}")
else:
    print(f"★ v29 SKIP_ASSETS:        (없음, 환경변수 SKIP_ASSETS 로 지정)")
print(f"★ ATOM 34개 + V5 tier 저장")
...
def simulate_portfolio_v25(candidates_packs, max_total_risk=0.1325, scenario_name="BOOST15"):
        ...
        if current_time in entry_map:
            ...
            for idx, row in entry_events:
                ...
                open_positions[idx] = {
                    ...
                    "tier_v5": tier_v5,
                    "v5_tier_mult": v5_mult if USE_V5_TIER_GATE else 1.0,
                    ...
                    # ★ v29: V5 tier ★
                    "tier_v5_final": tier_v5 if USE_V5_TIER_GATE else "OFF",
    ...
    return {
        ...
        "strategy_variant": f"v29_{RISK_SCENARIO}_BOOST15",
        "scenario": scenario_name,
        "trades": trades_df,
        "skipped": skipped_df,
        "equity": equity_df,
...
if __name__ == "__main__":
    ...
    try:
        res["trades"].to_csv(outdir / f"v29_{RISK_SCENARIO}_trades.csv", index=False)
        res["equity"].to_csv(outdir / f"v29_{RISK_SCENARIO}_equity.csv", index=False)
        res["skipped"].to_csv(outdir / f"v29_{RISK_SCENARIO}_skipped.csv", index=False)
        res["phase_transitions"].to_csv(outdir / f"v29_{RISK_SCENARIO}_phase_transitions.csv", index=False)
        res["excess_log"].to_csv(outdir / f"v29_{RISK_SCENARIO}_excess_log.csv", index=False)
        res["tax_log"].to_csv(outdir / f"v29_{RISK_SCENARIO}_tax_log.csv", index=False)
        all_cand.to_csv(outdir / f"v29_{RISK_SCENARIO}_all_candidates.csv", index=False)
        print(f"\n💾 CSV 저장 완료: {outdir}/")
    ...
    if len(tdf) == 0:
        ...
        summary_df.to_csv(outdir / f"v29_{RISK_SCENARIO}_summary.csv", index=False)
        ...
        monthly_summary.to_csv(outdir / f"v29_{RISK_SCENARIO}_monthly_summary.csv", index=False)
        ...
        asset_summary.to_csv(outdir / f"v29_{RISK_SCENARIO}_asset_summary.csv", index=False)
        ...
        if "tier_label" in tdf.columns:
            ...
            tier_summary.to_csv(outdir / f"v29_{RISK_SCENARIO}_tier_summary.csv", index=False)
        ...
        if "tier_v5_final" in tdf.columns:
            ...
            v5_summary.to_csv(outdir / f"v29_{RISK_SCENARIO}_v5_tier_summary.csv", index=False)

        # 차트
        try:
            plot_equity_v25(res, save_path=str(outdir / f"v29_{RISK_SCENARIO}_equity_chart.png"))
        except Exception as e:
