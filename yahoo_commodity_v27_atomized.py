# =========================================================
# YAHOO COMMODITY FUTURES BACKTEST — v2.7 ATOMIZED (5-COMBO WHITELIST + RISK FLAT)
#
# [v27 변경점 (vs v26)]
#   ★★★ 5 조합 whitelist 외 모두 SKIP (commodity v26 백테 결과 기반) ★★★
#       1. ALPHA_MAX × S   (n=10, PF 3.56,  Win 90.0%)
#       2. ALPHA_MAX × A   (n=6,  PF 129.5, Win 83.3%)  ← 보석
#       3. ALPHA_HIGH × S  (n=9,  PF 2.74,  Win 66.7%)
#       4. ALPHA_HIGH × B  (n=10, PF 5.22,  Win 70.0%)
#       5. ALPHA_MED × S   (n=6,  PF 25.27, Win 83.3%)  ← WIN69+S 환상
#   ★★★ Risk 모두 평탄화 (1.0 통일) ★★★
#   ★★★ Trade dict 에 atom 모두 저장 (atom OR search 용) ★★★
# =========================================================
#
# =========================================================
...
ATOMIZED_MODE = True
ALLOWED_TIER_COMBOS = {
    ("ALPHA_MAX",  "S"),   # n=10, PF 3.56,  Win 90.0%
    ("ALPHA_MAX",  "A"),   # n=6,  PF 129.5, Win 83.3%  ★보석
    ("ALPHA_HIGH", "S"),   # n=9,  PF 2.74,  Win 66.7%
    ("ALPHA_HIGH", "B"),   # n=10, PF 5.22,  Win 70.0%
    ("ALPHA_MED",  "S"),   # n=6,  PF 25.27, Win 83.3%  ★WIN69+S
}
USE_FLAT_RISK_MULT = True
...
OUTDIR = Path("yahoo_commodity_v27_atomized_outputs")
...
print("=" * 80)
print("YAHOO COMMODITY v27 — ATOMIZED (5-COMBO WHITELIST + RISK FLAT, 14 COMMODITIES)")
print("=" * 80)
...
print()
print(f"★ v27 ATOMIZED_MODE: {ATOMIZED_MODE}")
print(f"★ v27 USE_FLAT_RISK_MULT: {USE_FLAT_RISK_MULT} (모든 mult 1.0)")
print(f"★ v27 ALLOWED_TIER_COMBOS ({len(ALLOWED_TIER_COMBOS)} combos):")
for c in sorted(ALLOWED_TIER_COMBOS):
    print(f"     - {c[0]} × {c[1]}")
    ...
    for current_time in event_times:
        ...
        if current_time in entry_map:
            ...
            for idx, row in entry_events:
                ...
                # ★★★ v27 신규: 5 조합 whitelist 차단 ★★★
                if ATOMIZED_MODE and (tier_label, tier_v19b_label) not in ALLOWED_TIER_COMBOS:
                    skipped_entries.append({
                        "entry_time": row["entry_time"], "symbol_key": symbol_key,
                        "reason": f"v27_combo_skip_{tier_label}_x_{tier_v19b_label}",
                        "tier_label": tier_label, "tier_v19b": tier_v19b_label,
                        "phase_at_entry": current_phase_for_entry,
                    })
                    continue

                # ★★★ v27 신규: Risk 평탄화 (모든 mult 1.0) ★★★
                ...
                open_positions[idx] = {
                    ...
                    "rp_eq_4": rp_v == 4,
                    "rp_eq_5": rp_v == 5,
                    "is_alpha_max":  tier_label == "ALPHA_MAX",
                    "is_alpha_high": tier_label == "ALPHA_HIGH",
                    "is_alpha_med":  tier_label == "ALPHA_MED",
                    "is_v19b_S": tier_v19b_label == "S",
                    "is_v19b_A": tier_v19b_label == "A",
                    "is_v19b_B": tier_v19b_label == "B",
                    "is_v19b_C": tier_v19b_label == "C",
