#!/usr/bin/env python3
# =========================================================================
# run_negcut_union.py — setups_pf12_negcut.json(14셋업) union 게이트 단일 백테스트 + 자본시뮬
#   pf12(19셋업)에서 총R<0 손실셋업 5개 제거 = 14셋업. 클린 엔진(_ls_* 비멱등 픽스 적용).
#   핵심 질문: pf12(견고, PF~1.31)처럼 나오나, pf14(14셋업, OOS 0.90 약화)처럼 나오나.
#   독립 프로세스 1패스. 멱등성 assert + 룩어헤드 통과 후 자본시뮬.
#   baseline: OB_MODE=engulf, DISP_ATR_MULT=1.3, USE_H1_REFINE=1, HONEST_STAGE=5, risk_mult=1.0
# =========================================================================
import os, json, time
...
os.environ["COMBO_UNION_JSON"] = "setups_pf12_negcut.json"
...
SETUPS_JSON = "setups_pf12_negcut.json"
OUTDIR = "pf12_negcut_union_result"
# pf12 손실제거 사후 기대값 (비교용)
REF = {"n": 1186, "PF": 1.30, "OOS": 1.35}
...
def reset_prepared(prepared):
    # 멱등성: _ls_*(advance_zone_lifespan 캐시) + used 제거. 엔진이 이미 픽스됐지만 벨트앤서스펜더.
    for sym in prepared:
...
def r_unit_mdd(r_series):
    # exit_time 순 누적 R 곡선의 최대낙폭(R 단위)
    cum = np.cumsum(np.asarray(r_series, float))
...
def summarize(name, trades):
    ...
    row["MDD_cap_pct"] = round(mdd * 100, 2)
...
def setup_breakdown(trades):
    ...
    for key, grp in d.groupby("matched_setup"):
        ...
        rows.append({"matched_setup": key, "n": n,
                     ...
                     "expR": round(float(g["r_multiple"].astype(float).mean()), 4),
...
def main():
    print(f"[INFO] baseline OB_MODE={os.environ['OB_MODE']} DISP_ATR_MULT={os.environ['DISP_ATR_MULT']} "
          ...
          f"셋업 {len(SETUPS)}개(pf12 손실5제거) | 자본 시드{SEED}+{MONTHLY}x{NDEP}")
    ...
    print("\n[STEP2] candidates(union 게이트 14셋업) — 멱등성 검증 겸 2패스")
    t1 = time.time()
    ...
    assert n1 == n2, f"[ABORT] 비멱등 — candidate {n1}!={n2} (엔진 픽스 미작동?)"
    print("  → 멱등성 OK (재실행 candidate 동일)")
    ...
    if zero != 0:
        raise SystemExit(f"[ABORT] 게이트 진입했으나 post-hoc 매칭0 {zero}건 — 게이트/귀속 불일치")
    ...
    print(f"\n[저장] {OUTDIR}/trades.csv ({len(trades)}행, +matched_setup +n_setups_matched, a_trend_align 포함)")

    row = summarize("pf12_negcut", trades)
    ...
    print("\n" + "=" * 70)
    print("=== 전체 결과 (pf12_negcut, 14셋업) ===")
    print("=" * 70)
    for k in ["n", "win_pct", "PF", "IS_PF", "OOS_PF", "oosN", "expR", "Sharpe",
              "MDD_R", "r6_PF", "r6_n", "all_years_ge1"]:
    ...
    print(f"    final_cap={row['final_cap']}  mult={row['mult_vs_dep']}x  CAGR={row['CAGR_pct']}%  "
          f"MDD_cap={row['MDD_cap_pct']}%  (MDD_R={row['MDD_R']})")
    ...
    print("\n=== pf12 손실제거 사후값 대비 ===")
    print(f"  거래수: 실측 {row['n']} / 사후 {REF['n']}  (Δ{dn:.1f}%)")
    print(f"  PF    : 실측 {row['PF']} / 사후 {REF['PF']}  (Δ{dpf:.1f}%)")
    print(f"  OOS_PF: 실측 {row['OOS_PF']} / 사후 {REF['OOS']}  (Δ{doos:.1f}%)")
    flag = (dn > 15) or (dpf > 15)
    print(f"  → {'⚠️ 강제대체 플래그(pf14처럼 약화) — 거래수/PF 15%+ 괴리' if flag else 'OK — 사후값과 근접(pf12처럼 견고)'}")
    verdict = "pf12형 견고" if (row["OOS_PF"] >= 1.0 and row["all_years_ge1"]) else "pf14형 약화(OOS<1 또는 매년실패)"
    print(f"  [판정] {verdict}")
