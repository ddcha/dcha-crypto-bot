# =========================================================
# smc_crypto_9coins  STAGE 4J: BOOSTER + ADDITIONAL BLOCK (1.0× base + layered mult)
#
# [기반]
#   Stage 4I 의 모든 차단 규칙 유지 (GEM SHORT, ROOM_ONLY 4심볼 BLACK)
#
# [Stage 4J 신규 매트릭스] — risk_mult 곱셈 layer (우선순위: 차단 > 1.5× > 1.2× > 1.0×)
#
#   ── 추가 차단 (mult = 0.0) ──
#     GEM LONG RP1   : BTC R=167 단건 의존 (top1 의존도 105%, 나머지 적자)
#     ROOM_ONLY AVAX : AVAX R=33 단건 의존 (top1 의존도 115%, 나머지 적자)
#
#   ── 1.5× booster (outlier 제거 후도 PF≥3.0) ──
#     ROOM_FVG + DOGE       : n=36, win 77.8%, PF 3.96, top3 제거시 PF 3.01
#     GEM LONG + RP2        : n=13, win 76.9%, PF 10.57, top3 제거시 PF 6.58
#
#   ── 1.2× booster (PF 안정적) ──
...
# =========================================================
# Stage 4I 차단 규칙 그대로 유지 + outlier 의존 그룹 추가 차단
# + 매트릭스 기반 1.5× / 1.2× / 1.0× extra mult layer
# 리스크 배수 base: 모두 1.0× (= Scen B 평평)
# Runner time exit: 그대로 풀어둠 (수동 청산 의도)

SCENARIO = "BOOSTER"   # Stage 4J 단일 시나리오
...
# Stage 4J: 단일 시나리오 (모두 1.0×) — base mult 는 평평
# 1.5× / 1.2× booster 는 별도 layer 로 곱셈 (get_stage4j_extra_mult)
...
# ─── Stage 4I 신규 차단 규칙 ───
# Scen B 분석 결과 기반 — 거래 단에서 risk_mult=0 으로 차단
GEM_LONG_ONLY = True                                    # SWEEP_GEM SHORT 차단
ROOM_ONLY_SYMBOL_BLACKLIST = {                          # SWEEP_ROOM_ONLY 적자 심볼
    "BTCUSDT", "SOLUSDT", "BNBUSDT", "LINKUSDT",
}

# ─── Stage 4J 신규 BLOCK + BOOSTER 매트릭스 ───
# 우선순위: 차단 > 1.5× > 1.2× > 1.0×
# 적용 시점: Stage 4I 차단 통과 후 한번 더 곱셈 layer

# (a) Stage 4J 추가 차단 — outlier 1건 의존 그룹
def is_stage4j_blocked(tier, side, symbol, run_potential):
    """outlier 1건이 PF 거의 전부 책임지는 도박 그룹 차단."""
    rp = int(run_potential) if run_potential is not None else -1
    # GEM LONG RP1 — BTC R=167 한 건 (top1 105% 의존, 나머지 16t 적자)
    if tier == "SWEEP_GEM" and side == "long" and rp == 1:
        return "GEM_LONG_RP1_outlier_dep"
    # ROOM_ONLY AVAX — AVAX R=33 한 건 (top1 115% 의존, 나머지 12t 적자)
    if tier == "SWEEP_ROOM_ONLY" and symbol == "AVAXUSDT":
        return "ROOM_ONLY_AVAX_outlier_dep"
    return None

# (b) Stage 4J extra mult — 1.5× / 1.2× booster
def get_stage4j_extra_mult(tier, side, symbol, run_potential):
    """
    Stage 4J 추가 곱셈 layer. 차단 통과한 거래에 한해 호출.
    우선순위: 1.5× > 1.2× > 1.0× (default).
    Returns: (mult, label)  e.g. (1.5, 'BOOST_DOGE_FVG')
    """
    rp = int(run_potential) if run_potential is not None else -1
    
    # ── 1.5× booster (robust, outlier 제거 후도 PF≥3.0) ──
    # ROOM_FVG + DOGE
    ...
    for current_time in event_times:
        ...
        if current_time in entry_map:
            for idx, row in entry_map[current_time]:
                ...
                # 최종 risk 계산 (Stage 4J 체계)
                risk_pct = risk_pct_base * tier_mult_4e * rp_mult_4e * stage4j_mult * risk_multiplier
                effective_notional_cap = get_tier_cap_4e(tier_4e)
                
                # 하위 호환용 변수
                tier_label = tier_4e
                tier_mult  = tier_mult_4e
                sa_cap_tag = f"tier_{tier_4e}_rp{rp_v}_{stage4j_label}"
...
print()
print("=" * 75)
print("[Stage 4J] candidates 생성 — SWEEP 세분화 + 차단/booster layer")
print("=" * 75)
...
# =========================================================
# ★★★ 셀 3: Stage 4J — 단일 시나리오 백테스트 (BOOSTER) ★★★
# =========================================================
import os
OUTDIR = "stage4j_outputs"
os.makedirs(OUTDIR, exist_ok=True)
print(f"\n[OUTDIR] {os.path.abspath(OUTDIR)}")
print(f"[CONFIG] OUT_BEHAVIOR = {OUT_BEHAVIOR!r}")
...
SCENARIO_DESCRIPTIONS = {
    "BOOSTER": "Stage 4J BOOSTER (4I 차단 + outlier 추가 차단 + 1.5×/1.2× booster matrix)",
}

# 결과 누적 (단일 키지만 구조 유지)
scenario_results = {}

SCENARIO_CURRENT = "BOOSTER"
SCENARIO = SCENARIO_CURRENT
globals()["SCENARIO"] = SCENARIO_CURRENT

print()
print("#" * 80)
print(f"# Stage 4J {SCENARIO_CURRENT}: {SCENARIO_DESCRIPTIONS[SCENARIO_CURRENT]}")
print(f"#   base SWEEP_GEM       × {SWEEP_RISK_BY_SCENARIO[SCENARIO_CURRENT]['SWEEP_GEM']}  (LONG only, 4I) + RP1 차단 (4J)")
print(f"#   base SWEEP_ROOM_FVG  × {SWEEP_RISK_BY_SCENARIO[SCENARIO_CURRENT]['SWEEP_ROOM_FVG']}  (전체 유지)")
print(f"#   base SWEEP_ROOM_ONLY × {SWEEP_RISK_BY_SCENARIO[SCENARIO_CURRENT]['SWEEP_ROOM_ONLY']}  (BLOCK: BTC/SOL/BNB/LINK + AVAX 추가)")
print(f"#   booster: 1.5×=DOGE_FVG, GEM_LONG_RP2 / 1.2×=GEM_LONG_RP0, ROOM_ONLY_ADA, ROOM_ONLY_LONG_RP1, FVG_LONG_RP2")
print("#" * 80)

STAGE4J_CASE = {
    "name":  f"stage4j_{SCENARIO_CURRENT.lower()}",
    "label": f"Stage 4J [{SCENARIO_CURRENT}]: {SCENARIO_DESCRIPTIONS[SCENARIO_CURRENT]}",
    "risk_mult": 1.0,
...
res = simulate_scenario_v19b_rpboost(
    ...
    candidates_dict=STAGE4J_CASE["candidates"],
    risk_multiplier=STAGE4J_CASE["risk_mult"],
)

monthly_report = print_scenario_summary(res)

prefix = f"stage4j_{SCENARIO_CURRENT.lower()}_out_{OUT_BEHAVIOR}"
...
summary_row = {
    "scenario":          SCENARIO_CURRENT,
    "label":             STAGE4J_CASE["label"],
    "sweep_gem_mult":    SWEEP_RISK_BY_SCENARIO[SCENARIO_CURRENT]["SWEEP_GEM"],
    "sweep_room_fvg":    SWEEP_RISK_BY_SCENARIO[SCENARIO_CURRENT]["SWEEP_ROOM_FVG"],
    "sweep_room_only":   SWEEP_RISK_BY_SCENARIO[SCENARIO_CURRENT]["SWEEP_ROOM_ONLY"],
    "gem_long_only":     GEM_LONG_ONLY,
    "room_only_blacklist": ",".join(sorted(ROOM_ONLY_SYMBOL_BLACKLIST)),
    "stage4j_blocks":    "GEM_LONG_RP1, ROOM_ONLY_AVAX",
    "stage4j_boost15":   "FVG_DOGE, GEM_LONG_RP2",
    "stage4j_boost12":   "GEM_LONG_RP0, ROOM_ONLY_ADA, ROOM_ONLY_LONG_RP1, FVG_LONG_RP2",
    "out_behavior":      OUT_BEHAVIOR,
    "trades":            len(trades),
    "win%":              wr_overall,
    "PF":                pf_overall,
...
print()
print("=" * 75)
print(f"[Stage 4J {SCENARIO_CURRENT}] 전체 성과")
print("=" * 75)
...
if len(tdf) > 0 and "tier_4e" in tdf.columns:
    ...
    tier_order = ["ALPHA_MAX", "ALPHA_HIGH", "ALPHA_MED",
    ...
    print(f"[1] Tier 분포")
    print("-" * 100)
    print(f"{'tier':<18} {'trades':>7} {'win%':>7} {'avg_R':>7} {'PF':>7} "
          f"{'$/trade':>10} {'total $':>14}")
    tier_rows = []
    for t in tier_order:
        ...
        print(f"{t:<18} {int(st['trades']):>7} {st['win_pct']:>6.2f}% "
              f"{st['avg_R']:>7.3f} {st['PF']:>7.3f} "
              f"{st['pnl_per_trade']:>10,.0f} {st['total_pnl']:>14,.0f}")
    pd.DataFrame(tier_rows).round(4).to_csv(
        f"{OUTDIR}/tier_stage4j_{SCENARIO_CURRENT.lower()}_distribution.csv", index=False)
    ...
    for t in tier_order:
        ...
        for side in ["long", "short"]:
            ...
            print(f"{t:<18} {side:<6} {int(st['trades']):>5} {st['win_pct']:>6.2f}% "
                  f"{st['PF']:>7.3f} {st['total_pnl']:>14,.0f}")
    pd.DataFrame(tx_rows).round(4).to_csv(
        f"{OUTDIR}/tier_x_side_stage4j_{SCENARIO_CURRENT.lower()}.csv", index=False)
    ...
    if "run_potential" in tdf.columns:
        for t in tier_order:
            ...
            for rp in sorted(sub_t["run_potential"].dropna().unique().tolist()):
                ...
                print(f"{t:<18} RP{int(rp):<2} {int(st['trades']):>5} {st['win_pct']:>6.2f}% "
                      f"{st['PF']:>7.3f} {st['total_pnl']:>14,.0f}")
    pd.DataFrame(rt_rows).round(4).to_csv(
        f"{OUTDIR}/rp_x_tier_stage4j_{SCENARIO_CURRENT.lower()}.csv", index=False)
    
    # [4] Stage 4I 신규: SWEEP 계열 Tier × Symbol — 차단 효과 검증용
    print()
    print("-" * 100)
    print(f"[4] SWEEP 계열 Tier × Symbol (차단 결과 검증)")
    print("-" * 100)
    ts_rows = []
    for t in ["SWEEP_GEM", "SWEEP_ROOM_FVG", "SWEEP_ROOM_ONLY"]:
        sub_t = tdf[tdf["tier_4e"] == t]
        if len(sub_t) == 0: continue
        for sym in sorted(sub_t["symbol"].dropna().unique().tolist()):
            sub = sub_t[sub_t["symbol"] == sym]
            if len(sub) == 0: continue
            st = _stats(sub)
            ts_rows.append({"tier": t, "symbol": sym, **st})
            print(f"{t:<18} {sym:<10} {int(st['trades']):>5} {st['win_pct']:>6.2f}% "
                  f"{st['PF']:>7.3f} {st['total_pnl']:>14,.0f}")
    pd.DataFrame(ts_rows).round(4).to_csv(
        f"{OUTDIR}/tier_x_symbol_stage4j_{SCENARIO_CURRENT.lower()}.csv", index=False)
    
    # [5] Stage 4I/4J 차단된 거래 사유 분포
    print()
    print("-" * 100)
    print(f"[5] Skipped 거래 사유 분포 (Stage 4I + Stage 4J 차단 효과)")
    print("-" * 100)
    sk = res["skipped"]
    if len(sk) > 0 and "reason" in sk.columns:
        reason_counts = sk["reason"].value_counts()
        print(reason_counts.to_string())
        # Stage 4I 차단
        stage4i_blocks = sk[sk["reason"].str.startswith("stage4i_skip_")]
        print(f"\n  ── Stage 4I 차단: {len(stage4i_blocks)}건 ──")
        if len(stage4i_blocks) > 0:
            print(stage4i_blocks["reason"].value_counts().to_string())
        # Stage 4J 추가 차단
        stage4j_blocks = sk[sk["reason"].str.startswith("stage4j_skip_")]
        print(f"\n  ── Stage 4J 신규 추가 차단: {len(stage4j_blocks)}건 ──")
        if len(stage4j_blocks) > 0:
            print(stage4j_blocks["reason"].value_counts().to_string())
...
print()
print("#" * 85)
print("# Stage 4J — BOOSTER 결과 요약")
print("#" * 85)

comp_df = pd.DataFrame([scenario_results[SCENARIO_CURRENT]])
comp_df.to_csv(f"{OUTDIR}/stage4j_summary.csv", index=False)

print()
print(f"{'Scenario':<12} {'trades':>7} {'win%':>7} {'PF':>7} {'avg_R':>7} "
      f"{'MDD%':>7} {'Return%':>10} {'퇴사개월':>9} {'peak $':>14}")
print("-" * 100)
r = scenario_results[SCENARIO_CURRENT]
months = r["retirement_months"]
months_str = f"{months}m" if months != "X" else "X"
print(f"{SCENARIO_CURRENT:<12} {r['trades']:>7} {r['win%']:>6.2f}% {r['PF']:>7.3f} {r['avg_R']:>7.3f} "
      f"{r['MDD%']:>6.2f}% {r['Return_%']:>9.1f}% {months_str:>9} ${r['total_assets_peak']:>12,.0f}")

print()
print("-" * 100)
print("Stage 4J 적용 규칙:")
print(f"  [4I 유지] GEM_LONG_ONLY              = {GEM_LONG_ONLY}")
print(f"  [4I 유지] ROOM_ONLY_SYMBOL_BLACKLIST = {sorted(ROOM_ONLY_SYMBOL_BLACKLIST)}")
print(f"  [4J 추가 차단] SWEEP_GEM LONG RP1, SWEEP_ROOM_ONLY AVAX")
print(f"  [4J Booster 1.5×] FVG_DOGE, GEM_LONG_RP2")
print(f"  [4J Booster 1.2×] GEM_LONG_RP0, ROOM_ONLY_ADA, ROOM_ONLY_LONG_RP1, FVG_LONG_RP2")
print(f"  SWEEP risk base            = all 1.0× (평평)")

print()
print("=" * 75)
print("[Stage 4J BOOSTER] 백테스트 완료")
print("=" * 75)
print(f"  산출물: {OUTDIR}/")
print(f"    stage4j_booster_out_skip_*.csv  (trades/equity/monthly/summary/skipped)")
print(f"      └ trades.csv 신규 컬럼: sl_dist_pct, worst_low_during_hold,")
print(f"                              worst_high_during_hold, max_adverse_excursion_pct,")
print(f"                              sl_proximity_pct  ← SL 취약성 분석용")
print(f"    tier_stage4j_booster_distribution.csv")
print(f"    tier_x_side_stage4j_booster.csv")
print(f"    rp_x_tier_stage4j_booster.csv")
print(f"    tier_x_symbol_stage4j_booster.csv")
print(f"    stage4j_summary.csv")
